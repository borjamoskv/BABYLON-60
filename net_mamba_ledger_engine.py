...
class MambaLedgerEngine:
    ...
    def mut_generate_audited(self, prompt: str, max_new_tokens: int=10, temperature: float=1.0, k: int=5) -> Tuple[str, ClosureCertificate]:
        ...
        prompt_node = self.ledger.mut_append_node(parent_id=parent_id, claim=f'Prompt len {len(prompt_ids)}', payload_hash=prompt_hash)
        ...
        cert = ClosureCertificate(evidence_hash=prompt_hash, ruleset_hash=hash_evidence({'model': 'mamba', 'temperature_milli': int(temperature * 1000), 'k': k}), final_state=self.ledger.crdt.to_dict(), residual_microbits=crdt_entropy, epsilon_threshold=1000000)
        cert.verify()
