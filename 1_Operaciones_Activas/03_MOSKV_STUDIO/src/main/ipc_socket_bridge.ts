...
export class IpcSocketBridge {
  ...
  public connect(): void {
    ...
    this.client = net.createConnection(this.socketPath, () => {
      ...
      this.retryCount = 0;
      console.log(
        `[C5-REAL] Electron connected to Agent Igor IPC at ${this.socketPath}`,
      );
    ...
    this.client.on("error", (err) => {
      if (!this.isConnected && this.retryCount < this.maxRetries) {
        this.retryCount++;
        console.warn(
          `[C5-REAL] IPC socket non-ready (${err.message}). Retrying connection (${this.retryCount}/${this.maxRetries})...`,
        );
      ...
      console.warn(
        `[C5-REAL] IPC Connection Warning after ${this.retryCount} retries: ${err.message}. Running in offline UI mode.`,
      );
    });
    ...
    this.client.on("close", (hadError) => {
      ...
      if (hadError) {
        console.warn(
          `[C5-REAL] IPC: Socket closed with error. Will attempt auto-reconnect.`,
        );
    ...
    this.client.on("data", (data) => {
      ...
      if (this.buffer.length > this.MAX_BUFFER_SIZE) {
        console.warn(
          `[C5-REAL] Warning: NDJSON Buffer overflow (>${this.MAX_BUFFER_SIZE} bytes). Clearing buffer.`,
        );
        this.buffer = "";
        return;
      }
        ...
        if (line.trim()) {
          try {
            ...
            if (this.webContents && !this.webContents.isDestroyed()) {
              ...
              console.warn(
                "[C5-REAL] Anergía: WebContents inyectado no existe o fue destruido. Payload descartado.",
              );
            }
          } catch (e) {
            console.warn(
              `[C5-REAL] NDJSON Stream Warning: Ignored malformed payload chunk. ${e instanceof Error ? e.message : "Unknown"}`,
            );
          }
        }
      }
  ...
  public sendPayload(payload: IpcPayload): void {
    if (this.client && !this.client.destroyed && this.isConnected) {
      try {
        ...
        console.warn(
          `[C5-REAL] Warning: Failed to serialize IPC payload: ${e instanceof Error ? e.message : "Unknown"}`,
        );
      }
    } else {
