...
def main():
    ...
    crystal_path = os.path.expanduser(
        f"~/.gemini/antigravity/brain/{conv_id}/autodidact_omega_quantum_error_mitigation_crystal.md"
    )
    if not os.path.exists(crystal_path):
        crystal_path = os.path.expanduser(
            "~/.gemini/antigravity/brain/3a42e487-1db5-423a-891c-58e1833e2964/autodidact_omega_deep_research_crystal.md"
        )
    if not os.path.exists(crystal_path):
