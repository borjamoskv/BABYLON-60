...
/* -------------------------------------------------------------------------- */
/* Unified Batch Execution Pipeline                                           */
/* -------------------------------------------------------------------------- */
...
void execute_10_primitives_neon(const float* a, const float* b, size_t len, primitive_results_t* out) {
    ...
    out->pi_inv = primitive_pi_inv_neon(a, b, len);
    out->pi_entr = primitive_pi_entr_neon(a, b, len);
    out->pi_zk = primitive_pi_zk_neon(a, b, len);
    out->pi_causal = primitive_pi_causal_neon(a, b, len);
    out->pi_ll = primitive_pi_ll_neon(a, b, len);
    out->pi_st = primitive_pi_st_neon(a, b, len, DEFAULT_INFINITESIMAL_EPS, NULL);
    out->pi_pmi = primitive_pi_pmi_neon(a, b, len);
    out->pi_landauer = primitive_pi_landauer_neon(a, b, len, DEFAULT_TEMP_KELVIN);
    out->pi_kl = primitive_pi_kl_neon(a, b, len);
    out->pi_dedup = primitive_pi_dedup_neon(a, b, len, 1e-4f);
}
