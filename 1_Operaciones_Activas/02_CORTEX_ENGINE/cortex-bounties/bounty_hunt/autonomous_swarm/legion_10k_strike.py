...
class Legion10KStrike:
    ...
    async def _execute_centurion(
        self, target: ProtocolTarget, vector: AttackVector
    ) -> None:
        """
    ...
    def _evaluate_vector(self, vector: AttackVector, pass_type: str) -> str:
        ...
        if vector.vector_type == "precision" and pass_type in (
            "AST-structural",
            "state-machine",
        ):
            return "confirm"
    ...
    def _compile_strike_report(self, elapsed_s: float) -> dict:
        ...
        report = {
            ...
            "bounty_potential": {
                "max_per_protocol": {
                    t.name: f"${t.max_bounty:,}" for t in self.targets
                },
