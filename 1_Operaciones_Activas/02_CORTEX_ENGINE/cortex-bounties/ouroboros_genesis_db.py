...
def drop_corrupt_indexes(conn):
    ...
    try:
        ...
        cur.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'vec_facts_%'"
        )
        for row in cur.fetchall():
            cur.execute(f"DROP TABLE IF EXISTS {row['name']}")

        cur.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'vec_void_%'"
        )
        for row in cur.fetchall():
            cur.execute(f"DROP TABLE IF EXISTS {row['name']}")
...
async def rebuild_indexes():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT rowid, id, tenant_id, project_id, content, storage_tier FROM facts_meta"
    )
    rows = cur.fetchall()

    logger.info(
        f"🌌 [OUROBOROS] Re-quantizing and re-embedding {len(rows)} records into INT8."
    )
    ...
    for row in rows:
        ...
        meta_tb, vec_tb, vec_void_tb, mih_tb = store._get_domain_tables(
        ...
        if vec_void_tb:
            insert_cur.execute(
                f"INSERT OR REPLACE INTO {vec_void_tb}(rowid, embedding) VALUES (?, ?)",
                (rowid, binary_bytes),
            )
            # MIH shards
