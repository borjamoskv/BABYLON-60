...
def long_slots( slot_completion, shreds_data, first_turbine):
    ...
    for idx, row in long_slots.iterrows():
        ...
        repairs = shreds_data[(shreds_data['slot'] == row['slot']) & (not shreds_data['is_turbine'])]
    ...
    long_slots['num_repair_requests'] = long_slots['slot'].map(
        lambda slot: len(shreds_data[(shreds_data['slot'] == slot) & (not shreds_data['is_turbine'])])
    )
    ...
    for idx, row in long_slots.iterrows():
        shreds = shreds_data[(shreds_data['slot'] == row['slot']) & (shreds_data['is_turbine'])]
    ...
    long_slots['num_repair_shreds_after_400ms'] = long_slots.apply(
        lambda row: len(shreds_data[(shreds_data['slot'] == row['slot']) & (not shreds_data['is_turbine']) & (shreds_data['timestamp'] >= row['first_shred_ts_fec0'] + 400_000_000) & (shreds_data['timestamp'] <= row['timestamp_fec1'])]),
        axis=1
    )
...
def turbine_stats(catchup, live):
    ...
    for _, repair_shred in live[not live['is_turbine']].iterrows():
    ...
    repaired_during_live = live[not live['is_turbine']]
...
def turbine_shred_timeline(shreds_data, first_turbine, pdf):
    ...
    turbine_shreds = live_data[live_data['is_turbine']]
    repair_shreds = live_data[not live_data['is_turbine']]
    ...
    turbine_norm = live_data_normalized[live_data_normalized['is_turbine']]
    repair_norm = live_data_normalized[not live_data_normalized['is_turbine']]
...
def slot_request_rate_analysis(shreds_data, first_turbine, pdf, include_after_turbine=False):
    ...
    if 'is_turbine' in df_filtered.columns:
        df_filtered = df_filtered[~((df_filtered['is_turbine']) & (df_filtered['slot'] < first_turbine))]
    ...
    if not slot_timestamps:
        print("No slots found with the current filtering criteria")
        return
...
def peer_stats_analysis(repair_requests, shreds_data, pdf):
    ...
    if not reqs_per_ip.empty:
        ...
        ax5.bar(x_positions, sorted_requests.values, color='black', alpha=0.7, width=0.8)
...
def repair_efficiency_heatmap(repair_requests, shreds_data, snapshot_slot, first_turbine, pdf):
    ...
    efficiency_data_buffered['efficiency_ratio'].quantile(0.95)
    ...
    # summary stats
    print("Repair Efficiency Summary:")
...
def generate_report( log_path, request_data_path, shred_data_path, peers_data_path, fec_complete_path=None, pdf=None, include_after_turbine=False ):
    ...
    if request_data_path:
        repair_requests[repair_requests['slot'].between(snapshot_slot, first_turbine - 1)]
        repair_requests[repair_requests['slot'].between(first_turbine, last_executed)]
