...
def get_env_var(var_name):
    ...
    if not value:
        ...
        click.echo("Please activate the agave-cluster environment with: source activate <FIREDANCER_REPO_PATH> <AGAVE_RELEASE_PATH> <LEDGER_DIR>", err=True)
        sys.exit(1)
    return Path(value)
...
def validate_ledger_directory():
    ...
    if not ledger_path.exists():
        ...
        click.echo("Please create the directory or set a different path with: export AGAVE_LEDGER_PATH=<LEDGER_PATH>", err=True)
        sys.exit(1)
    return ledger_path
...
def create_unstaked_keys(ctx, validator_name):
    ...
    click.echo("Identity key content:")
...
def create_vote_account_cmd(ctx, name):
    ...
    click.echo("Creating vote account on-chain...")
...
def create_stake_account_cmd(ctx, name):
    ...
    click.echo("Creating stake account on-chain...")
...
def recreate_vote_account_cmd(ctx, identifier):
    ...
    if check.returncode == 0:
        ...
        click.echo("Vote account not found on-chain, skipping close step")
