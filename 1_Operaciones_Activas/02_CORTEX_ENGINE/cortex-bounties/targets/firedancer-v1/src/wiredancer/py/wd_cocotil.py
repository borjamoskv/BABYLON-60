...
def random_tr(
    ...
    mlen = random.randint(0, 1280) if mlen is None else mlen
    msg = [random.randint(0, 255) for i in range(mlen)] if msg is None else msg
    tr                  = dict()
    ...
    tr['err']           = 100
    tr['src']           = src
    tr['tid']           = random_int(64) if tid is None else tid
    tr['sig_l']         = random_int(256) if sig is None else bits(sig, 256,   0)
    tr['sig_h']         = random_int(256) if sig is None else bits(sig, 256, 256)
    tr['pub']           = random_int(256) if pub is None else pub
    tr['msg_sz']        = len(msg)
    tr['msg_s']         = msg
    ...
    tr['sha_pre_meta']  = sha_pre_meta if sha_pre_meta is not None else build_sha_pre_meta(tr)
    tr['sha_modq_meta'] = sha_modq_meta if sha_modq_meta is not None else build_sha_modq_meta(tr)
...
async def mon_pcie_tr_ext(ddut, dut, clk, q_i=None, q_o=None, do_log=False):
    ...
    while True:
        ...
        if q_i is not None and str(dut.pcie_v) == '1':
        ...
        if str(dut.o_v) == '1' and str(dut.o_r) == '1' and q_o is not None:
...
def build_ed25519_sigverify_0_o(tr):
    ...
    os = tuple(_.ast.literal_eval() for _ in os)
