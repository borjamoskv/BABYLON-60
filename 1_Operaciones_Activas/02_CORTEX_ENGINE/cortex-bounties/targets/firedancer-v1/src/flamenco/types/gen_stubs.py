...
class StaticVectorMember(TypeNode):
    ...
    def emitEncode(self):
        ...
        if self.element == "uchar":
            ...
            print(' TODO: implement this windowed properly', file=body)
            print('  if( FD_UNLIKELY( err ) ) return err;', file=body)
            return
...
class StructType(TypeNode):
    ...
    def __init__(self, json):
        ...
        if "alignment" in json:
            ...
            self.attribute = '__attribute__((packed)) '
            self.alignment = 8
        else:
            self.attribute = ''
            self.alignment = 0
    ...
    def emitHeader(self):
        ...
        if self.isFixedSize():
            ...
            print('/* Encoded Size: Dynamic */', file=header)
    ...
    def emitImpls(self):
        ...
        if self.encoders is not False:
            if self.isFixedSize() and self.isFuzzy():
                ...
                print('  if( ctx->data>=ctx->dataend ) { return FD_BINCODE_ERR_OVERFLOW; };', file=body)
                print('  int err = 0;', file=body)
                for f in self.fields:
                ...
                print('  void const * start_data = ctx->data;', file=body)
                print(f'  int err = {n}_decode_footprint_inner( ctx, total_sz );', file=body)
                print('  if( ctx->data>ctx->dataend ) { return FD_BINCODE_ERR_OVERFLOW; };', file=body)
                print('  ctx->data = start_data;', file=body)
            ...
            print('  void * * alloc_mem = &alloc_region;', file=body)
            print(f'  {n}_decode_inner( mem, alloc_mem, ctx );', file=body)
            print('  return self;', file=body)
            print('}', file=body)
