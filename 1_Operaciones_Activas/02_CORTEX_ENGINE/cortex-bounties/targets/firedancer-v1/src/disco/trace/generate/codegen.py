...
def _write_rec_publish(rec_size: int, f):
    if rec_size == 8:
        ...
        print("  fd_fxt_pub_rec_publish( fd_fxt_pub_cur, sizeof(words) );", file=f)
...
def _write_duration(event: Event, cat: str, strtab: StringTable, f):
    ...
    print("static inline void", file=f)
    print(f"fd_trace_{event.name}_enter( {arg_list} ) {{", file=f)
    print("  ulong const words[] = {", file=f)
    print("    fd_fxt_rec_event_hdr(", file=f)
    print(f"      /* rec_sz     */ {enter_size}UL,", file=f)
    print("      /* event_type */ FD_FXT_EVENT_DURATION_BEGIN,", file=f)
    print(f"      /* arg_cnt    */ {len(event.args)}UL,", file=f)
    print("      /* thread_ref */ fd_fxt_pub_cur->thread_id,", file=f)
    print(f"      /* cat_ref    */ {cat_ref}UL,", file=f)
    print(f"      /* name_ref   */ {name_ref}UL ),", file=f)
    print("    (ulong)fd_tickcount(),", file=f)
    ...
    print("      /* event_type */ FD_FXT_EVENT_DURATION_END,", file=f)
    print("      /* arg_cnt    */ 0UL,", file=f)
    print("      /* thread_ref */ fd_fxt_pub_cur->thread_id,", file=f)
    print(f"      /* cat_ref    */ {cat_ref}UL,", file=f)
    print(f"      /* name_ref   */ {name_ref}UL ),", file=f)
    print("    (ulong)fd_tickcount(),", file=f)
