...
def make_recover_var(n, max_shreds):
    ...
    with open(f'fd_reedsol_recover_{n}.c', 'wt') as outf:
        ...
        cprint("ulong shred_cnt = data_shred_cnt + parity_shred_cnt;")

        cprint('ulong loaded_cnt = 0UL;')
        cprint(f'for( ulong i=0UL; i<{n}UL; i++) ' + '{')
        cprint('int load_shred = ((i<shred_cnt)&(loaded_cnt<data_shred_cnt))&&( erased[ i ]==0 );')
        cprint('_erased[ i ] = !load_shred;')
        cprint('loaded_cnt += (ulong)load_shred;')
        cprint('}')

        cprint('if( FD_UNLIKELY( loaded_cnt<data_shred_cnt ) ) return FD_REEDSOL_ERR_PARTIAL;')
        ...
        cprint("#define ALL_VARS " + ", ".join(all_vars))
        if n>64:
            cprint("#define ALL_VARS_REF &" + ", &".join(all_vars))
        cprint('')
        if n>64:
