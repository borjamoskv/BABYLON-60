...
def main() -> None:
    ...
    logging.getLogger(__name__).info(f"🚀 Running Repository Benchmarks...{label}")

    cold_import_time, import_time = bench_imports()
    logging.getLogger(__name__).info(f"📦 Core Import Time (cold): {cold_import_time:.4f}s")
    logging.getLogger(__name__).info(f"📦 Core Import Time (steady-state): {import_time:.4f}s")

    size_mb = check_package_size()
    logging.getLogger(__name__).info(f"💽 Cortex Source Size: {size_mb:.2f} MB")
    ...
    if cold_import_time > STEADY_STATE_IMPORT_THRESHOLD:
        logging.getLogger(__name__).info("ℹ️ Cold import exceeded the steady-state budget, but warmed import passed.")

    logging.getLogger(__name__).info("✅ All benchmarks passed.")
