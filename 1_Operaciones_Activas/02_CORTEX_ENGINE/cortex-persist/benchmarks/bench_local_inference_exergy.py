...
def simulate_inference(
    ...
    exergy_efficiency = (
        1.0 - (total_exergy_destroyed / total_energy_consumed) if total_energy_consumed > 0 else 0.0
    )
...
def print_report():
    ...
    logging.getLogger(__name__).info("  AUDITORÍA DE SILICIO LOCAL: INFERENCIA DE LLMS BAJO TERMODINÁMICA DE LA INFORMACIÓN")
    logging.getLogger(__name__).info(f"  Contexto Simulado: {context_length} tokens | Autor: Borja Moskv | Ledged-ID: C5-REAL")
    logging.getLogger(__name__).info("=================================================================================\n")
    ...
    logging.getLogger(__name__).info("# Matriz Termodinámica de Ejecución Local (Resultados de Simulación C4-SIM)\n")
    logging.getLogger(__name__).info(
        "| Arquitectura Silicio | Tiempo Total (s) | Latencia Media (ms/tok) | Temp Unión Media (°C) | Energía Total (J) | Exergía Destruida (J) | Eficiencia Exergética |"
    )
    logging.getLogger(__name__).info("| :--- | :---: | :---: | :---: | :---: | :---: | :---: |")
    for r in results:
        logging.getLogger(__name__).info(
    ...
    for r in results:
        ...
        logging.getLogger(__name__).info(f"- **{r['architecture']}**:")
        logging.getLogger(__name__).info(f"  - Latencia Inicial (Token 1): {initial_lat:.2f} ms")
        logging.getLogger(__name__).info(f"  - Latencia Final (Token {context_length}): {final_lat:.2f} ms")
        logging.getLogger(__name__).info(f"  - Crecimiento de Latencia por Carga de KV-Cache: {lat_growth:.1f}%")
        logging.getLogger(__name__).info(
            f"  - Tasa de Destrucción de Exergía Media: {r['total_exergy_destroyed_j'] / r['total_latency_s']:.3e} J/s"
        )

    logging.getLogger(__name__).info("\n=================================================================================")
    logging.getLogger(__name__).info("Fin del reporte de simulación de Exergía.")
    logging.getLogger(__name__).info("=================================================================================")
