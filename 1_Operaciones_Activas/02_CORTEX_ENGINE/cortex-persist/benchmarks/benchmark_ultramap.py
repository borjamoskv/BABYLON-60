...
def run_benchmark(capacity, iterations=10000):
    ...
    logging.getLogger(__name__).info(f"Allocation Time: {alloc_time:.4f}s")
    ...
    for op, times in metrics.items():
        ...
        logging.getLogger(__name__).info(f"[{op}]")
        logging.getLogger(__name__).info(f"  ops/sec : {ops_sec:.2f}")
        logging.getLogger(__name__).info(f"  p50     : {p50:.4f} ms")
        logging.getLogger(__name__).info(f"  p95     : {p95:.4f} ms")
        logging.getLogger(__name__).info(f"  p99     : {p99:.4f} ms")
