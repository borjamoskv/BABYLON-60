...
use babylon60_types::{hex32, hash_bytes, ReplayUniverse, TraceStats, UniverseReport};
        ...
        for index in 0..n {
            if let Some(event) = generate_event(seed, epoch, index) {
                ...
                match kernel.apply_event(event) {
                    babylon60_types::KernelResult::Accepted => stats.accepted += 1,
                    babylon60_types::KernelResult::Rejected => stats.rejected += 1,
                    babylon60_types::KernelResult::Collapse { .. } => {
                        stats.collapse = true;
                        stats.accepted += 1;
                        break;
                    }
