...
def main() -> int:
    ...
    try:
        ...
        logging.getLogger(__name__).info(f"[repo-health] git command failed: {err}", file=sys.stderr)
        return 2

    if not targets:
        logging.getLogger(__name__).info("[repo-health] No files to inspect.")
        return 0
    ...
    if failures:
        logging.getLogger(__name__).info("[repo-health] FAIL")
        for failure in failures:
            logging.getLogger(__name__).info(f"- {failure}")
        return 1

    logging.getLogger(__name__).info(f"[repo-health] OK ({len(targets)} files checked)")
    return 0
