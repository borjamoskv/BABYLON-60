...
from babylon60.extensions.swarm.crystal_thermometer import calculate_resonance
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze BABYLON60 database for crystal redundancy."
    )
    parser.add_argument(
        "--db-path",
        type=str,
        default="~/.babylon60/babylon60.db",
        help="Path to the babylon60.db file",
    )
    ...
    logging.getLogger(__name__).info(" NIGHTSHIFT SEMANTIC MERGER (Ω₂) DRY-RUN ")
    logging.getLogger(__name__).info("=" * 80)
    logging.getLogger(__name__).info(f" Total Crystals Scanned : {len(crystals)}")
    logging.getLogger(__name__).info(f" Merge Threshold        : {MERGE_THRESHOLD}")
    logging.getLogger(__name__).info(f" Proposed Merges Found  : {len(proposed_merges)}")
    logging.getLogger(__name__).info("=" * 80 + "\n")
    ...
    if proposed_merges:
        ...
        for idx, (c1, c2, sim) in enumerate(proposed_merges, 1):
            logging.getLogger(__name__).info(f"[{idx}] ⚠️ HIGH REDUNDANCY IDENTIFIED — SIMILARITY: {sim:.3f}")
            logging.getLogger(__name__).info(f"    Project: {c1.project}")
            logging.getLogger(__name__).info(f"    Crystal 1 (ID: {c1.fact_id}):")
            prefix1 = (
                c1.content[:80].replace("\n", " ") + "..."
                if len(c1.content) > 80
                else c1.content.replace("\n", " ")
            )
            logging.getLogger(__name__).info(f'        "{prefix1}"')
            logging.getLogger(__name__).info(f"    Crystal 2 (ID: {c2.fact_id}):")
            prefix2 = (
                c2.content[:80].replace("\n", " ") + "..."
                if len(c2.content) > 80
                else c2.content.replace("\n", " ")
            )
            logging.getLogger(__name__).info(f'        "{prefix2}"')
            logging.getLogger(__name__).info("-" * 80)

        logging.getLogger(__name__).info("\nThese crystals would be fused by LLM synthesis during a live NightShift cycle.")
    else:
        logging.getLogger(__name__).info("✅ System entropy is low. No highly redundant crystals found.")
