...
def fix_file(filepath):
    ...
    if new_content != content:
        ...
        content = re.sub(
            r"(Union\[[^\]]+\])\s*\|\s*([\w\.]+(?:\[[^\|\]]+\])?)", r"Union[\1, \2]", content
        )
    ...
    if changed:
        ...
        if has_union:
            ...
            if "from typing import" in content:
                if "Union" not in re.search(r"from typing import (.*)", content).group(1):
                    content = re.sub(
                        r"from typing import (.*)", r"from typing import \1, Union", content
                    )
        ...
        with open(filepath, "w") as f:
            f.write(content)
        logging.getLogger(__name__).info(f"Fixed {filepath}")


def main():
    # Only target babylon60/extensions/llm/ as it seems to be the main source of new issues
    for root, _dirs, files in os.walk("babylon60/extensions/llm"):
        for file in files:
            if file.endswith(".py"):
