...
Executes all notebook cells as a script. Generates:
  1. babylon60_knowledge_graph.png  (visual)
...
# ── 1. Connect ────────────────────────────────────────────────────────
DB_PATH = Path.home() / ".babylon60" / "babylon60.db"
OUTPUT_DIR = Path("notebooklm_sources")
OUTPUT_DIR.mkdir(exist_ok=True)
MASTER_FILE = "babylon60_notebooklm_digest.md"

conn = connect(str(DB_PATH))
logging.getLogger(__name__).info(f"🔌 Conectado a BABYLON60: {DB_PATH}")
...
for name, q in tables.items():
    try:
        ...
        logging.getLogger(__name__).info(f"  {name:20s} → {n:,}")
    except sqlite3.Error:
        logging.getLogger(__name__).info(f"  {name:20s} → (tabla no existe)")
...
try:
    ...
    logging.getLogger(__name__).info(f"  Entidades: {len(df_entities)}, Relaciones: {len(df_relations)}")
except sqlite3.Error:
...
for p in projects:
    ...
    logging.getLogger(__name__).info(f"   • {p} ({cnt} facts)")
...
ax.set_title(
    "🕸️ BABYLON60 Knowledge Graph  (🔴 Proyectos · 🔵 Facts · 🟢 Agentes)", fontsize=14, pad=15
)
ax.axis("off")
plt.tight_layout()
plt.savefig("babylon60_knowledge_graph.png", dpi=150, bbox_inches="tight")
logging.getLogger(__name__).info("📸 Grafo guardado: babylon60_knowledge_graph.png")
...
if not df_agents.empty and "reputation_score" in df_agents.columns:
    ...
    plt.tight_layout()
    plt.savefig("babylon60.agents_reputation.png", dpi=150, bbox_inches="tight")
    logging.getLogger(__name__).info("📸 Reputación guardada: babylon60.agents_reputation.png")
...
if not df_hb.empty:
    ...
    plt.tight_layout()
    plt.savefig("babylon60_activity_timeline.png", dpi=150, bbox_inches="tight")
    logging.getLogger(__name__).info("📸 Timeline guardada: babylon60_activity_timeline.png")
...
doc = []
doc.append("# 🧠 BABYLON60 MASTER KNOWLEDGE DIGEST\n")
doc.append(f"> *Auto-generado: {ts}*\n\n")

doc.append("## INTRODUCCIÓN Y CONTEXTO DEL SISTEMA\n")
doc.append(
    "BABYLON60 v4.0 es un motor de memoria soberano con capacidades de "
...
logging.getLogger(__name__).info(f"   Archivo: {MASTER_FILE}")
logging.getLogger(__name__).info(f"   Tamaño:  {len(content):,} caracteres")
logging.getLogger(__name__).info("\n📋 Siguientes pasos:")
logging.getLogger(__name__).info("   1. Ve a https://notebooklm.google.com/")
logging.getLogger(__name__).info("   2. Crea un cuaderno 'BABYLON60 Brain'")
logging.getLogger(__name__).info(f"   3. Sube '{MASTER_FILE}' como fuente")
logging.getLogger(__name__).info("   4. ¡Dale a 'Generate Audio Overview'! 🎧")
