...
def check_git_state() -> CheckResult:
    ...
    if not passed:
        detail = (
            f"dirty working tree: {result.stdout.strip()}"
...
def check_vercel_ban() -> CheckResult:
            ...
            if lock == "package-lock.json":
                try:
                    ...
                    for pkg_name in packages:
                        if (
                            pkg_name.startswith("node_modules/@vercel/")
                            or pkg_name == "node_modules/vercel"
                        ):
    ...
    wrangler_present = (REPO_ROOT / "wrangler.toml").exists() or (
        REPO_ROOT / "wrangler.json"
    ).exists()
...
def main() -> None:
    ...
    for label, fn in checks:
        try:
            ...
            result = CheckResult(name=label.lower(), passed=False, detail="Internal server error")
    ...
    logging.getLogger(__name__).info(json.dumps(report.to_dict(), indent=2, ensure_ascii=False))
