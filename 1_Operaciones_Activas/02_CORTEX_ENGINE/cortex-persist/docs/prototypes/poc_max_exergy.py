...
async def stress_test_max_exergy():
    ...
    logging.getLogger(__name__).info(" 💥 PRUEBA DE ESTRÉS DE MÁXIMA EXERGÍA (CORTEX NATIVE) 💥")
    logging.getLogger(__name__).info("==========================================================\n")
    ...
    logging.getLogger(__name__).info(f"[*] Inyectando nodo raíz: {root_belief.id}")
    ...
    logging.getLogger(__name__).info(f"[*] Topología fractal generada: {total_beliefs} creencias activas.")

    # Check DB
    active_count = sum(1 for f in mock_db.facts if f.meta["belief_object"]["status"] == "active")
    logging.getLogger(__name__).info(f"[*] Creencias en Base de Datos: {len(mock_db.facts)} ({active_count} ACTIVAS)")

    logging.getLogger(__name__).info("\n[!] SIMULANDO COLAPSO DE CIMIENTO (BFT GUARD ACTIVADO)")
    logging.getLogger(__name__).info(
        f"[!] Auditor Premium Opus declara la raíz {root_belief.id} en cuarentena por contradicción grave."
    )
    ...
    logging.getLogger(__name__).info("\n==========================================================")
    logging.getLogger(__name__).info(f" ⏳ TIEMPO DE ORFANDAD TOPOLÓGICA: {(t1 - t0) * 1000:.2f} ms")
    logging.getLogger(__name__).info("==========================================================")
    ...
    logging.getLogger(__name__).info(f"[*] Creencias activas post-colapso:     {active_count}")
    logging.getLogger(__name__).info(f"[*] Creencias huérfanas (quarantined): {quarantined_count}")
