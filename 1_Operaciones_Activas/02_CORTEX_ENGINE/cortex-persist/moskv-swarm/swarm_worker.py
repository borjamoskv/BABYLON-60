...
class SwarmWorker:
    ...
    def process_file(self, path: Path) -> Optional[dict[str, Any]]:
        ...
        logging.getLogger(__name__).info(f"[WORKER] Job {job.get('job_id')} | chain: {agent_chain}")

        for agent_name in agent_chain:
            agent = self.resolve_agent(agent_name)
            artifact = agent.run(artifact, constraints)

            logging.getLogger(__name__).info(f"  -> {agent.name}: {artifact['status']}")
        ...
        logging.getLogger(__name__).info(f"[WORKER] Completed. Output: {out_path}")
        return feedback
    ...
    def run_once(self) -> list[dict[str, Any]]:
        ...
        for path in sorted(INBOX.glob("*.json")):
            try:
                ...
                logging.getLogger(__name__).info(f"[WORKER ERROR] {path.name}: {e}", file=sys.stderr)

        return results

    def run_daemon(self, interval: float = 5.0) -> None:
        logging.getLogger(__name__).info(f"[WORKER] Daemon started. Watching {INBOX}")
        while True:
            self.run_once()
            time.sleep(interval)
...
def main() -> None:
    ...
    if args.daemon:
        ...
        logging.getLogger(__name__).info(json.dumps(results, indent=2, ensure_ascii=False))
