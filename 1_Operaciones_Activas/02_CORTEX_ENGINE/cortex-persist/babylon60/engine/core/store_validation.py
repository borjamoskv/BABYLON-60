...
async def _enforce_ctre(meta: dict | None) -> None:
    ...
    if meta and meta.get("intent") == "UI_ACTION" and "expected_ui_hash" in meta:
        ...
        if not success:
            logger.error(
                "🛑 [CTRE] SAGA ABORT: UI TOCTOU Collision detected. Epsilon: %sµs", epsilon
            )
...
async def run_store_validation_logic(
    ...
    if not (
        meta and (meta.get("previous_fact_id") or meta.get("nonce") or meta.get("logos_signature"))
    ):
    ...
    await _enforce_cortex_taint(conn, content, fact_type, meta, tenant_id)
...
async def _enforce_cortex_taint(
    ...
    await enforce_taint_check(conn, token, content, tenant_id)
...
async def _apply_bridge_guard(conn, content, project, tenant_id, fact_type):
    ...
    if bridge_candidate:
        ...
        if "→" not in adaptation and "->" not in adaptation:
            bridge_eval_content = (
                f"Pattern from {source_proj} → {project}. Adaptation: {adaptation}"
            )

        bridge_result = await BridgeGuard.validate_bridge(
            conn, bridge_eval_content, project, tenant_id
        )
        if bridge_result:
