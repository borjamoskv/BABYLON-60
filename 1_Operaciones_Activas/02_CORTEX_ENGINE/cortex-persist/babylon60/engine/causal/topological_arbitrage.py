...
class TopologyIndex:
    ...
    async def sync(self) -> bool:
        ...
        if db_version != self.current_version:
            logger.info(
                "Graph version drift (%s -> %s). Rebuilding TopologyIndex.",
    ...
    async def invalidate_cascade(self, node_id: str, reason: str = "parent_false") -> None:
        ...
        try:
            ...
            logger.info("Invalidated %s descendants of ***id", len(target_list))
            ...
            logger.error("Failed to cascade invalidate: %s", e)
