...
class ReflexionEngine:
        ...
        *,
        ...
        for iteration in range(self.config.max_iterations):
            ...
            if elapsed_ms > self.config.timeout_ms:
                return self._handle_timeout_exhaustion(
                    elapsed_ms, iteration, reflections, current_tree
                )
            ...
            try:
                ...
                return self._handle_success(
                    result, loop_start, iteration, reflections, current_tree
                )

            except (ValueError, TypeError, KeyError, OSError, RuntimeError) as error:
                current_tree = await self._handle_failure(
                    error, exec_start, iteration, reflections, current_tree, on_reflection
                )
        ...
        return self._handle_exhaustion(loop_start, reflections, current_tree)

    def _handle_timeout_exhaustion(
        self,
        elapsed_ms: float,
        iteration: int,
        reflections: list[Reflection],
        current_tree: AgentOp,
    ) -> ReflexionOutcome:
        logger.warning(
    ...
    async def _handle_failure(
        ...
        reflections: list[Reflection],
        current_tree: AgentOp,
        on_reflection: Callable[[Reflection], Awaitable[None]] | None,
    ) -> AgentOp:
        ...
        if self.config.enable_tree_rewrite and iteration < self.config.max_iterations - 1:
            return self._apply_rewrite(current_tree, error, reflection, iteration)
        return current_tree
