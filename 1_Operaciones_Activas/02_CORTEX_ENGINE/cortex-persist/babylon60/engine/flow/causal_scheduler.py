...
class CausalScheduler:
    ...
    async def tick_and_act(
        ...
        if mode == "chaotic_irreversible":
            ...
            logger.warning(
                "[Causal Scheduler] COHERENCE FIELD < %s. Blocking mutations until causal reconciliation.",
                self.cf_threshold,
            )
            ...
            for c in candidates:
                logger.warning(
                    "[Causal Scheduler] Defensive Macro-Rewind for %s (permission_to_exist=%.3f)",
                    c["id"],
                    c["permission_to_exist_score"],
                )
