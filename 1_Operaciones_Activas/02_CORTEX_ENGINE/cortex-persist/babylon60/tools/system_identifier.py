...
class MahalanobisDistanceCalculator:
    def __init__(self, calibration_data: np.ndarray):
        ...
        try:
            ...
            self.inv_cov = np.eye(cov_reg.shape[0])
...
class SystemIdentifier:
    ...
    def extract_state(
        self, turn_idx: int, response: str, prev_response: str | None, itl: float
    ) -> BehavioralStateVector:
        from babylon60.extensions.security.utils import calculate_lexical_entropy

        entropy = calculate_lexical_entropy(response)
    ...
    def compute_trajectory_dtw(
        ...
        mahalanobis: MahalanobisDistanceCalculator | None = None,
