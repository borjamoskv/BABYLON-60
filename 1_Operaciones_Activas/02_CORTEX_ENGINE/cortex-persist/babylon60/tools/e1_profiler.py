...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args:
        logging.getLogger(__name__).info("Usage: python -m cortex.tools.e1_profiler <traces.jsonl> [key=value ...]")
        sys.exit(1)
    ...
    if not traces:
        logging.getLogger(__name__).info(f"No traces loaded from {path}")
        sys.exit(1)
