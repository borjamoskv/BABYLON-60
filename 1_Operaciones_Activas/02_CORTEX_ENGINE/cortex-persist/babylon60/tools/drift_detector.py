...
class BehavioralSnapshot:
    ...
    def __post_init__(self):
        if not self.sha256_hash and self.state_vectors is not None:
            ...
            self.sha256_hash = bytes_to_base60(h.digest())
...
class DriftDetector:
    def __init__(self, regularization_eps: float = 1e-5, eps: float | None = None):
        self.regularization_eps = eps if eps is not None else regularization_eps

    def capture_snapshot(
        self, model_id: str, states: list[np.ndarray], metadata: dict[str, Any] | None = None
    ) -> BehavioralSnapshot:
        if len(states) == 0:
