...
class SingleResult:
    ...
    latency_ms: float
    ttft_ms: float | None  # None if not measurable
    completion_tokens: int | None  # None if provider doesn't return
    tokens_per_sec: float | None  # None if completion_tokens None

    json_valid: bool
    exact_match: bool | None  # None if no ground truth provided
    response_text: str
...
class OpenAIChatCompletionsAdapter:
    ...
    def call_stream(
    ...
    ) -> tuple[int, str, float | None, float, dict[str, Any]]:
        """
        Returns:
        ...
        for line in r.iter_lines(decode_unicode=True):
            ...
            try:
                ...
                logger.warning("Error parsing SSE stream: %s", e)
                continue
...
class ProvenanceAuditor:
    ...
    def _extract_features(self, results: list[SingleResult]) -> dict[str, float] | None:
    ...
    def analyze(self, results: list[SingleResult]) -> dict[str, Any]:
        ...
        nearest = min(distances.keys(), key=lambda k: distances[k]) if distances else "unknown"
...
class BlackBoxHarness:
    ...
    def _extract_text_usage(self, data: dict[str, Any]) -> tuple[str, int | None]:
        try:
            text = data["choices"][0]["message"]["content"]
        except Exception as e:  # noqa: BLE001
            logger.warning("Error extracting text usage: %s", e)
            text = ""
        completion_tokens = None
        try:
            completion_tokens = int(data.get("usage", {}).get("completion_tokens"))
        except Exception as e:  # noqa: BLE001
            logger.warning("Error extracting completion tokens: %s", e)
            completion_tokens = None
        return text, completion_tokens

    def run_prompt(self, prompt: str, ground_truth: str | None, force_stream: bool) -> SingleResult:
        self.total_calls += 1
