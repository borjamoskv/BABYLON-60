...
class SwarmSupervisor:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.warning("Could not load LocalEmbedder: %s", e)
        ...
        logger.info("SwarmSupervisor initialized [Lease: ***id")
    ...
    async def _heartbeat_monitor(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.critical(
                    "HEARTBEAT FAILURE: Consumer loop crashed: %s. Initiating Controlled Apocalypse.",
                    e,
                )
                self._running = False
                sys.exit(1)
    ...
    async def _state_consumer_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.error("State consumer encountered error: %s", e)
                raise
        ...
        for _ in range(count):
            ...
            try:
                ...
                try:
                    ...
                    if self._embedder:
                        ...
                        try:
                            ...
                            logger.error("Semantic cache error: %s", e)

                    if cached_payload:
                        logger.info(
                            "⚡ [Semantic Cache Hit] Bypassing LLM inference for task %s",
                            task["id"],
                        )
                    ...
                    logger.warning(
                        "[EpistemicBreaker] Task %s rejected (Fable-Verify Failure): %s",
                        task["id"],
                        epi_err,
                    )
                ...
                try:
                    ...
                    logger.error(
                        "Dispatch failed for task %s. Rolling back lease. Error: %s", task["id"], e
                    )
                    with causal_write(self._db):
                        await self._db.execute(
                            "UPDATE system_hypotheses SET status = 'PENDING', owner_id = NULL, lease_expires_at = NULL WHERE id = ?",
                ...
                logger.error("Critical dispatch loop failure for task %s: %s", task["id"], e)

        if dispatched > 0:
            logger.info("Dispatched %s optimal hypotheses with Fable 5 Steerability.", dispatched)
