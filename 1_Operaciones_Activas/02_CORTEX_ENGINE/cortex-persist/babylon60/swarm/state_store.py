...
class CausalStateStore:
    ...
    async def process_signal(self, signal: Any) -> None:
        ...
        async with self._lock:
            ...
            if signal.target.startswith("hyp-"):
                ...
                if not is_active:
                    logger.warning(
                        "Dropping signal for %s: Hypothesis is INVALIDATED.", signal.target
                    )
                    return
            ...
            try:
                ...
                await enforce_taint_check(
                    ...
                    getattr(signal, "tenant_id", "default"),
                )
                    ...
                    if signal.status in ("SUCCESS", "FAILURE"):
                        ...
                        if signal.target.startswith("hyp-") and signal.status == "SUCCESS":
                            ...
                            try:
                                ...
                                logger.warning(
                                    "[EpistemicBreaker] Failed to compile RawEvidence from %s: %s",
                                    signal.target,
                                    epi_e,
                                )
                ...
                logger.error("[SAGA ROLLBACK] State mutation failed: %s", e)
        ...
        async with self._lock:
            ...
            for signal in signals:
                ...
                try:
                    ...
                    logger.error(
                        "[SAGA REJECT] Signal validation failed for %s: %s", signal.target, e
                    )
                    continue
            ...
            try:
                ...
                logger.error("[SAGA ROLLBACK] Batch AOL mutation failed: %s", e)
    ...
    async def recover_in_flight_tasks(self, lease_id: str | None = None) -> int:
        ...
        async with self._lock:
            ...
            if count > 0:
                logger.info("Recovering %s IN_FLIGHT tasks to ACTIVE status.", count)
    ...
    async def sweep_expired_leases(self) -> int:
        ...
        async with self._lock:
            ...
            with causal_write(self._db):
                ...
                if count > 0:
                    logger.warning(
                        "[SANEDRIN] Swept %s expired lease locks. Ghost tasks recovered.", count
                    )
