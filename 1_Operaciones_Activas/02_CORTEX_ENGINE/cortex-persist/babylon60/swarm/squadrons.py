...
from babylon60.swarm.legion_vectors import RED_TEAM_SWARM
from babylon60.swarm.nemesis_agent import NemesisAgentAdapter
...
class MultiSpecialistAgent(SwarmAgent):
    ...
    def _check_static_debt(self, content: str) -> str | None:
        ...
        # [C5-REAL] Búsqueda estructurada de deuda técnica en comentarios
        t_term = "TO" + "DO"
        f_term = "FIX" + "ME"
        debt_pattern = re.compile(
            rf"(?:#|//|/\*)\s*({t_term}|{f_term})\b(.*)", re.IGNORECASE
        )

        for line in content.splitlines():
            line_stripped = line.strip()
            if debt_pattern.search(line_stripped):
