...
from babylon60.auth.deps import require_permission
from babylon60.auth.manager import AuthResult
...
async def _stream_labyrinth_proxy(request: Request, payload: dict) -> AsyncGenerator[str, None]:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            yield f"data: {json.dumps({'error': 'Labyrinth proxy failed', 'details': 'Internal Server Error'})}\n\n"
...
async def proxy_chat_completions(
    ...
    if is_stream:
        return StreamingResponse(
            # codeql[py/stack-trace-exposure] - Data is safe and does not expose secrets
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code != 200:
            logger.error("Labyrinth upstream error: %s", resp.text)
