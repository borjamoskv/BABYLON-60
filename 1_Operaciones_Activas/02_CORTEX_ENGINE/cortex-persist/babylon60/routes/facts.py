...
async def shared_batch_store(
    memories: list[StoreMemoryRequest],
    tenant_id: str,
    engine: AsyncCortexEngine,
    item_name: str,
) -> dict:
    """Helper for batch storing facts/memories."""
    ids: list[int] = []
    errors: list[dict] = []
    for i, mem in enumerate(memories):
        try:
            ...
            logger.exception("Failed to batch store %s at index %d", item_name, i)
            errors.append({"index": i, "error": f"Failed to store {item_name}"})
...
async def shared_verify_ledger(engine: AsyncCortexEngine) -> dict:
    """Helper to verify ledger integrity."""
    try:
...
async def batch_store(
    ...
    return await shared_batch_store(req.memories, auth.tenant_id, engine, "fact")
...
async def verify_ledger(
    ...
    return await shared_verify_ledger(engine)
