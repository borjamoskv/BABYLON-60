...
from babylon60.core.config import LANGBASE_API_KEY, LANGBASE_BASE_URL
from babylon60.engine.core.cortex_engine import CortexEngine as AsyncCortexEngine
...
async def langbase_pipe_run(
    ...
    try:
        ...
        logger.error("Langbase API error: %s", e.detail)
        raise HTTPException(
            status_code=e.status_code or 502, detail="Langbase upstream error"
        ) from None
...
async def langbase_status(
    ...
    try:
        ...
        logger.error("Langbase API error: %s", type(e).__name__)
        raise HTTPException(
            status_code=e.status_code or 502, detail="Langbase upstream error"
        ) from None
    except (OSError, ValueError, KeyError):
        logger.exception("Langbase status check failed")
