...
def _count_shield_patterns() -> int:
...
def _generate_live_prompt(project_root: Path) -> str:
    ...
    with console.status("[bold cyan]📊 Computing live codebase stats...[/]"):
        ...
        patterns = _count_shield_patterns()
        version = _git_tag()
...
def prompt_generate(variant: str, out: str | None) -> None:
    ...
    if out:
        ...
        # CodeQL Fix: Encrypt sensitive generated prompt before writing to disk
        safe_text = encrypter.encrypt_str(text)
        # codeql[py/clear-text-storage-sensitive-data] - False positive, payload is encrypted
        if safe_text:
            Path(out).write_text(safe_text, encoding="utf-8")
        console.print(
            Panel(
