...
@click.option(
    ...
    help="Escape export output directory",
)
@click.option(
    "--threshold-days", type=int, default=30, help="Dead man switch inactivity threshold in days"
)
@click.option("--force", is_flag=True, help="Force export immediately even if switch not triggered")
def escape_hatch_cmd(
    db: str, check: bool, touch: bool, export_dir: Path | None, threshold_days: int, force: bool
):
    ...
    async def _run():
        ...
        async with connect_async_ctx(db) as conn:
            if touch:
                await record_liveness(conn)
                console.print(
                    "[bold green]Escape Hatch: Liveness logged successfully.[/bold green]"
                )
                return
            ...
            if check:
                ...
                if triggered:
                    console.print(
                        "[bold red]Dead Man Switch: TRIGGERED! System inoperable threshold exceeded.[/bold red]"
                    )
                else:
                    console.print(
                        "[bold green]Dead Man Switch: ACTIVE. System is live and healthy.[/bold green]"
                    )
                return
            ...
            if triggered or force:
                if not export_dir:
                    console.print(
                        "[bold red]Error: --export-dir is required to execute the escape hatch export.[/bold red]"
                    )
                    return
                console.print(
                    f"[bold yellow]Triggering Autonomic Flat Export to {export_dir}...[/bold yellow]"
                )
                res = await trigger_escape_hatch_export(conn, export_dir)
                console.print(
                    f"[bold green]Export Complete![/bold green] Manifest written to: {res['manifest_path']}"
                )
            else:
                console.print(
                    "[bold green]Dead Man Switch not triggered (system is live). Export skipped. Use --force to override.[/bold green]"
                )
