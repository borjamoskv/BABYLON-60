...
def _cli_taint_scope(content: str, meta: dict, agent_source: str):
    """Ouroboros Auto-Healing: Inject CORTEX-TAINT for a single CLI write.

    Loads Ed25519 key from keyring or env to generate a valid cryptographic
    signature. If no key is provisioned (or token generation fails), taint
    enforcement is bypassed ONLY for the write executed inside this context —
    never process-wide (FIND-001). Enforcement state is restored on exit.
    """
    import os
    ...
    # Scoped bypass: same save/restore pattern as init_cmds, never a permanent flip.
    prev = os.environ.get("CORTEX_NO_TAINT_ENFORCE")
    ...
    try:
        ...
        if prev is None:
            ...
            os.environ["CORTEX_NO_TAINT_ENFORCE"] = prev
...
def store(
    ...
    try:
        ...
        # Ouroboros Auto-Healing: CORTEX-TAINT scoped to this write only (FIND-001)
        with _cli_taint_scope(content, meta, source):
            fact_id = _run_async(
                engine.store(
                    project=project,
                    content=content,
...
def store_batch(file_path, db) -> None:
    ...
    async def _run_batch() -> int:
        ...
        for idx, fact in enumerate(facts):
            ...
            meta = meta or {}
            logging.getLogger(__name__).info(f"DEBUG: Storing fact: {fact_type=} from file {file_path}")
            with _cli_taint_scope(content, meta, source):
