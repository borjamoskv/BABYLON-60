...
def extract_features(response: str, latency_ms: float = 0.0) -> ResponseFeatures:
    ...
    entropy = calculate_distribution_entropy(counts, word_count)
...
class BehavioralCoverage:
    ...
    def compute(feature_matrix: list[list[float]]) -> CoverageReport:
        ...
        # H_cov = -Σ p_d log₂ p_d
        from babylon60.extensions.security.utils import calculate_entropy_from_probabilities

        h_cov = calculate_entropy_from_probabilities(p)
