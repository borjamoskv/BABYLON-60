...
def base60_to_bytes(s: str, expected_len: int) -> bytes:
    ...
    try:
        ...
        raise ValueError(
            f"Decoded Base-60 value exceeds capacity of {expected_len} bytes."
        ) from err
...
def base60_check_to_bytes(s: str, expected_data_len: int) -> bytes:
    ...
    if len(s) != expected_chars:
        raise ValueError(
            f"Base60Check failed: expected {expected_chars} chars for a "
            f"{expected_data_len}-byte payload, got {len(s)}."
        )
    ...
    if _checksum(data) != chk:
        raise ValueError(
            f"Base60Check failed: Checksum mismatch for data length {expected_data_len}."
        )
