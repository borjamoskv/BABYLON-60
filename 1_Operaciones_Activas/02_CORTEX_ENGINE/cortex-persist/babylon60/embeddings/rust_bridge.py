...
try:
    ...
    if not os.environ.get("CORTEX_TESTING"):
        ...
        logging.critical(
            "[INV_ZERO_EMULATION] onnxruntime missing. Pure Python emulation forbidden. CRASHING."
        )
        sys.exit(1)
    else:
        ort = None
...
class RustNativeEmbeddings:
    ...
    def __init__(self, model_path: str, tokenizer_path: str):
        ...
        if not os.path.exists(model_path) and not os.environ.get("CORTEX_TESTING"):
            logger.error("[RustBridge] ONNX model missing at %s", model_path)
        ...
        logger.info("[RustBridge] Initialized ONNX Native Backend for %s", model_path)
