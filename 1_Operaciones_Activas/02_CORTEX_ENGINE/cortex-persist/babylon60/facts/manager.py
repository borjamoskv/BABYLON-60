        ...
        **kwargs,
        ...
        try:
            ...
            cursor = await conn.execute(
                "SELECT id FROM facts WHERE hash = ? AND project = ? AND tenant_id = ?",
                (f_hash, project, tenant_id),
            )
            row = await cursor.fetchone()
            if row and not (meta.get("nonce") or meta.get("logos_signature")):
                fact_id = row[0]
                ...
                if hasattr(self.engine.embeddings, "embed_text"):
                    ...
                    if vec:
                        ...
                        if (
                            ...
                            and not (meta.get("nonce") or meta.get("logos_signature"))
        ...
        try:
            fact_id = await StoreMixin._store_impl(
            ...
            try:
                from babylon60.routes.notch_ws import notify_notch_mutation

                await notify_notch_mutation()
            except (ImportError, AttributeError, ValueError, RuntimeError):
                pass
            return fact_id
        except Exception as e:  # noqa: BLE001
            try:
                from babylon60.routes.notch_ws import notify_notch_error

                await notify_notch_error()
            except (ImportError, AttributeError, ValueError, RuntimeError):
                pass
            ...
            log.exception(
                "EXCEPTION IN _store_delegate: %s - started_tx=%s, in_transaction=%s",
                type(e).__name__,
                started_tx,
                conn.in_transaction,
                exc_info=e,
