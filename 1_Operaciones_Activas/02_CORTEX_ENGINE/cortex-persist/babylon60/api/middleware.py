...
class SecurityFraudMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, log_path: str = "~/.babylon60/firewall.log"):
        super().__init__(app)
...
class SovereignIsolationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # 0. Allow OPTIONS requests to pass through to CORS middleware
...
class CortexBillingMiddleware(BaseHTTPMiddleware):
    ...
    async def _report_usage(self, api_key: str, request: Request):
        ...
        try:
            ...
            logger.info("Stripe usage reported for tenant %s", result.tenant_id)
        except Exception as e:  # noqa: BLE001
            api_key[:4] + "***" if api_key else "***"
            logger.error("Stripe billing increment failed: %s", type(e).__name__)
