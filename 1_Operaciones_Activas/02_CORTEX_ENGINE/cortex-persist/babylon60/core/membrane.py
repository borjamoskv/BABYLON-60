...
try:
    ...
    Z3_AVAILABLE = False
    logger.warning("Warning: Z3 not available. Logical guards will be disabled.")
            ...
            for g in guards:
                if g in self.presets:
                    try:
                        ...
                        logger.error("Error compilando guard %s: %s", g, e)
    ...
    def check(
        ...
        if guards and self.z3_guard and self.z3_guard.enabled:
            ...
            for g in guards:
                ...
                check_res = self.z3_guard.check(value=value)
