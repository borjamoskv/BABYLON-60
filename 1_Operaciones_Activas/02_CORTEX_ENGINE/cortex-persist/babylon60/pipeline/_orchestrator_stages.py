    ...
    def _persist(self, request: PipelineRequest, output: Any) -> str:
        ...
        if self.engine is not None:  # pyright: ignore[reportAttributeAccessIssue]
            ...
            try:
                ...
                try:
                    ...
                    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                        pool.submit(asyncio.run, persist_to_engine()).result(timeout=10)
                except RuntimeError:
                    asyncio.run(persist_to_engine())
            except Exception as e:  # noqa: BLE001
                logger.error("  [PERSIST] Engine SQLite write failed: %s", e)
                raise RuntimeError(f"[C5-REAL] Fatal persistence failure to Engine SQLite: {e}") from e
            ...
            try:
                ...
                logger.error("  [PERSIST] Ledger write failed: %s", e)
                raise RuntimeError(f"[C5-REAL] Fatal persistence failure to audit ledger: {e}") from e
