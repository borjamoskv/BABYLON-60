...
def build_causal_graph():
    ...
    if not os.path.exists(CRONOS_LOG):
        logging.getLogger(__name__).info(f"No CRONOS log found at {CRONOS_LOG}")
        return
    ...
    logging.getLogger(__name__).info(f"Causal graph generated: {len(nodes)} nodes, {len(edges_list)} edges.")
