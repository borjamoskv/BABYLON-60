...
class CausalRollbackEngine:
    ...
    def apply_selective_reexecution(self, target_node: str) -> dict[str, Any]:
        ...
        """
        logger.info("🌀 [ROLLBACK] Initiating selective causal reversal for node: %s", target_node)
        subgraph = self.compute_subgraph(target_node)
        if not subgraph:
            logger.info(
                "🌀 [ROLLBACK] Node %s has no downstream causality. Reversal cost is zero.",
                target_node,
            )
            return {
                "target": target_node,
        ...
        logger.warning(
            "⚠️ [ROLLBACK] Reversal Cost: %s exergy units across %s affected nodes.",
            cost,
            len(affected_nodes),
        )
        ...
        if cost > 500.0:
            logger.error(
                "🛑 [ROLLBACK] ABORTED. Exergy cost (%s) exceeds structural safety limit (500.0).",
                cost,
            )
            return {
                "target": target_node,
        ...
        logger.info(
            "✅ [ROLLBACK] Causal timeline successfully pruned starting from %s.", target_node
        )
