...
def run_cli(
    ...
    logging.getLogger(__name__).info(f"   Model: {agent.model}{toolbox_status}")
    logging.getLogger(__name__).info("   Type 'quit' to exit\n")
        ...
        for event in runner.run(
            ...
            if event.is_final_response():
                if event.content:
                    ...
                    for part in parts:
                        if part.text:
                            logging.getLogger(__name__).info(part.text)
        logging.getLogger(__name__).info()
