    ...
    if model_name not in _MODEL_CACHE:
        if HAS_SENTENCE_TRANSFORMERS:
            try:
                ...
                logger.info(
                    "[PROMPT_SECURITY] Loaded SentenceTransformer model '%s'. Threading optimized.",
                    model_name,
                )
            except Exception as e:  # noqa: BLE001
                logger.warning(
                    "[PROMPT_SECURITY] Failed to load SentenceTransformer: %s. Falling back to syntactic overlap.",
                    e,
                )
...
class PromptSecurityGuard:
    ...
    def __init__(self, system_prompt: str, threshold: float = 0.55, window_size: int = 5) -> None:
        ...
        if self.model is not None:
            try:
                ...
                logger.warning(
                    "[PROMPT_SECURITY] Failed to encode system prompt: %s. Disabling model.", e
                )
                self.model = None
    ...
    def _calculate_semantic_similarity(self, text: str) -> float:
        ...
        if (
            ...
            try:
                ...
                logger.error("[PROMPT_SECURITY] Error calculating cosine similarity: %s", e)
    ...
    def verify_input(self, user_query: str, history: list[dict[str, Any]]) -> None:
        ...
        if obfuscated_decoded:
            logger.info(
                "[PROMPT_SECURITY] Decoded potential obfuscated text: '%s'", obfuscated_decoded
            )
            normalized_query += " " + clean_text(obfuscated_decoded)

        if self.fast_path_regex.search(normalized_query):
            logger.warning(
                "[PROMPT_SECURITY] Blocked input due to fast-path match: '%s'", user_query[:100]
            )
    ...
    def verify_output(self, response_text: str) -> None:
        ...
        for sent in self.system_sentences:
            if sent in cleaned_response:
                logger.error("[PROMPT_SECURITY] Verbatim system prompt sentence leaked: '%s'", sent)
        ...
        if rolling_avg > self.threshold:
            logger.error(
                "[PROMPT_SECURITY] Leakage threshold breached. Score: %.4f > %.4f",
                rolling_avg,
                self.threshold,
            )
            self.history_scores.clear()
