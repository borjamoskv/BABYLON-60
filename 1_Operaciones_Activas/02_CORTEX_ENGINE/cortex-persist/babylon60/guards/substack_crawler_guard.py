...
class SubstackCrawlerGuard:
    ...
    def evaluate_subscriber_exergy(
        ...
        safe_email = f"REDACTED_{email_hash}"

        # 2. Hardcoded Firewall Thresholds
        if local_part in CRAWLER_BOT_PREFIXES:
            ...
            if opens > 10:
                ...
                raise ValueError(
                    ...
                    f"Email '{safe_email}' is a Corporate Dead Inbox exhibiting Crawler Inflation. "
        ...
        if ts_delta_ms is not None:
            if opens > 0 and ts_delta_ms < 1500.0:
                logger.error(  # codeql[py/clear-text-logging-sensitive-data]
                    "Non-human reaction time [%s ms] detected for %s", ts_delta_ms, safe_email
                )
                raise ValueError(
                    f"[Axiom OUROBOROS-014] Causal Impossibility: Interaction delta of "
                    f"{ts_delta_ms}ms is sub-human. Firewall scanner confirmed for {safe_email}."
                )
        ...
        if opens > 50 and clicks > 0 and abs(opens - clicks) <= 5:
            logger.error(  # codeql[py/clear-text-logging-sensitive-data]
                "Symmetrical engagement ratio anomaly detected for %s", safe_email
            )
            raise ValueError(
                f"[Axiom OUROBOROS-094] Thermodynamic Violation: 1:1 Open/Click ratio anomaly. "
                f"Identity '{safe_email}' is operating a Link Scanner."
            )
