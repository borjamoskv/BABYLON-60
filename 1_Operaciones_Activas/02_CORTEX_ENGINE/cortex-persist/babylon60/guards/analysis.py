...
def _scan_node_sequence(sequence: Iterable[ast.AST]) -> list[str]:
    """Scan a sequence of AST nodes for string constants containing oracle references."""
    found: list[str] = []
    for item in sequence:
        if isinstance(item, ast.Constant) and isinstance(item.value, str):
            hit = oracle_in_str(item.value)
            if hit:
                found.append(hit)
    return found
...
def scan_fstring(node: ast.JoinedStr) -> list[str]:
    ...
    return _scan_node_sequence(node.values)
...
def scan_exec_args(node: ast.Call) -> list[str]:
    ...
    return _scan_node_sequence(node.args)
