...
logger = logging.getLogger("babylon60.guards.uptimebolt_guard")
...
def enforce_deploy_safety(payload_str: str) -> None:
    ...
    if not api_key:
        logger.debug(
            "[UptimeBoltGuard] UPTIMEBOLT_API_KEY not set. Bypassing deploy safety check (Fail Open)."
        )
        return
    ...
    logger.info("🛡️ [UptimeBoltGuard] Verifying deployment safety for service: %s", service_name)

    mcp_payload = {
        "jsonrpc": "2.0",
        "id": 1,
        "method": "tools/call",
        "params": {"name": "is_safe_to_deploy", "arguments": {"service_name": service_name}},
    }
    ...
    try:
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            if result_obj.get("isError"):
                logger.error("[UptimeBoltGuard] MCP returned error: %s", result_obj)
            ...
            if recommendation not in ("proceed", "proceed_with_caution"):
                ...
                err_msg = (
                    f"SAGA-1 Rejection by UptimeBolt: Risk is {risk.upper()} ({recommendation})"
                )
                logger.error("❌ [UptimeBoltGuard] %s", err_msg)
                raise ValueError(err_msg)

            logger.info(
                "✅ [UptimeBoltGuard] Deployment safe to proceed. Recommendation: %s",
                recommendation,
            )

    except urllib.error.URLError as e:
        logger.warning(
            "⚠️ [UptimeBoltGuard] Connection to UptimeBolt MCP failed: %s. Bypassing (Fail Open).", e
        )
