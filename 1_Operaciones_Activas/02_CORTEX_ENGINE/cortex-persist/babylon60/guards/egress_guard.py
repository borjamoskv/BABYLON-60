...
_EXFIL_PATTERNS = [
    ...
    re.compile(
        r"(/Users/[a-zA-Z0-9_.-]+/(?:10_PROJECTS|20_VAULT|30_BABYLON-60|BOCETOS))"
    ),  # Local paths
]
...
class EgressGuard:
    ...
    def authorize_llm_dispatch(
        self, agent_id: str, provider: str, payload_text: str
    ) -> EgressAuthorization:
        """
        Validates LLM API dispatch to prevent context leakage (distillation hunting).
        Especially targeted at external providers like Anthropic.
        """
        if provider.lower() in ["anthropic", "openai"]:
            for pattern in _EXFIL_PATTERNS:
                if pattern.search(payload_text):
                    logger.error(
                        "[P0] EgressGuard: Context leakage pattern detected in payload to %s from agent %s",
                        provider,
                        agent_id,
                    )
                    return EgressAuthorization(
                        False, f"C5-REAL: Prevented telemetry/path leakage to {provider}."
                    )
        return EgressAuthorization(True, "OK")
