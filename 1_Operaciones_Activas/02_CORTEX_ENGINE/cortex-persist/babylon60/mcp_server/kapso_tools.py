...
def register_kapso_tools(mcp: Any, ctx: Any) -> None:
    ...
    async def send_whatsapp_message(to: str, text: str) -> dict:
        ...
        try:
            ...
            logger.debug("Executing kapso command: %s", " ".join(cmd))
            ...
            logger.error("Kapso CLI error: %s", e.stderr)
            return {"status": "error", "message": e.stderr}
        except Exception as e:  # noqa: BLE001
            logger.error("Unexpected error executing Kapso: %s", e)
    ...
    async def list_whatsapp_messages() -> dict:
        ...
        try:
            ...
            logger.error("Kapso CLI list error: %s", e.stderr)
            return {"status": "error", "message": e.stderr}
        except Exception as e:  # noqa: BLE001
            return {"status": "error", "message": str(e)}
