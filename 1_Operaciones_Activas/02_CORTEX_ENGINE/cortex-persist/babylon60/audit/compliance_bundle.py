...
class ComplianceBundler:
    ...
    def export_bundle(self, output_zip_path: str) -> bool:
        ...
        if not self.db_path.exists():
            logger.error("[ComplianceBundler] Database not found at %s", self.db_path)
            return False
        ...
        try:
            ...
            logger.info(
                "[ComplianceBundler] Successfully exported %s records to %s",
                len(export_data),
                output_zip_path,
            )
            return True
            ...
            logger.error("[ComplianceBundler] Failed to export bundle: %s", e)
            return False
