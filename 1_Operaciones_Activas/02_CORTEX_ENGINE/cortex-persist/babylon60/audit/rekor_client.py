    ...
    async def log_entry(
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                logger.error(
                    "[Rekor] Failed to log entry. Status: %s, Body: %s",
            ...
            logger.error("[Rekor] Exception during log_entry: %s", e)
            return None
    ...
    async def verify_entry(self, rekor_uuid: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    "[Rekor] Failed to fetch entry %s. Status: %s", rekor_uuid, response.status_code
                )
                return None
        except Exception as e:  # noqa: BLE001
            logger.error("[Rekor] Exception fetching entry: %s", e)
            return None
