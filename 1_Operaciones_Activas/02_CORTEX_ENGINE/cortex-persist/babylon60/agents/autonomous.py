    ...
    async def execute_objective(
        ...
        while not plan.is_complete:
            ...
            if plan.total_entropy_paid > max_entropy:
                ...
                # ── Thermal Apoptosis (Shannon Entropy Circuit Breaker) ──
                # If entropy budget is exceeded, defensively kill all active subagents
                # to prevent further anergy/token dissipation.
                logger.warning("[%s] Triggering Thermal Apoptosis (kill_all) due to entropy breach", self.agent_id)
                try:
                    # Best-effort invocation. Will fail safely if tool isn't registered.
                    await self.use_tool("manage_subagents", Action="kill_all")
                except Exception as exc:  # noqa: BLE001
                    logger.debug("[%s] Thermal Apoptosis skip/fail: %s", self.agent_id, exc)
