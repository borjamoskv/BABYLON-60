...
class ExergyGradient:
    ...
    degradation_threshold: Decimal = field(default_factory=lambda: Decimal("-0.15"))
    patience_steps: int = 3

    def record(self, exergy: Decimal, entropy: Decimal) -> None:
        self.history.append(
            {
    ...
    @property
    def net_derivative(self) -> Decimal:
        """Calculate ∂E/∂t of the latest steps with temporal regularization."""
        if len(self.history) < 2:
            return Decimal("0.0")
        latest = self.history[-1]
        prev = self.history[-2]
        dt = Decimal(str(max(latest["timestamp"] - prev["timestamp"], 1.0)))
        return Decimal(str(latest["net"] - prev["net"])) / dt
    ...
    def is_degrading(self) -> bool:
        ...
        for i in range(-1, -self.patience_steps, -1):
            ...
            dt = Decimal(str(max(curr["timestamp"] - prev["timestamp"], 1.0)))
            diff = Decimal(str(curr["net"] - prev["net"]))
            deriv = diff / dt
...
class BoltzmannEngineAgent(BaseAgent):
            ...
            [
                {
                    ...
                    "arguments": {
                        "plan_summary": {
                            "net_exergy": Decimal("0.5"),
                            "entropy_paid": Decimal("0.05"),
                            "progress": "0%",
                        }
                    },
                    "description": "Initial environment exergy scan",
                    "exergy_estimate": Decimal("0.5"),
    ...
    async def _phase_orient(self) -> None:
        ...
        if self.current_plan:
            self.gradient.record(
                Decimal(str(self.current_plan.total_exergy_produced)),
                Decimal(str(self.current_plan.total_entropy_paid)),
            )
    ...
    async def _phase_replan(self) -> None:
        ...
        observations = {
            ...
            "entropy": Decimal(str(self.current_plan.total_entropy_paid)),
        }
        ...
        replacement = [
            {
                ...
                "exergy_estimate": Decimal("0.4"),
                "entropy_cost": Decimal("0.1"),
            }
        ]
                ...
                "steps": [
                    {
                        ...
                        "exergy_estimate": str(step.exergy_estimate),
                        "entropy_cost": str(step.entropy_cost),
                    }
                ],
            },
