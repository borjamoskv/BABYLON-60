...
async def _upsert_entity(
    ...
    blind_name = get_blind_name(ent["name"], tenant_id)
    meta = encrypt_node_meta(ent["name"], tenant_id)
    ...
    cursor = await conn.execute(
        ...
        "(name, entity_type, project, tenant_id, first_seen, last_seen, mention_count, meta) "
        "VALUES (?, ?, ?, ?, ?, ?, 1, ?)",
        (blind_name, ent["entity_type"], project, tenant_id, timestamp, timestamp, meta),
    )
