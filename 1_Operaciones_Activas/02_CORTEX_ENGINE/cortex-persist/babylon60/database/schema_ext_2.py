...
CREATE TABLE IF NOT EXISTS entity_events (
    ...
    prev_hash       TEXT NOT NULL DEFAULT '00000000000000000000000000000000000000000000',
...
CREATE TRIGGER IF NOT EXISTS exec_trace_ledger_no_update
BEFORE UPDATE ON execution_trace_ledger
BEGIN
    SELECT RAISE(ABORT, 'ledger is append-only: UPDATE forbidden');
END;

CREATE TRIGGER IF NOT EXISTS exec_trace_ledger_no_delete
BEFORE DELETE ON execution_trace_ledger
BEGIN
...
CREATE TRIGGER IF NOT EXISTS prevent_exec_trace_update BEFORE UPDATE ON execution_trace_ledger
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: execution_trace_ledger is append-only'); END;

CREATE TRIGGER IF NOT EXISTS prevent_exec_trace_delete BEFORE DELETE ON execution_trace_ledger
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: execution_trace_ledger is append-only'); END;
"""
...
CREATE TRIGGER IF NOT EXISTS prevent_merkle_roots_update BEFORE UPDATE ON merkle_roots
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: merkle_roots is append-only'); END;

CREATE TRIGGER IF NOT EXISTS prevent_merkle_roots_delete BEFORE DELETE ON merkle_roots
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: merkle_roots is append-only'); END;
"""
...
CREATE TRIGGER IF NOT EXISTS prevent_audit_exports_update BEFORE UPDATE ON audit_exports
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: audit_exports is append-only'); END;

CREATE TRIGGER IF NOT EXISTS prevent_audit_exports_delete BEFORE DELETE ON audit_exports
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: audit_exports is append-only'); END;
"""
