...
def _sha256_file(path: Path, force_hex: bool = False) -> str:
...
def _event_hash(event: Mapping[str, Any]) -> str:
    ...
    use_hex = len(expected_hash) == 64
    payload = _canonical_public_json(scope).encode("utf-8")
    if use_hex:
        import hashlib
...
def _merkle_root_v1(event_hashes: Sequence[str]) -> str:
    use_hex = False
    if event_hashes and len(event_hashes[0]) == 64:
        use_hex = True
    ...
    nodes = [
        h(("CORTEX-MERKLE-LEAF-v1:" + event_hash).encode("utf-8")) for event_hash in event_hashes
    ]
    if not nodes:
        return h(b"CORTEX-MERKLE-EMPTY-v1")
    ...
    while len(nodes) > 1:
        ...
        for index in range(0, len(nodes), 2):
            ...
            if right is None:
                ...
                next_nodes.append(h(payload.encode("utf-8")))
        nodes = next_nodes
    return nodes[0]
