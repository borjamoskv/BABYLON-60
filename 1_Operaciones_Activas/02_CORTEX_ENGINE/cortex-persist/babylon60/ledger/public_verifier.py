...
class _LegacyVectorVerifier:
    ...
    def _verify_vector(self, path: Path) -> None:
        self.artifacts[path.name] = _sha256_file(path, force_hex=True) if path.exists() else None
        ...
        try:
            ...
            actual_hash = hashlib.sha256(h_input.encode("utf-8")).hexdigest()
...
class _PublicLedgerVerifier:
    ...
    def _artifact_hashes(self) -> dict[str, str | None]:
        ...
        use_hex = any(isinstance(v, str) and len(v) == 64 for v in hashes.values())
        return {
            artifact_name: _sha256_file(path, force_hex=use_hex) if path.exists() else None
