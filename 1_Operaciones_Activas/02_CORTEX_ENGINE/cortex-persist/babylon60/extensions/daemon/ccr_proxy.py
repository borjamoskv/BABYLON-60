...
def verify_token(
    ...
    """Validate Bearer token. Even on loopback, unauthenticated access
    lets any local process (browser JS, rogue extension, SSRF) call the
    engine. Token is read from CORTEX_API_TOKEN env var."""
    expected = os.getenv("CORTEX_API_TOKEN", "")
    if not expected:
        raise HTTPException(
            status_code=503,
            detail="CORTEX_API_TOKEN not set — CCR refuses to serve without auth.",
        )
...
def _build_anthropic_response(text: str) -> dict:
    return {
        ...
        "type": "message",
        "role": "assistant",
        "model": LOCAL_MODEL,  # report actual model, never spoof
...
async def messages_endpoint(request: Request, _token: str = Depends(verify_token)):
    """
    Anthropic-to-OpenAI translation proxy. Receives POST /v1/messages,
    translates to OpenAI format, routes to local model, translates back.
    """
