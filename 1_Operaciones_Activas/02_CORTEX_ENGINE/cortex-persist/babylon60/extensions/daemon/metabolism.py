...
logger = logging.getLogger("babylon60_extensions.daemon.metabolism")
...
class MetabolismEngine:
    ...
    def metabolize(self, total_alerts: int) -> float:
        ...
        logger.info(
            "PULSE-Ω: %d [HR: %.2f | Sig: %.2f | Ent: %.2f] -> Next beat in %.0fs",
            int(self.bpm),
            float(self.vitals.heart_rate),
            float(self.vitals.signal),
            float(self.vitals.entropy),
            final_wait,
        )
        return final_wait
