...
class EventLedgerL3:
    ...
    async def append_event(self, event: MemoryEvent) -> None:
        ...
        await enforce_taint_check(self._conn, token, event.content, event.tenant_id)
...
def _compute_event_signature_from_parts(
    *,
    ...
    content_hash = bytes_to_base60(hashlib.sha3_256(content.encode()).digest())
    metadata_hash = bytes_to_base60(
        hashlib.sha3_256(_canonical_metadata(metadata).encode()).digest()
    )
    payload = (
        f"{event_id}:{timestamp}:{tenant_id}:{role}:{content_hash}:"
        f"{token_count}:{session_id}:{metadata_hash}:{prev_hash}"
    )
    return bytes_to_base60(hashlib.sha3_256(payload.encode()).digest())
