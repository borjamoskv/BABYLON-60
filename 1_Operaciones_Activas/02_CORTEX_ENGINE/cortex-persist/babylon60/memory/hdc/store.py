...
class HDCVectorStoreL2:
    ...
    def __init__(
        ...
        db_path: str | Path = "~/.babylon60/hdc_vectors.db",
        ...
        if self._conn is None:
            ...
            if sqlite_vec is not None:
                try:
                    ...
                    logger.warning(
                        "Could not load sqlite_vec extension: %s. Falling back to pure Python vectors.",
                        e,
                    )
                    self._vec_loaded = False
            else:
    ...
    async def memorize(self, fact: CortexFactModel, fact_type: str | None = None) -> None:
        ...
        await enforce_taint_check(conn, token, fact.content, fact.tenant_id)
