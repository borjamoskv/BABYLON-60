...
# Paths
CORTEX_DB_PATH = Path("~/10_PROJECTS/Teorema-Robinson-Moskv/1_Operaciones_Activas/cortex-persist/cortex_data.db")
...
def _generate_taint_hash(email: str, timestamp: str) -> str:
    ...
    return bytes_to_base60(hashlib.sha3_256(raw.encode()).digest())
    ...
    with open(csv_path, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            try:
                ...
                logger.error("Error ingesting %s: %s", email, e)

    conn.commit()
    conn.close()
    logger.info("Elite cohort ingested: %s records (C5-REAL).", count)
    ...
    with open(csv_path, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            try:
                ...
                logger.error("Error ingesting volume %s: %s", email, e)

    conn.commit()
    conn.close()
    logger.info("Volume base ingested: %s new records.", count)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sovereign Audience Ingestion for CORTEX-Sidecar")
