...
Implements Argon2id KDF for Master Identity generation and an Accumulative Hash Chain
for the deterministic, verifiable state root.
"""
...
class L0IdentityForge:
    """
    Handles the cryptographic forging and retrieval of the L0 Master Identity.
    Enforces KDF-0 invariant: OS permissions + Argon2id AEAD.
    """
    ...
    def forge_identity(self, passphrase: str) -> IdentityPayload:
        """
        Generates a new L0 TRNG seed, encrypts it via Argon2id + AES-GCM,
        and saves it to disk with strict 0600 permissions.
        """
    ...
    def load_identity(self, passphrase: str) -> IdentityPayload:
        """Loads and decrypts the L0 Identity using the provided passphrase."""
...
class StateRootAccumulator:
    """
    Implements the Accumulative Hash Chain for the Sovereign Ledger (Ockham-Hash).
    ...
    def get_latest_root(self) -> tuple[int, bytes]:
        """Returns the latest (lamport, root_hash). Returns (0, b'') if empty."""
    ...
    def append_event(self, lamport_tick: int, event_hash: bytes, private_key_b64: str) -> bytes:
        """
        Calculates the new root and inserts it into the state_root table.
        MUST be called within an active SQLite WAL transaction.
        """
    ...
    def verify_chain(self, public_key_b64: str) -> bool:
        """
        Verifies the mathematical integrity of the entire state root chain.
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT lamport, event_hash, root, signature FROM state_root ORDER BY lamport ASC"
        )
        ...
        for row in rows:
            ...
            try:
                ...
                logger.error(
                    f"Invalid signature at tick {lamport}. Cryptographic forgery detected!"
                )
                return False
...
class IdentityAnchorManager:
    """
    Manages the 'identity_anchor' table, linking the L0 Keystore to the SQLite Ledger.
    ...
    def is_anchored(self) -> bool:
        """Checks if the ledger has already been anchored."""
        cursor = self.conn.cursor()
    ...
    def anchor_genesis(self, payload: IdentityPayload) -> str:
        """
        Anchors the L0 Identity into the SQLite DB.
        Generates the Node ID and signs the genesis payload.
        """
        if self.is_anchored():
