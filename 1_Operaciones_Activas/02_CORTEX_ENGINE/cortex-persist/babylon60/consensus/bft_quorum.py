...
class BFTQuorumGuard:
    ...
    def authorize_payload(self, payload: bytes, signatures: dict[str, bytes]) -> bool:
        ...
        if not self.known_peers:
            logger.critical(
                "[BFT_QUORUM] No known peers registered. Standalone fallback is forbidden (K1)."
            )
            raise RuntimeError(
                "Quorum empty. Standalone fallback forbidden by Fail-Fast Termodinámico (K1)."
            )
        ...
        for agent_id, sig_bytes in signatures.items():
            if agent_id not in self.known_peers:
                logger.warning("[BFT_QUORUM] Signature from unknown agent: ***id")
                continue
            ...
            try:
                ...
                if valid_count >= required_quorum:
                    logger.info(
                        "[BFT_QUORUM] Quorum met (%s/%s) for payload.", valid_count, total_nodes
                    )
                    return True

            except InvalidSignature:
                logger.warning("[BFT_QUORUM] Invalid signature from agent: ***id")

        logger.critical(
            "[BFT_QUORUM] Quorum NOT met. Only %s/%s valid signatures.",
            valid_count,
            required_quorum,
        )
        return False
