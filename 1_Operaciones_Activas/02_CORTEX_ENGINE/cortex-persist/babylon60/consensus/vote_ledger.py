...
class ImmutableVoteLedger:
    ...
    async def verify_merkle_roots(self, tenant_id: str = "default") -> list[dict]:
        """Verifica todas las raíces de Merkle registradas contra el estado causal del ledger."""
        cursor = await self.conn.execute(
        ...
        report = []
        for r_id, expected_root, chk_timestamp in roots:
            # Reconstruir el estado del ledger hasta el momento del checkpoint
            v_cursor = await self.conn.execute(
                "SELECT hash FROM vote_ledger WHERE tenant_id = ? AND timestamp <= ? ORDER BY id ASC",
                (tenant_id, chk_timestamp),
            )
            hashes = [row[0] for row in await v_cursor.fetchall()]
            actual_root = self._build_merkle_tree(hashes)

            report.append(
                {
                    "checkpoint_id": r_id,
                    "valid": actual_root == expected_root,
