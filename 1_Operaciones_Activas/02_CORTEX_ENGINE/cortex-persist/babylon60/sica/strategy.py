...
class SearchStrategy:
    ...
    def _mutate_weight(
        ...
        reason: str,
        mutation_type: MutationType,
        new_weight_fn: Callable[[float], float],
    ) -> StrategyMutation:
        """Helper to apply a weight mutation to a heuristic."""
        h = self._find_heuristic(heuristic_name)
        before_hash = self._genome.genome_hash

        h.weight = new_weight_fn(h.weight)
    ...
    def mutate_amplify(
        ...
        return self._mutate_weight(
            heuristic_name, reason, MutationType.AMPLIFY, lambda w: min(1.0, w * factor)
        )
    ...
    def mutate_attenuate(
        ...
        return self._mutate_weight(
            heuristic_name, reason, MutationType.ATTENUATE, lambda w: max(0.0, w * factor)
        )
