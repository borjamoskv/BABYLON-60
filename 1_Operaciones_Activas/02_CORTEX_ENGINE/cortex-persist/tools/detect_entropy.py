...
def _fmt_rich(report_dict: dict[str, Any], label: str = "") -> str:
    ...
    lines.append(
        f"  │  {_exergy_badge(exergy)}   {c(str(exergy), ec, bold=True)}  {_bar(exergy, 24, ec)}"
    )
    lines.append("  │")
    ...
    rot_l = (
        "LOW"
        if r["context_rot_score"] < 0.2
        else ("HIGH" if r["context_rot_score"] > 0.5 else "MEDIUM")
    )
...
def _output_single_report(rdict: dict[str, Any], fmt: str, label: str) -> None:
    if fmt == "rich":
        logging.getLogger(__name__).info(_fmt_rich(rdict, label))
    elif fmt == "json":
        logging.getLogger(__name__).info(_fmt_json(rdict))
    elif fmt == "yaml":
        logging.getLogger(__name__).info(_fmt_yaml(rdict))
    elif fmt == "table":
        logging.getLogger(__name__).info(_fmt_table([(label, rdict)]))


def _process_single_input(args: Any, detector: Any, threshold: float) -> list[str]:
    if args.text:
        text = args.text
        label = "<inline>"
    elif args.stdin:
        ...
        try:
            ...
            logging.getLogger(__name__).info(f"{RED}ERROR{RESET}: {e}", file=sys.stderr)
            return ["__ERROR__"]
    ...
    _output_single_report(rdict, args.format, label)
    if args.fail_on_anergy and report.exergy_score < threshold:
        return [label]
    return []


def _format_batch_table(top_reports: list[tuple[str, dict[str, Any]]], reports: list[tuple[str, dict[str, Any]]], args: Any, exts: set[str], threshold: float, files_count: int) -> None:
    logging.getLogger(__name__).info(c(f"\n  BABYLON-60 LINGUISTIC ENTROPY SCAN — {args.dir}", BLUE, bold=True))
    logging.getLogger(__name__).info(c(f"  Files: {files_count} | Extensions: {', '.join(sorted(exts))}", GRAY))
    if threshold > 0:
        logging.getLogger(__name__).info(c(f"  Threshold: {threshold}", GRAY))
    logging.getLogger(__name__).info()
    logging.getLogger(__name__).info(_fmt_table(top_reports))
    ...
    logging.getLogger(__name__).info()
    logging.getLogger(__name__).info(
        c(f"  Summary: {files_count} files | avg exergy {avg_score:.3f}", WHITE, bold=True)
    )
    if threshold > 0 and below > 0:
        logging.getLogger(__name__).info(c(f"  ⚠ {below} file(s) below threshold {threshold}", YELLOW))


def _output_batch_reports(top_reports: list[tuple[str, dict[str, Any]]], reports: list[tuple[str, dict[str, Any]]], args: Any, exts: set[str], threshold: float, files_count: int) -> None:
    if args.format == "table":
        _format_batch_table(top_reports, reports, args, exts, threshold, files_count)
        ...
        logging.getLogger(__name__).info(
            json.dumps(
                [{"file": k, "report": v} for k, v in reports], ensure_ascii=False, indent=2
            )
        )
    elif args.format == "yaml":
        logging.getLogger(__name__).info(_fmt_yaml({"files": [{"file": k, **v} for k, v in reports]}))


def _process_batch_directory(args: Any, detector: Any, threshold: float) -> list[str]:
    exts = {f".{e.lstrip('.')}" for e in args.ext}
    files = _scan_directory(args.dir, exts, args.exclude)

    if not files:
        logging.getLogger(__name__).info(c(f"No files found in '{args.dir}' with extensions {exts}", YELLOW))
        return []
    ...
    _output_batch_reports(top_reports, reports, args, exts, threshold, len(files))
    return failed
...
def main() -> int:
    ...
    if args.text or args.file or args.stdin:
        failed = _process_single_input(args, detector, threshold)
        if "__ERROR__" in failed:
            return 2
    else:
        failed = _process_batch_directory(args, detector, threshold)

    if failed:
        logging.getLogger(__name__).info(
            c(
                f"\n  ✗ ANERGY GATE FAILED: {len(failed)} file(s) below threshold {threshold}",
                RED,
                bold=True,
            )
        )
        for f in failed:
            logging.getLogger(__name__).info(c(f"    · {f}", RED))
        return 1
