...
def migrate_agent_file(path: Path) -> bool:
    try:
        ...
        new_content = yaml.dump(
            new_data, default_flow_style=False, sort_keys=False, allow_unicode=True
        )
        path.write_text(new_content, encoding="utf-8")
        return True
    except Exception as e:  # noqa: BLE001
        logging.getLogger(__name__).info(f"Error migrating {path.name}: {e}")
        return False
...
def main():
    ...
    if not agents_dir.exists():
        logging.getLogger(__name__).info("Definitions directory not found.")
        return
