...
class ChaosAgent(BaseAgent):
    ...
    async def tick(self) -> None:
        self.tick_count += 1
        logging.getLogger(__name__).info(f"[{self.agent_id}] ChaosAgent tick {self.tick_count}/{self.crash_on_tick}")
        if self.tick_count >= self.crash_on_tick:
            logging.getLogger(__name__).info(f"[{self.agent_id}] CRASHING NOW!")
