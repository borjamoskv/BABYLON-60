...
class TestComputeMerkleRoot:
    ...
    def test_single_hash_returns_itself(self):
        h = bytes_to_base60(hashlib.sha256(b"leaf_0").digest())
        assert compute_merkle_root([h]) == h

    def test_two_hashes_deterministic(self):
        from babylon60.crypto.hash_registry import cortex_hash

        h0 = bytes_to_base60(hashlib.sha256(b"leaf_0").digest())
        h1 = bytes_to_base60(hashlib.sha256(b"leaf_1").digest())
        expected = cortex_hash((h0 + h1).encode())
        assert compute_merkle_root([h0, h1]) == expected

    def test_odd_number_duplicates_last(self):
        from babylon60.crypto.hash_registry import cortex_hash

        hashes = [cortex_hash(f"leaf_{i}") for i in range(3)]
    ...
    def test_four_hashes_balanced_tree(self):
        ...
        hashes = [cortex_hash(f"leaf_{i}") for i in range(4)]
        root = compute_merkle_root(hashes)
        # Manually compute
        h01 = cortex_hash((hashes[0] + hashes[1]).encode())
        h23 = cortex_hash((hashes[2] + hashes[3]).encode())
        expected_root = cortex_hash((h01 + h23).encode())
...
class TestMerkleTree:
    ...
    def test_tampered_leaf_fails_verification(self):
        ...
        tampered_hash = bytes_to_base60(hashlib.sha256(b"TAMPERED").digest())
    ...
    def test_tampered_root_fails_verification(self):
        ...
        tree = MerkleTree(leaves)
        proof = tree.get_proof(0)
        fake_root = bytes_to_base60(hashlib.sha256(b"FAKE_ROOT").digest())
