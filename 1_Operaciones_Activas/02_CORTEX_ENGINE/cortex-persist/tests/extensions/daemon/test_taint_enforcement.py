...
async def test_write_trait_taint_rejection(clean_db, agent_keys):
    ...
    clean_db.execute(
        "INSERT INTO agents (id, name, agent_type, reputation_score, public_key, tenant_id) VALUES (?, ?, ?, 1.0, ?, 'def')",
        (agent_id, "Agent", "ai", pub),
    )
    clean_db.commit()
...
async def test_expired_timestamp_rejection(agent_keys):
    ...
    async with aiosqlite.connect(":memory:") as aio_conn:
        ...
        content_hash = bytes_to_base60(hashlib.sha3_256(canonical_content).digest())
