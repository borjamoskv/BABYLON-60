...
def test_confidence_resolution() -> None:
    ...
    # Test float
    b_float = MockBelief(proposition="Test", confidence=0.7)
    assert scheduler._extract_confidence_value(b_float) == 0.7

    b_enum = MockBelief(proposition="Test", confidence_score=BeliefConfidence.C4_CONFIRMED)
    assert scheduler._extract_confidence_value(b_enum) == 0.8

    # Test String
    b_str = MockBelief(proposition="Test", confidence="C5")
    assert scheduler._extract_confidence_value(b_str) == 1.0

    # Test Unknown fallback
    b_unknown = MockBelief(proposition="Test", confidence="UNKNOWN")
...
def test_rank_beliefs_with_budget() -> None:
    ...
    b1 = MockBelief(proposition="Small", cost_tokens=2.0, confidence=1.0)
    b2 = MockBelief(proposition="Medium", cost_tokens=5.0, confidence=0.8)
    b3 = MockBelief(proposition="Large", cost_tokens=10.0, confidence=0.6)
