...
def _writer_process_target(db_path: Path, sync_event: multiprocessing.Event, fault_point: str):
    ...
    async def _run():
        try:
            ...
            logging.getLogger(__name__).info("WORKER: Calling engine.start()")
            await engine.start()

            if fault_point == "before_sqlite":
                logging.getLogger(__name__).info("WORKER: Setting sync_event (before_sqlite)")
                sync_event.set()
            ...
            async def _faulty_checkpoint(*args, **kwargs):
                logging.getLogger(__name__).info(f"WORKER: In _faulty_checkpoint, fault_point={fault_point}")
                if fault_point == "after_sqlite_before_ledger":
                    logging.getLogger(__name__).info("WORKER: Setting sync_event (after_sqlite_before_ledger)")
                    sync_event.set()
                    logging.getLogger(__name__).info("WORKER: Waiting for SIGKILL (after_sqlite_before_ledger)")
            ...
            logging.getLogger(__name__).info("WORKER: Calling engine.facts.store()")
            ...
            if fault_point == "after_commit":
                logging.getLogger(__name__).info("WORKER: Setting sync_event (after_commit)")
                sync_event.set()
                logging.getLogger(__name__).info("WORKER: Waiting for SIGKILL (after_commit)")
                await asyncio.sleep(10)

            logging.getLogger(__name__).info("WORKER: Closing engine")
            await engine.close()
        except Exception as e:  # noqa: BLE001
            logging.getLogger(__name__).info(f"WORKER ERROR: {type(e).__name__}: {e}")
            import traceback
