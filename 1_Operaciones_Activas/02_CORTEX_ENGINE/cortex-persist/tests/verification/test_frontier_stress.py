...
async def test_ledger_concurrency_bombing(ledger_db):
    ...
    try:
        ...
        # Verify the entire chain using the built-in verifier
        verify_res = await ledger.verify_chain()
        assert verify_res["status"] == "verified", (
            f"Ledger verification failed: {verify_res.get('violations')}"
        )
        assert verify_res["verified_count"] == CONCURRENCY_LEVEL

        # Verify the last hash of the object is correctly updated
        assert ledger._last_hash == rows[-1][0]
...
async def test_byzantine_taint_tsunami(ledger_db, monkeypatch):
    ...
    await ledger_db.execute(
        "CREATE TABLE IF NOT EXISTS agents (id TEXT PRIMARY KEY, public_key TEXT, is_active INTEGER, tenant_id TEXT DEFAULT 'default')"
    )
    await ledger_db.execute(
