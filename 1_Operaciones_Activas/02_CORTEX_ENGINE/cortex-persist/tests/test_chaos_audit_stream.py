...
def fresh_ghost_pipeline():
    ...
    # Patch Thread.start to run synchronously to avoid background thread leakage
    # during tests which outlive the AsyncMock patches!
    original_start = threading.Thread.start

    def sync_start(self):
        if "ghost-persist" in self.name:
            self._target(*self._args, **self._kwargs)
        else:
            original_start(self)

    patcher = patch.object(threading.Thread, "start", side_effect=sync_start, autospec=True)
    patcher.start()
