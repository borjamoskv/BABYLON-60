...
def test_create_api_key_bootstrap_is_single_use(admin_client) -> None:
    ...
    if first.status_code != 200:
        logging.getLogger(__name__).info(f"DEBUG: {first.json()}")
...
def test_create_api_key_bootstrap_with_token(admin_client, monkeypatch) -> None:
    manager, client = admin_client
    monkeypatch.setenv("MOSKV_BOOTSTRAP_TOKEN", "secure-bootstrap-token-123")
    ...
    # 2. With invalid authorization header
    ...
    # 3. With valid authorization header
    res_valid = client.post(
        "/v1/admin/keys",
        params={"name": "bootstrap-admin", "tenant_id": "tenant-alpha"},
        headers={"Authorization": "Bearer secure-bootstrap-token-123"},
    )
