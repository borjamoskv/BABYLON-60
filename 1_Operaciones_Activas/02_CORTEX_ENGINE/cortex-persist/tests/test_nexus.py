...
def nexus_env():
    ...
    with open(os.path.join(canonical_root, "AGENTS.md"), "w") as f:
        f.write("nexus content")
...
def test_nexus_propagate_symlinks(nexus_env):
    ...
    # Run propagation
    results = engine.propagate([satellite], ["AGENTS.md"])

    assert results[satellite] is True

    # Verify symlink exists
    target = os.path.join(satellite, "AGENTS.md")
    assert os.path.islink(target)
    assert os.readlink(target) == os.path.join(canonical, "AGENTS.md")

    # Verify invariants
    assert engine.validate_invariants([satellite], ["AGENTS.md"]) is True
...
def test_nexus_overwrites_physical_redundancy(nexus_env):
    ...
    assert engine.validate_invariants([satellite], ["AGENTS.md"]) is False

    # Run propagation to purge redundancy
    engine.propagate([satellite], ["AGENTS.md"])
