...
def disable_taint_bypass(monkeypatch):
    """Enforce strict taint token verification during unit testing."""
    monkeypatch.setenv("CORTEX_NO_TAINT_ENFORCE", "0")
    monkeypatch.setenv("CORTEX_TESTING", "0")
...
def sqlite_conn() -> sqlite3.Connection:
    ...
    with causal_write(conn):
        conn.execute(
            "CREATE TABLE agents (id TEXT PRIMARY KEY, public_key TEXT, is_active INTEGER, tenant_id TEXT DEFAULT 'default')"
        )
        conn.commit()
    return conn
