...
def valid_bundle(tmp_path, km):
    ...
    smt.update(cortex_hash(audit_id_1.encode()), audit_id_1)
    smt.update(cortex_hash(audit_id_2.encode()), audit_id_2)
...
def test_compliance_verifier_tampered_signature(valid_bundle, tmp_path):
    bundle_path, public_key_b64 = valid_bundle

    # Unpack, tamper, repack
    tampered_zip = tmp_path / "tampered_signature_bundle.zip"
    ...
    # Tamper the signature to trigger a signature mismatch
    export_data[0]["signature"] = "a" * len(export_data[0]["signature"])
...
def test_compliance_verifier_tampered_hash(valid_bundle, tmp_path):
    ...
    # Unpack, tamper, repack
    tampered_zip = tmp_path / "tampered_hash_bundle.zip"
    ...
    report = verifier.verify()
    assert report["status"] == "CRITICAL_TAMPER_DETECTED"
    assert "Chain linkage broken" in report["reason"]
