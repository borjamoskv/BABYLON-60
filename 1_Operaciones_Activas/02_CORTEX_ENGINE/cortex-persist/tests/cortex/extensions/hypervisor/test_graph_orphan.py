...
async def test_cascading_quarantine_graph_orphan():
    ...
    for i in range(5):
        child = BeliefObject(
            ...
            state=BeliefStatus.ACTIVE,
            relations=BeliefRelations(entails=(root_belief.id,)),
        )
    ...
    for l1_child in children_l1:
        for j in range(10):
            child = BeliefObject(
                ...
                project=PROJECT,
                tenant_id=TENANT,
                confidence_score=0.5,  # Default value since BeliefConfidence is deprecated
                state=BeliefStatus.ACTIVE,
                relations=BeliefRelations(entails=(l1_child.id,)),
            )
            await engine._persist_belief(child)
            total_beliefs += 1
