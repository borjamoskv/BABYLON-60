...
class BCIMaestroBridge:
    ...
    async def handle_desktop_action(self, instruction: str, payload: str | bytes) -> Any:
        ...
        """
        logger.info("[BCI-Bridge] Resolving desktop action: %s", instruction)
        ...
        try:
            ...
            logger.error("[BCI-Bridge] Invalid JSON payload: %s", e)
        ...
        try:
            ...
            logger.info("[BCI-Bridge] Execution result of %s: %s", instruction, result)
            return result
        except Exception as e:  # noqa: BLE001
            logger.error("[BCI-Bridge] Error executing %s: %s", instruction, e)
