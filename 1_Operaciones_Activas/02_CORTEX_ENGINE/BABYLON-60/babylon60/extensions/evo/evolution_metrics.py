...
from babylon60.extensions.evo.evolution_types import DomainMetrics, Mutation
    ...
    def _init_database(self) -> None:
        ...
        try:
            conn.executescript("""
                ...
                CREATE TRIGGER IF NOT EXISTS mutation_ledger_no_update
                BEFORE UPDATE ON mutation_ledger
                BEGIN
                    SELECT RAISE(ABORT, 'ledger is append-only: UPDATE forbidden');
                END;
                CREATE TRIGGER IF NOT EXISTS mutation_ledger_no_delete
                BEFORE DELETE ON mutation_ledger
                BEGIN
