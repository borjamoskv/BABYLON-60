...
try:
    from babylon60.database.core import connect_sync as db_connect_sync
except ImportError:
    from babylon60.database.core import connect as db_connect_sync
...
class NexusDB:
    ...
    def _get_conn(self) -> sqlite3.Connection:
        if not hasattr(self._local, "conn"):
            try:
                self._local.conn = db_connect_sync(
                    self._db_path,
                    timeout=10,
                    row_factory=sqlite3.Row,
                )
            except TypeError:
                conn = db_connect_sync(self._db_path)
    ...
    def query(
        ...
        rows = conn.execute(
            f"SELECT * FROM nexus_mutations {where} ORDER BY priority ASC, timestamp DESC LIMIT ?",  # nosec B608
            params,
        ).fetchall()
