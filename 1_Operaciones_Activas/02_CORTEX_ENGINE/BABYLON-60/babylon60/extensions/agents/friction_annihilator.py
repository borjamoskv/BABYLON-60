...
class FrictionAnnihilatorAgent(EngineAwareMixin):
    ...
    async def annihilate_friction(
        ...
        logger.info("FrictionAnnihilator: Engaging on %s", target_file)
        self._ensure_engine()
        ...
        try:
            ...
            logger.error("FrictionAnnihilator: Failed to annihilate friction: %s", e)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        logging.getLogger(__name__).info("Usage: python friction_annihilator.py <target_file> <error_trace>")
