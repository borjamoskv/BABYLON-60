...
async def anergy_reducer(client: InferenceClient, original_prompt: str, results: list) -> str:
    ...
    if not successful:
        return f"🔴 Colapso fallido: {len(failed)}/{len(results)} subagentes erraron.\n" + \
               "\n".join(f"  - Subagente {r['task_id']}: {r['result'][:100]}" for r in failed)

    combined = "\n\n".join(
        f"### Subtarea {r['task_id']}: {r['task_desc']}\n{r['result']}"
        for r in successful
    )
...
async def run_swarm_orchestrator(
    ...
    tasks = [
        asyncio.create_task(
            execute_subagent(i, desc, client, pager, semaphore, config.mcts_delay_s, on_progress)
        )
