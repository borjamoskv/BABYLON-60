...
def main():
    parser = argparse.ArgumentParser(description='Generate Vm.sol based on the cheatcodes json created by Foundry')
    parser.add_argument('--from', metavar='PATH', dest='path', required=False, help='path to a json file containing the Vm interface, as generated by Foundry')
    ...
    pp = CheatcodesPrinter(spdx_identifier='MIT OR Apache-2.0', solidity_requirement='>=0.8.13 <0.9.0')
    pp.p_prelude()
    pp.prelude = False
    out += pp.finish()
    out += '\n\n'
    out += VM_SAFE_DOC
    vm_safe = Cheatcodes(errors=[], events=contract.events, enums=contract.enums, structs=contract.structs, cheatcodes=safe)
    pp.p_contract(vm_safe, 'VmSafe')
    out += pp.finish()
    out += '\n\n'
    out += VM_DOC
    vm_unsafe = Cheatcodes(errors=[], events=[], enums=[], structs=[], cheatcodes=unsafe)
    pp.p_contract(vm_unsafe, 'Vm', 'VmSafe')
    out += pp.finish()

    def memory_to_calldata(m: re.Match) -> str:
        return ' calldata ' + m.group(1)
    out = re.sub(' memory (.*returns)', memory_to_calldata, out)
    with open(OUT_PATH, 'w') as f:
        f.write(out)
    forge_fmt = ['forge', 'fmt', OUT_PATH]
    res = subprocess.run(forge_fmt)
    assert res.returncode == 0, f'command failed: {forge_fmt}'
    logging.info(f'Wrote to {OUT_PATH}')
...
class Function:
    ...
    def __init__(self, id: str, description: str, declaration: str, visibility: Visibility, mutability: Mutability, signature: str, selector: str, selector_bytes: bytes):
        self.id = id
    ...
    def from_dict(d: dict[str, typing.Any]) -> 'Function':
        return Function(d['id'], d['description'], d['declaration'], Visibility(d['visibility']), Mutability(d['mutability']), d['signature'], d['selector'], bytes(d['selectorBytes']))
...
class Cheatcode:
    ...
    def from_dict(d: dict[str, typing.Any]) -> 'Cheatcode':
        return Cheatcode(Function.from_dict(d['func']), str(d['group']), str(d['status']), str(d['safety']))
...
class Enum:
    ...
    def from_dict(d: dict[str, typing.Any]) -> 'Enum':
        return Enum(d['name'], d['description'], list(map(lambda v: EnumVariant(**v), d['variants'])))
...
class Struct:
    ...
    def from_dict(d: dict[str, typing.Any]) -> 'Struct':
        return Struct(d['name'], d['description'], list(map(lambda f: StructField(**f), d['fields'])))
...
class Cheatcodes:
    ...
    def __init__(self, errors: list[Error], events: list[Event], enums: list[Enum], structs: list[Struct], cheatcodes: list[Cheatcode]):
        self.errors = errors
        self.events = events
        self.enums = enums
    ...
    def from_dict(d: dict[str, typing.Any]) -> 'Cheatcodes':
        return Cheatcodes(errors=[Error.from_dict(e) for e in d['errors']], events=[Event.from_dict(e) for e in d['events']], enums=[Enum.from_dict(e) for e in d['enums']], structs=[Struct.from_dict(e) for e in d['structs']], cheatcodes=[Cheatcode.from_dict(e) for e in d['cheatcodes']])
...
class ItemOrder:
    ...
    def default() -> 'ItemOrder':
        return ItemOrder([Item.ERROR, Item.EVENT, Item.ENUM, Item.STRUCT, Item.FUNCTION])
...
class CheatcodesPrinter:
    ...
    def __init__(self, buffer: str='', prelude: bool=True, spdx_identifier: str='UNLICENSED', solidity_requirement: str='', block_doc_style: bool=False, indent_level: int=0, indent_with: int | str=4, nl_str: str='\n', items_order: ItemOrder=ItemOrder.default()):
