...
# from babylon60.agents.primitives.dispatcher import apex_dispatcher  # purgado por anergía
# from babylon60.engine.crystallizer import auto_crystallizer  # purgado por anergía
...
class NightShiftDaemon:
    ...
    def execute_purge(self) -> str:
        ...
        if not self.transcript_path.exists():
            logger.error("Transcript not found at %s", self.transcript_path)
            apex_dispatcher.execute("OP_APOPTOSIS")

        logger.info("Initiating NightShift Purge on %s", self.transcript_path.name)
        ...
        logger.info(
            "NightShift complete. Episodic noise purged. Axiom crystallized at %s", axiom_file
        )
