...
3.  **Immediate Response (Hard Path):** Vía de baja latencia que acusa recibo y detiene el sensor para evitar *flooding*. No modifica el estado a largo plazo.
...
7.  **Homeostasis vs. Collapse:** El sistema evalúa la relación $\rho = \lambda / \mu$ (llegada vs. capacidad de procesamiento). Si $\rho > 1$, ocurre el *Interrupt Storm* o el *Dolor Crónico*.
