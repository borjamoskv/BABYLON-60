...
class EpistemicHaltUI:
    def __init__(self):
        ...
        # Título
        tk.Label(self.root, text="⬛️ KERNEL EPISTEMIC HALT [AX-4] ⬛️", 
                 fg="#ff3333", bg="#050505", font=title_font, pady=15).pack()
...
- Respaldo 2: Gemini 3.8 Flash (Menor exergía lógica)"""
        ...
        tk.Label(self.root, text=text, fg="#00ff00", bg="#050505", 
                 font=body_font, justify="left", padx=20, pady=10).pack(anchor="w")
        ...
        btn_style = {"bg": "#1a1a1a", "fg": "#ffffff", "font": btn_font, 
                     "activebackground": "#ff3333", "activeforeground": "#ffffff",
                     "bd": 1, "relief": "solid", "padx": 10, "pady": 5}
                     
        tk.Button(btn_frame, text="[F-1] FORZAR CLAUDE 4.6", command=lambda: on_click("Claude"), **btn_style).grid(row=0, column=0, padx=10)
        tk.Button(btn_frame, text="[F-2] DEGRADAR 3.8 FLASH", command=lambda: on_click("Flash"), **btn_style).grid(row=0, column=1, padx=10)
        
        btn_style["fg"] = "#ff3333"
        tk.Button(btn_frame, text="[F-3] PURGA TERMODINÁMICA", command=lambda: on_click("Kill"), **btn_style).grid(row=0, column=2, padx=10)
