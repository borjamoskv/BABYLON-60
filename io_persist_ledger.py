...
class LedgerPersist:
    ...
    def io_persist_ledger(self, ledger: GraphLedger) -> int:
        ...
        for crdt_entry in ledger.crdt.to_dict().values():
            ...
            cursor.execute('INSERT OR IGNORE INTO dag_nodes (node_id, parent_id, claim, payload_hash) VALUES (?, ?, ?, ?)', (node.node_id, node.parent_id, node.claim_summary, node.payload_hash))
