...
def run_improved_mapping(matrix_path: Path, article_path: Path) -> dict:
    ...
    return {
        "article_metrics": {
            "path": str(article_path.relative_to(article_path.parents[1])),
            "entropy": article_entropy,
            "length_chars": len(article_text)
        },
        "primitives_metrics": {
            "total": len(primitives),
            "average_entropy": avg_p_entropy,
            "boltzmann_ratio": round(sum(1 for r in mapped_records if r["vector"] == "BOLTZMANN_EQUILIBRIUM") / len(primitives), 4)
        },
        "records_head": mapped_records[:10]  # First 10 for validation
    }
...
def main():
    ...
    try:
        ...
        output = {
            "cortex_taint": f"borjamoskv:improved_thermo:{v_hash[:16]}",
            "report": report,
            "hash": v_hash
        }
