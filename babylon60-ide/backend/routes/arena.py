...
def get_leaderboard() -> dict[str, Any]:
    ...
    try:
        run_cursor = conn.execute('SELECT id, fetched_at, last_updated, latency_ms, entropy FROM sync_runs ORDER BY id DESC LIMIT 1')
        run = run_cursor.fetchone()
        if not run:
            return {'exists': False, 'models': [], 'meta': {}}
        run_id = run['id']
        meta = {'fetched_at': run['fetched_at'], 'last_updated': run['last_updated'], 'latency_ms': run['latency_ms'], 'entropy': run['entropy']}
        models_cursor = conn.execute('SELECT rank, model, vendor, score, votes FROM leaderboard_snapshots WHERE run_id = ? ORDER BY rank', (run_id,))
...
def get_battles() -> dict[str, Any]:
    ...
    conn = connect_readonly(db_path)
    try:
        cursor = conn.execute('SELECT id, timestamp, vector, prompt, model_a, model_b, response_a, response_b, winner, entropy_a, entropy_b, cortex_taint_hash FROM alpha_ledger ORDER BY id DESC LIMIT 50')
