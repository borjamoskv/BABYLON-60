...
def get_battles() -> dict[str, Any]:
    ...
    try:
        cursor = conn.execute(
            "SELECT id, timestamp, vector, prompt, model_a, model_b, response_a, response_b, winner, entropy_a, entropy_b, cortex_taint_hash FROM alpha_ledger ORDER BY id DESC LIMIT 50"
        )
