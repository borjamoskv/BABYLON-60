...
def _projection(root: Path) -> list[dict[str, Any]]:
    ...
    for ev in reversed(data['events']):
        ...
        if et == 'DELEGATION_QUEUED':
            ...
            by_id[did] = {'delegation_id': did, 'directive': p.get('directive', ''), 'kind': p.get('kind', 'custom'), 'state': 'QUEUED', 'created_at': ev['created_at'], 'event_id': ev['event_id'], 'result': None}
            order.append(did)
            continue
...
def enqueue(req: DelegateRequest) -> dict[str, Any]:
    ...
    ev = cortex_ledger.append_event(root, event_type='DELEGATION_QUEUED', entity_ref=ENTITY, payload={'directive': req.directive, 'kind': kind}, metadata={'taint': 'operator:delegation-intent', 'cloud': kind in _CLOUD_KINDS})
    did = ev['current_hash'][:16]
    return {'delegation_id': did, 'directive': req.directive, 'kind': kind, 'state': 'QUEUED', 'cloud_blocked': kind in _CLOUD_KINDS and P0_OPEN, 'event': ev}

@router.get('')
def list_delegations() -> dict[str, Any]:
    root = _get_project_root()
    items = _projection(root)
    return {'delegations': items, 'p0_open': P0_OPEN, 'policy': 'cloud-upload (push/merge/ship/deploy) blocked until P0 key rotation'}
...
def execute(delegation_id: str) -> dict[str, Any]:
    ...
    if kind in _CLOUD_KINDS and P0_OPEN:
        ...
        cortex_ledger.append_event(root, event_type='DELEGATION_BLOCKED', entity_ref=ENTITY, payload={'delegation_id': delegation_id, 'kind': kind, 'reason': reason}, metadata={'taint': 'git-sentinel:p0-guard'})
    ...
    if kind in ('commit', 'precommit'):
        if not (root / '.git').is_dir():
            ...
            cortex_ledger.append_event(root, event_type='DELEGATION_FAILED', entity_ref=ENTITY, payload={'delegation_id': delegation_id, 'kind': kind, 'result': result}, metadata={'taint': 'git-sentinel:no-repo'})
        ...
        if code != 0:
            ...
            cortex_ledger.append_event(root, event_type='DELEGATION_FAILED', entity_ref=ENTITY, payload={'delegation_id': delegation_id, 'kind': kind, 'result': result}, metadata={'taint': 'git-sentinel:commit-fail'})
            raise HTTPException(422, result)
        _, head, _ = _git(root, 'rev-parse', '--short', 'HEAD')
        result = f'commit {head}: {(out.splitlines()[0] if out else msg)}'
        ev = cortex_ledger.append_event(root, event_type='DELEGATION_EXECUTED', entity_ref=ENTITY, payload={'delegation_id': delegation_id, 'kind': kind, 'result': result, 'head': head}, metadata={'taint': 'git-sentinel:commit-sealed'})
    ...
    if kind == 'status':
        ...
        ev = cortex_ledger.append_event(root, event_type='DELEGATION_EXECUTED', entity_ref=ENTITY, payload={'delegation_id': delegation_id, 'kind': kind, 'result': result}, metadata={'taint': 'git-sentinel:status'})
        return {'delegation_id': delegation_id, 'state': 'EXECUTED', 'result': result, 'event': ev}
    result = 'directiva custom registrada en el ledger (ejecución manual del agente)'
    ev = cortex_ledger.append_event(root, event_type='DELEGATION_EXECUTED', entity_ref=ENTITY, payload={'delegation_id': delegation_id, 'kind': kind, 'result': result}, metadata={'taint': 'operator:custom-intent'})
    return {'delegation_id': delegation_id, 'state': 'EXECUTED', 'result': result, 'event': ev}
...
def cancel(delegation_id: str) -> dict[str, Any]:
    ...
    ev = cortex_ledger.append_event(root, event_type='DELEGATION_CANCELLED', entity_ref=ENTITY, payload={'delegation_id': delegation_id, 'kind': deleg['kind']}, metadata={'taint': 'operator:cancel'})
