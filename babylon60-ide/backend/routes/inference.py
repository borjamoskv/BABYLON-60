...
def validate_zero_network(url: str) -> None:
    ...
    if parsed.scheme != 'http' or parsed.hostname not in ('127.0.0.1', 'localhost'):
        raise HTTPException(status_code=403, detail=f"C5-REAL VIOLATION: Endpoint '{url}' must be confined to loopback (http://127.0.0.1 or http://localhost).")
    lower = url.lower()
    forbidden = ['openai.com', 'anthropic.com', 'dashscope', 'googleapis.com', 'deepmind']
    for domain in forbidden:
        if domain in lower:
            raise HTTPException(status_code=403, detail=f"C5-REAL VIOLATION: Zero-Network Policy breached. External endpoint '{domain}' is strictly forbidden.")
...
def generate_local(req: InferenceRequest) -> dict[str, Any]:
    ...
    payload = {'model': req.model, 'messages': [{'role': 'system', 'content': 'You are MOSKV-1 APEX, a sovereign C5-REAL execution kernel operating on local Apple Silicon.'}, {'role': 'user', 'content': req.prompt}], 'temperature': req.temperature, 'max_tokens': req.max_tokens, 'stream': False}
    start_time = time.perf_counter()
    try:
        req_obj = urllib.request.Request(endpoint, data=json.dumps(payload).encode('utf-8'), headers={'Content-Type': 'application/json'}, method='POST')
    ...
    return {'text': text, 'model': req.model, 'tps': tps, 'latency_ms': latency_ms, 'sha256': sha256, 'provider': 'LOCAL_SILICON_FASTAPI_BRIDGE'}
...
def status_local() -> dict[str, Any]:
    try:
        ...
        return {'status': 'ONLINE', 'provider': 'Ollama/MLX Local Silicon', 'endpoint': 'http://127.0.0.1:11434', 'models': models}
...
def generate_mamba(req: MambaInferenceRequest) -> dict[str, Any]:
    try:
        ...
        return {'text': text, 'certificate': {'evidence_hash': cert.evidence_hash, 'ruleset_hash': cert.ruleset_hash, 'cert_hash': cert.cert_hash, 'residual_microbits': cert.residual_microbits, 'nodes_count': len(ledger.crdt.state)}, 'provider': 'NATIVE_MAMBA_SSM_LEDGER_ENGINE', 'vocab_size': len(tokenizer.vocab)}
    except (ValueError, TypeError, KeyError, RuntimeError, OSError, AssertionError) as e:
