...
def list_entries(limit: int = Query(50, ge=1, le=500), offset: int = Query(0, ge=0)) -> dict[str, Any]:
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT seq, event_id, stream, entity_id, event_type, lamport_t, entry_hash, prev_hash, cortex_taint, created_at FROM ledger_entries ORDER BY seq DESC LIMIT ? OFFSET ?",
            (limit, offset),
        )
        entries = []
