...
def ledger_stats() -> dict[str, Any]:
    """Chain statistics: total entries, latest hash, latest lamport_t."""
    ...
    try:
        ...
        if total > 0:
            cursor = conn.execute(
                "SELECT entry_hash, lamport_t, created_at FROM ledger_entries ORDER BY seq DESC LIMIT 1"
            )
            row = cursor.fetchone()
            latest = {
...
def list_entries(
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT seq, event_id, stream, entity_id, event_type, "
            "lamport_t, entry_hash, prev_hash, cortex_taint, created_at "
            "FROM ledger_entries ORDER BY seq DESC LIMIT ? OFFSET ?",
            (limit, offset),
        )
        entries = []
...
def get_entry(seq: int) -> dict[str, Any]:
    """Single entry detail with full payload."""
    root = _get_project_root()
    db_path = _find_ledger_db(root)
    if not db_path:
        raise HTTPException(404, "No ledger database found")
