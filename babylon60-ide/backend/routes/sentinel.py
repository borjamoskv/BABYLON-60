...
def sentinel_status() -> dict[str, Any]:
    ...
    if not is_git:
        warnings.append(
            {"level": "red", "msg": f"'{repo_name}' no es un repo git — sin Git Sentinel no hay ledger de mutaciones."}
        )
    ...
    if branch and branch != CANONICAL_BRANCH:
        warnings.append(
            {"level": "amber", "msg": f"Rama '{branch}' ≠ '{CANONICAL_BRANCH}' (canónica). Verifica antes de mutar."}
        )
