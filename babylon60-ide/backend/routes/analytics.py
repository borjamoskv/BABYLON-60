...
Exergía real computada sobre `ledger_entries`:
- Agregación (streams, event_types, agentes por taint, continuidad Lamport, span).
- Búsqueda BM25 léxica (ranking Okapi BM25 por frecuencia de términos —
...
def ledger_analytics() -> dict[str, Any]:
    """Real aggregation over the master ledger (eje DETERMINAR)."""
...
def ledger_search(
    ...
    """Okapi BM25 lexical ranking over payload_json + cortex_taint + event_type."""
    root = _get_project_root()
    db_path = _find_ledger_db(root)
    if not db_path:
        raise HTTPException(404, "No ledger database found")
