...
def ledger_analytics() -> dict[str, Any]:
    ...
    try:
        ...
        streams = [{'stream': r['stream'], 'count': r['c']} for r in conn.execute('SELECT stream, COUNT(*) AS c FROM ledger_entries GROUP BY stream ORDER BY c DESC LIMIT 20')]
        event_types = [{'event_type': r['event_type'], 'count': r['c']} for r in conn.execute('SELECT event_type, COUNT(*) AS c FROM ledger_entries GROUP BY event_type ORDER BY c DESC LIMIT 20')]
        ...
        lam = conn.execute('SELECT MIN(lamport_t) AS mn, MAX(lamport_t) AS mx, COUNT(DISTINCT lamport_t) AS d FROM ledger_entries').fetchone()
        ...
        lamport = {'min': lam_min, 'max': lam_max, 'distinct': lam_distinct, 'expected_span': expected, 'gaps': max(0, expected - lam_distinct) if expected else 0, 'contiguous': bool(expected) and lam_distinct == expected}
        span = conn.execute('SELECT MIN(created_at) AS first, MAX(created_at) AS last FROM ledger_entries').fetchone()
        return {'db_path': db_path.name, 'total_entries': total, 'streams': streams, 'event_types': event_types, 'agents': agents, 'lamport': lamport, 'time_span': {'first': span['first'], 'last': span['last']}}
    finally:
        conn.close()
...
def ledger_search(q: str=Query(..., min_length=1), limit: int=Query(15, ge=1, le=100)) -> dict[str, Any]:
    ...
    try:
        ...
        rows = conn.execute('SELECT seq, stream, entity_id, event_type, cortex_taint, payload_json, created_at FROM ledger_entries ORDER BY seq DESC LIMIT ?', (_SEARCH_SCAN_CAP,)).fetchall()
        docs: list[tuple[int, list[str], sqlite3.Row]] = []
        df: Counter[str] = Counter()
        total_len = 0
        for r in rows:
            text = ' '.join((str(r[k] or '') for k in ('payload_json', 'cortex_taint', 'event_type', 'stream', 'entity_id')))
            toks = _tokenize(text)
        ...
        for _seq, toks, r in docs:
            ...
            if score > 0:
                ...
                scored.append({'seq': r['seq'], 'stream': r['stream'], 'entity_id': r['entity_id'], 'event_type': r['event_type'], 'cortex_taint': r['cortex_taint'], 'created_at': r['created_at'], 'score': round(score, 4), 'snippet': payload[:180]})
        scored.sort(key=lambda d: d['score'], reverse=True)
        return {'query': q, 'terms': q_terms, 'results': scored[:limit], 'corpus_size': scanned, 'scanned': scanned, 'total': total_rows, 'truncated': total_rows > scanned, 'method': 'Okapi BM25 (léxico, no neuronal)'}
    finally:
        conn.close()