...
def verify_chain(db_path: str | Path) -> dict[str, Any]:
    result: dict[str, Any] = {'valid': True, 'total_entries': 0, 'verified_entries': 0, 'broken_at': None, 'error': None, 'entries': []}
    try:
    ...
    try:
        ...
        for row in rows:
            ...
            seq = row_dict['seq']
            entry_status: dict[str, Any] = {'seq': seq, 'event_id': row_dict['event_id'], 'stream': row_dict['stream'], 'lamport_t': row_dict['lamport_t'], 'entry_hash': row_dict['entry_hash'][:16] + '...', 'valid': True, 'errors': []}
            ...
            computed = _compute_entry_hash(event_id=row_dict['event_id'], stream=row_dict['stream'], entity_id=row_dict['entity_id'], event_type=row_dict['event_type'], payload_json=row_dict['payload_json'], source_db=row_dict['source_db'], source_table=row_dict['source_table'], source_pk=row_dict['source_pk'], cortex_taint=row_dict['cortex_taint'], lamport_t=row_dict['lamport_t'], prev_hash=row_dict['prev_hash'], created_at=row_dict['created_at'])
