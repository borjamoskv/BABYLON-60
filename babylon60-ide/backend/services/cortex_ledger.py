...
def _row_to_event(row: sqlite3.Row) -> dict[str, Any]:
    ...
    if row['metadata']:
        try:
            ...
            metadata = {}
    return {'seq': row['seq'], 'event_id': row['event_id'], 'parent_hash': row['parent_hash'], 'current_hash': row['current_hash'], 'event_type': row['event_type'], 'entity_ref': row['entity_ref'], 'payload': payload, 'metadata': metadata, 'created_at': row['created_at']}

def append_event(project_root: Path, event_type: str, entity_ref: str, payload: dict[str, Any], metadata: dict[str, Any] | None=None) -> dict[str, Any]:
    conn = _connect(_ledger_path(project_root))
    try:
        conn.executescript(DDL)
        ...
        try:
            ...
            conn.execute('INSERT INTO cortex_events (event_id, parent_hash, current_hash, event_type, entity_ref, payload, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', (event_id, parent_hash, current_hash, event_type, entity_ref, payload_c, _canonical(metadata) if metadata else None, created_at))
            conn.execute('COMMIT')
...
def list_events(project_root: Path, limit: int=100, offset: int=0, event_type: str | None=None) -> dict[str, Any]:
    ...
    try:
        conn.executescript(DDL)
        if event_type:
            total = conn.execute('SELECT COUNT(*) AS c FROM cortex_events WHERE event_type = ?', (event_type,)).fetchone()['c']
            rows = conn.execute('SELECT * FROM cortex_events WHERE event_type = ? ORDER BY seq DESC LIMIT ? OFFSET ?', (event_type, limit, offset)).fetchall()
        else:
            total = conn.execute('SELECT COUNT(*) AS c FROM cortex_events').fetchone()['c']
            rows = conn.execute('SELECT * FROM cortex_events ORDER BY seq DESC LIMIT ? OFFSET ?', (limit, offset)).fetchall()
...
def verify_chain(project_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {'valid': broken_at is None, 'total_entries': len(rows), 'verified_entries': verified, 'broken_at': broken_at}
    finally:
        conn.close()