...
def query_ledger(query_type, param):
    ...
    try:
        ...
        if query_type == 'search':
            ...
            row = cursor.fetchone()
            results = {'class': row[0], 'module': row[1], 'definition': row[2]} if row else {'error': 'Symbol not found'}
        else:
...
def main():
    if len(sys.argv) < 3:
        logging.info(json.dumps({'error': 'Usage: mcp_symbol_helper.py [search|get_details] [param]'}))
        sys.exit(1)
    command = sys.argv[1]
    argument = sys.argv[2]
