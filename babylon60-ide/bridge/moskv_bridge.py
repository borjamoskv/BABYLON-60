...
def connect(db: Path) -> sqlite3.Connection:
    ...
    try:
        ...
        try:
            ...
            conn.close()
            raise SystemExit("MOSKV BRIDGE: 'disk I/O error' — este sistema de ficheros no soporta escritura SQLite (montaje de red/FUSE, p.ej. el bridge de Cowork). Ejecuta el bridge desde tu disco LOCAL real (tu Terminal / el proceso del kernel), donde vive babylon60_ide.db de verdad.") from e
    return conn
...
def append(db: Path, event_type: str, entity_ref: str, payload: dict, metadata: dict | None=None) -> dict:
    ...
    try:
        ...
        try:
            ...
            conn.execute('INSERT INTO cortex_events (event_id, parent_hash, current_hash, event_type, entity_ref, payload, metadata, created_at) VALUES (?,?,?,?,?,?,?,?)', (eid, parent, h, event_type, entity_ref, pc, canonical(metadata) if metadata else None, created))
            conn.execute('COMMIT')
...
def cmd_status(db: Path, repo: Path, agent: str, task: str) -> None:
    ev = append(db, STATUS, 'bridge/agent', {'agent': agent, 'task': task, **git_ctx(repo)}, {'taint': f'{agent}:bridge-status'})
    logging.info(f"⚡ STATUS sellado {ev.get('seq_hash')} — {agent}: {task}")

def cmd_handoff(db: Path, repo: Path, src: str, dst: str, task: str, context: str) -> None:
    ev = append(db, HANDOFF, 'bridge/handoff', {'from': src, 'to': dst, 'task': task, 'context': context, **git_ctx(repo)}, {'taint': f'{src}:handoff'})
    logging.info(f"⚡ HANDOFF sellado {ev.get('seq_hash')} — {src} → {dst}: {task}")
    logging.info(f"   event_id (para ack): {ev.get('event_id')}")
...
def cmd_inbox(db: Path, agent: str) -> None:
    ...
    for e in reversed(evs):
        if e['event_type'] == HANDOFF and e['payload'].get('to') == agent and (e['event_id'] not in acked):
            pend.append(e)
    if not pend:
        logging.info(f"inbox de '{agent}' vacío")
        return
    for e in reversed(pend):
        p = e['payload']
        logging.info(f"◈ {e['event_id'][:8]}… de {p.get('from')}: {p.get('task')}")
        if p.get('context'):
            logging.info(f"    ctx: {p['context'][:120]}")
...
def cmd_ack(db: Path, agent: str, handoff_id: str) -> None:
    ...
    logging.info(f"✓ ACK sellado {ev.get('seq_hash')}")
    ...
    for e in events(db):
        if e['event_type'] == STATUS:
            ...
            if a and a not in seen:
                seen[a] = e
    if not seen:
        logging.info('sin peers en el bus')
        return
    now = int(time.time() * 1000)
    for a, e in seen.items():
        p = e['payload']
        mins = (now - e['created_at']) // 60000
        logging.info(f"● {a} — {p.get('task', '?')} · {p.get('branch', '?')}@{p.get('head', '?')} · hace {mins} min")

def cmd_verify(db: Path) -> None:
    prev = ZERO
    rows = list(reversed(events(db, limit=100000)))
    ok = 0
    for e in rows:
        ...
        if e['parent_hash'] != prev or h != e['current_hash']:
            logging.info(f"✗ CADENA ROTA en seq {e['seq']}")
            sys.exit(1)
        ok += 1
        prev = e['current_hash']
    logging.info(f'✓ cadena íntegra: {ok}/{len(rows)} eventos (escritores mixtos incluidos)')
