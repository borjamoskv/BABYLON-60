 ...
 * v1.5.0 · C5-REAL · Author: Borja Moskv (borjamoskv)
 *
...
const SPINE_ROUTES = [
  ...
  { id: 'editor',    icon: '▤',  label: 'Editor',     tip: 'Zero Context-Switching AST Editor  ⌘9' },
];
...
function setupKeyboard() {
  const routeKeys = { '1': 'ledger', '2': 'databases', '3': 'query', '4': 'swarm', '5': 'canvas', '6': 'sentinel', '7': 'analytics', '8': 'ideas', '9': 'editor' };
...
async function renderQueryPage(container) {
  ...
  const runQuery = async () => {
    ...
    try {
      ...
      if (resultMeta) resultMeta.textContent = `${result.row_count ?? rows.length} rows${result.truncated ? ' (truncado a 1000 — afina la consulta)' : ''} · ${result.elapsed_ms ?? '—'}ms · ${result.database}`;
      if (resultBody) {
        if (rows.length === 0) {
...
async function renderSwarmAgents() {
  ...
  // ── Peers REALES del bus de agentes (CortexLedger compartido) ──
  // ANTIGRAVITY/moskv-1-APEX escribe vía bridge/moskv_bridge.py (SQLite
  // directo); este IDE los lee de la misma cadena. Sin teatro.
  let peerCards = [];
  let inboxCard = [];
  try {
    const [pr, ib] = await Promise.all([
      get('/api/bridge/peers'),
      get('/api/bridge/inbox?agent=claude-cowork'),
    ]);
    const now = Date.now();
    peerCards = (pr.peers || []).map(p => {
      const mins = Math.floor((now - p.last_seen) / 60000);
      const fresh = mins < 10;
      return {
        name: `◈ ${p.agent}`,
        status: fresh ? 'running' : 'idle',
        task: `${p.task}${p.branch ? ` · ${p.branch}@${p.head ?? '—'}` : ''} · hace ${mins < 1 ? '<1' : mins} min · ${p.hash}`,
        progress: fresh ? 100 : 0,
      };
    });
    if ((ib.pending || []).length > 0) {
      inboxCard = [{
        name: '📥 Handoffs pendientes',
        status: 'running',
        task: ib.pending.map(h => `de ${h.from}: ${h.task}`).join(' · ').slice(0, 140),
        progress: 100,
      }];
    }
  } catch { /* bus offline → solo agentes internos */ }

  if (peerCards.length === 0) {
    peerCards = [{ name: '◈ (bus vacío)', status: 'idle', task: 'Ningún agente ha hecho ping — tu kernel puede unirse con bridge/moskv_bridge.py status', progress: 0 }];
  }
