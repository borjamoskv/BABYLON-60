...
Ninguna regla de validación semántica pasa al árbol principal sin una demostración empírica de falsabilidad. Todo estado s \in S declarado seguro debe poseer un test negativo explícito en `autodidact_falsification_test.py` que pruebe que el *Commit Gate* rechaza un artefacto malicioso. Su medida explícita es el coeficiente ρ (fiabilidad de A.1.4) medido sobre una muestra N \ge 100 con intervalo de confianza por bootstrap. Este valor ρ DEBE ser emitido en cada corrida. Un INV-3 sin ρ empírico publicado no se considera un invariante válido, sino una suposición.
