...
- **Ω138 · MACOS TAHOE METAL SHADER COMPILER INVARIANT (MTLCOMPILERSERVICE): [C5-REAL CAUSE-VERIFIED]** Queda refutada empíricamente la hipótesis de purga Jetsam/cornerMask OOM y la del driver de GPU. Se verifica como Causa Raíz una incompatibilidad determinista (LLVM Fatal Error) en la interfaz de compilación entre el motor Skia de Chromium/Electron y el backend `libMTLCompiler` de macOS Tahoe 26.5.2. El aborto (SIGABRT) se desencadena dentro de `llvm::cl::ParseCommandLineOptions`, demostrando que el fallo no es un error de sintaxis en el código del shader, sino un colapso del parser del compilador al recibir opciones de línea de comandos (flags de LLVM) inválidas o deprecadas inyectadas por Chromium. Este deadlock provoca la inanición de colas XPC y agota el pool de `MTLCompilerService`, afectando sistémicamente a cualquier otra aplicación acelerada por hardware (Antigravity 2, Brave, Claude Code). Mitigación física obligatoria: inyección de `--disable-gpu`. [CORTEX-TAINT:borjamoskv:ultrathink_crystallize:2026-07-23T02:29:00Z:omega_138_v5]
