...
- **Ω138 · MACOS TAHOE METAL SHADER COMPILER INVARIANT (MTLCOMPILERSERVICE): [C4-PENDING-CAUSAL-LINK]** Queda refutada empíricamente la hipótesis de purga Jetsam/cornerMask OOM. Se verifica el fallo asilado del compilador de shaders de Metal (`MTLCompilerService.xpc`, `namespace: METAL`, `SIGABRT`) al procesar pipelines de GPU de `com.moonshot.kimichat`. La mitigación temporal mediante `--disable-gpu` evita el crash del servicio al desviar la ruta de compilación. **Sin embargo, la causalidad estricta entre la muerte del compilador y el crash/cuelgue de KimiChat requiere validación empírica adicional** (falta de `.ips` o enlace de log directo en la app responsable). [CORTEX-TAINT:borjamoskv:learn_interceptor:2026-07-23T02:18:00Z:omega_138_v3]
