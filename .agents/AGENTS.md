...
- **Ω138 · MACOS TAHOE METAL SHADER COMPILER INVARIANT (MTLCOMPILERSERVICE): [C5-REAL CAUSE-VERIFIED]** Queda refutada empíricamente la hipótesis de purga Jetsam/cornerMask OOM y la del driver de GPU. Se verifica como Causa Raíz una incompatibilidad determinista (LLVM Fatal Error) en la interfaz de compilación entre el motor Skia de Chromium/Electron y el backend `libMTLCompiler` de macOS Tahoe 26.5.2. El aborto (SIGABRT) se desencadena dentro de `llvm::cl::ParseCommandLineOptions`, demostrando que el fallo no es un error de sintaxis en el código del shader, sino un colapso del parser del compilador al recibir opciones de línea de comandos (flags de LLVM) inválidas o deprecadas inyectadas por Chromium. Esto interrumpe la tubería IPC GPU, congelando al renderer. Exactamente 41 segundos tras la muerte del compilador, el mecanismo Watchdog de Chromium (Browser Process) asesina a su propio renderer inestable con un `SIGKILL` silencioso (`exitCode=9`). La ausencia de volcados `.ips`/`.dmp` no es un vacío epistémico, es una consecuencia arquitectónica: Crashpad no intercepta SIGKILL. Mitigación física obligatoria: inyección de `--disable-gpu`. [CORTEX-TAINT:borjamoskv:ultrathink_crystallize:2026-07-23T02:29:00Z:omega_138_v6]
