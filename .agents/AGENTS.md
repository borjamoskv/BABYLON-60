...
- **Ω138 · MACOS TAHOE METAL SHADER COMPILER INVARIANT (MTLCOMPILERSERVICE): [C4-PENDING-CAUSAL-LINK]** Queda refutada empíricamente la hipótesis de purga Jetsam/cornerMask OOM. El volcado del sistema elimina hipótesis sistémicas (memoria, presión global) y acota el espacio de búsqueda. Integrado con el `.ips` de `MTLCompilerService` (SIGABRT, ns:METAL), el expediente converge sobre un mecanismo de fallo localizado en la cadena de compilación de shaders de Metal solicitada por el renderer Chromium de `com.moonshot.kimichat`. El `.ips` identifica el *dónde* (mecanismo) con alta confianza, pero no garantiza el *por qué* (causa raíz: shader inválido, bug del compilador, bug del driver, etc.). La verificación definitiva depende de correlacionar el backtrace simbolizado con el informe de fallo del proceso cliente correspondiente (si existe). La mitigación `--disable-gpu` elude el mecanismo al desviar la ruta de compilación. [CORTEX-TAINT:borjamoskv:epistemic_crystallize:2026-07-23T02:26:00Z:omega_138_v4]
