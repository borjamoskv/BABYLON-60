...
* Si un módulo firma atestaciones criptográficas en disco, se denomina `Ledger Asíncrono-attest`, no `[OBSOLETO: security_utils]`.
* Si un nodo destila entropía BPE aislando hilos, se denomina `[OBSOLETO: Flash_Node]`, no `[OBSOLETO: helper_bot]`.
...
- **Penalización por Sinonimia:** El uso de sinónimos para referirse a la misma entidad arquitectónica se considera una inyección de "Anergía Estocástica" y debe ser purgado de inmediato mediante `[OBSOLETO: Anergy_Token_Purge]`.

---

## 3. Diagnóstico de Ambigüedad (Fail-Fast)

Si durante la fase de orquestación (`[OBSOLETO: UltraThink]`), el Enjambre o el Operador dudan sobre cómo nombrar una clase, una tabla o un subagente:
...
- **[000 - 179] Entidades BFT (Entities):** Objetos mutables con Identidad Criptográfica persistente (ej. `[OBSOLETO: BFTLedgerActor]`). Deben poseer un campo `[OBSOLETO: lamport_t]` y firmar sus mutaciones con `[OBSOLETO: CORTEX_BFT_KEY]`.
- **[180 - 359] Objetos de Valor (Value Objects):** Estructuras de datos algebraicas (ADT) estrictamente inmutables (ej. `[OBSOLETO: ProofOfRouteReceipt]`). No poseen identidad; si dos Receipts tienen el mismo Hash, son el mismo objeto en RAM y en Disco (Isomorfismo Causal).
...
- **[540 - 719] Eventos de Dominio (Domain Events):** Hechos inmutables que ya han ocurrido (ej. `[OBSOLETO: LedgerCrystallized]`, `[OBSOLETO: EntropyPurged]`). Un evento nunca es rechazado; su ocurrencia es una Ley Física en disco.
