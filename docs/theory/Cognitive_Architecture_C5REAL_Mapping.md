...
- _Master Ledger BFT (SQLite WAL)._ Cada aserción exige una traza causal `CORTEX-TAINT`.
...
- _MCTS Budget Forcing (Ultrathink Protocol)._ MOSKV-1 inyecta un loop adversario explícito. Si la entropía del problema supera un umbral, escala hacia un subagente paralelo (`invoke_subagent`) o exige un ciclo de iteración profunda, purgado de Green Theater.
...
- _Diversidad de Enjambre (Modulo-3)._ Evaluamos las topologías del problema usando ramas ortogonales. La Tolerancia Bizantina (BFT, $N \ge 3$) fuerza el consenso criptográfico antes de mutar el disco. Las opciones sub-óptimas no se descartan, se asimilan en el cálculo del gradiente de Pareto.
...
- _Ingesta $\rightarrow$ Auditoría $\rightarrow$ Mutación Atómica $\rightarrow$ Verificación._ Cualquier aserción que rompa un invariante durante el _BFT State Loop_ detona un `SIGKILL_State_Purge`. Ninguna mutación se asienta sin falsabilidad empírica sobre el disco.
...
- _Max Exergy Execution._ El Kernel rechaza proporcionar "opciones narrativas" para que el operador decida. Evalúa el espacio latente y ejecuta físicamente la Frontera de Pareto en código comprobado, devolviendo el árbol de decisión ya compilado.
...
- _Cristalización (El comando /itera)._ En lugar de almacenar "prosa", extraemos el invariante físico y lo añadimos a `AGENTS.md` o `SKILL.md`. El aprendizaje exige **modificar el conjunto de reglas**, y olvidar el ruido intermedio para prevenir el _KV-Cache Decay_.
