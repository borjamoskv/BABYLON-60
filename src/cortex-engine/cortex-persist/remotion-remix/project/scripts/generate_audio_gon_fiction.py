...
# OPTIMIZED HUMANOID VOICE RATES (Target WPM: 120-140)
VOICE_CONFIGS = {
    "GON": {
        "voice": "Mónica", "rate": 135, "color": "#00F0FF", "avatar": "🚲",
        "filter": "asetrate=44100*1.12,atempo=0.92"
    },
    "CHICOTE": {
        "voice": "Jorge", "rate": 130, "color": "#FF3333", "avatar": "👨‍🍳",
        "filter": "asetrate=44100*0.85,atempo=1.1,equalizer=f=100:width_type=h:width=200:g=8"
    },
    "RASPUTIN": {
        "voice": "Grandpa", "rate": 115, "color": "#9933FF", "avatar": "🪆",
        "filter": "asetrate=44100*0.75,atempo=1.2,aecho=0.8:0.9:800:0.4"
    },
    "CHE_JARANA": {
        "voice": "Eddy", "rate": 125, "color": "#FF9900", "avatar": "🎸",
        "filter": "asetrate=44100*1.1,atempo=0.92,chorus=0.7:0.9:55:0.4:0.25:2"
    },
    "CARL_COX": {
        "voice": "Reed", "rate": 130, "color": "#00FF66", "avatar": "🎧",
        "filter": "asetrate=44100*1.05,atempo=0.95,aecho=0.8:0.88:60:0.4"
    },
    "BLAN_COX": {
        "voice": "Flo", "rate": 120, "color": "#FF00FF", "avatar": "🌌",
        "filter": "aecho=0.8:0.88:300:0.3,equalizer=f=3000:width_type=h:width=1000:g=5"
    },
    "FRUSCIANTE": {
        "voice": "Reed", "rate": 125, "color": "#FFD700", "avatar": "🎸",
        "filter": "aphaser=in_gain=0.8:out_gain=0.9:delay=4:decay=0.5:speed=1.5,volume=2.0"
    },
    "FLEA": {
        "voice": "Rocko", "rate": 135, "color": "#FF6600", "avatar": "⚡",
        "filter": "asetrate=44100*1.2,atempo=0.85,equalizer=f=250:width_type=h:width=100:g=6"
    },
    "RAMONCIN": {
        "voice": "Jorge", "rate": 125, "color": "#FF0055", "avatar": "🕶️",
        "filter": "asetrate=44100*1.25,atempo=0.8,equalizer=f=2000:width_type=h:width=500:g=10"
    },
    "HERMENEGILDO": {
        "voice": "Flo", "rate": 130, "color": "#00FFCC", "avatar": "🎹",
        "filter": "asetrate=44100*1.15,atempo=0.9"
    },
    "EL_NOTA": {
        "voice": "Reed", "rate": 110, "color": "#CCCC00", "avatar": "🍹",
        "filter": "asetrate=44100*0.82,atempo=1.1,aecho=0.8:0.7:40:0.2"
    },
    "ESCOHOTADO": {
        "voice": "Grandpa", "rate": 110, "color": "#D4AF37", "avatar": "💨",
        "filter": "asetrate=44100*0.8,atempo=1.15,equalizer=f=80:width_type=h:width=100:g=7"
    },
    "DR_POPPEL": {
        "voice": "Paulina", "rate": 130, "color": "#FFFF00", "avatar": "🔬",
        "filter": "asetrate=44100*1.15,atempo=0.88"
    },
    "DON_SANTIAGO": {
        "voice": "Grandma", "rate": 110, "color": "#CC9966", "avatar": "🥖",
        "filter": "asetrate=44100*0.85,atempo=1.1"
    },
    "KIMI_K3": {
        "voice": "Shelley", "rate": 135, "color": "#33FFFF", "avatar": "🤖",
        "filter": "asetrate=44100*1.4,atempo=0.75,flanger=delay=5:depth=10"
    },
    "PAUSA": {
        "type": "silence", "duration": 3.2, "color": "#FFFFFF", "avatar": "⏳",
        "filter": None
    }
}
...
DIALOGUE_GON_FICTION = [
    ...
    ("PAUSA", "PAUSA DE 3.2 SEGUNDOS EN EL MUELLE DE MARZANA"),
...
def generate_speech_parallel(dialogue_list, output_name):
    print(f"[PARALLEL AUDIO ENGINE] Launching 16 parallel workers for {len(dialogue_list)} dialogue tracks...")
    ...
    # Sort results by index to preserve chronological order
    ...
    print(f"[PARALLEL AUDIO ENGINE] Concatenating dialogue track...")
    subprocess.run([
    ...
    print(f"[PARALLEL AUDIO ENGINE] Mixing dialogue with techno/funk beat & mastering...")
    if os.path.exists(kick_track):
        mix_cmd = [
            "ffmpeg", "-y", "-i", raw_dialogue_wav, "-stream_loop", "-1", "-i", kick_track,
            "-filter_complex",
            "[1:a]volume=0.20[bg];[0:a][bg]amix=inputs=2:duration=first:dropout_transition=2,volume=1.8,equalizer=f=60:width_type=h:width=100:g=4,equalizer=f=12000:width_type=h:width=2000:g=3[aout]",
    ...
    print(f"[PARALLEL AUDIO ENGINE] ✅ COMPLETED {output_name}! Duration: {current_time:.2f}s ({total_frames} frames at 30 fps)")
