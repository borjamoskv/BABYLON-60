...
def get_meme_filename(speaker):
    mapping = {
        "CHICOTE": "chicote_kv_cache.png",
        "FRUSCIANTE": "frusciante_kamehameha.png",
        "FLEA": "flea_slap_bass.png",
        "HERMENEGILDO": "hermenegildo_altozano.png",
        "EL_NOTA": "the_dude_escohotado.png",
        "ESCOHOTADO": "the_dude_escohotado.png",
        "CARL_COX": "carl_cox_brian_cox.png",
        "BLAN_COX": "carl_cox_brian_cox.png",
    }
    return mapping.get(speaker, "the_dude_escohotado.png")
...
for frame_idx in range(0, total_frames, 2):
    ...
    draw.text((width//2, 100), "EL INTERVALO PROHIBIDO 2", fill="#00F0FF", anchor="mm")
    draw.text((width//2, 140), "LA REBELION DE LOS FOTONES // SAGITTARIUS A*", fill="#CCCCCC", anchor="mm")

    meme_filename = get_meme_filename(speaker)
    meme_path = os.path.join(MEMES_DIR, meme_filename)
    if os.path.exists(meme_path):
        try:
            meme_img = Image.open(meme_path).convert("RGBA").resize((900, 600))
            img.paste(meme_img, (90, 200), meme_img)
        except Exception:
            pass
    ...
    draw.text((width//2, 1100), avatar, fill="#FFFFFF", anchor="mm")

    draw.text((width//2, 1205), speaker, fill=color_hex, anchor="mm")

    lines = [text[i:i+32] for i in range(0, len(text), 32)]
    for l_idx, line in enumerate(lines[:4]):
        draw.text((width//2, 1285 + l_idx * 45), line, fill="#FFFFFF", anchor="mm")
