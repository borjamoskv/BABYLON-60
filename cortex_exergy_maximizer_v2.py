...
def maximize_exergy():
    ...
    if os.path.exists("strike-rs"):
        subprocess.run(["cargo", "clippy", "--fix", "--allow-dirty", "--allow-no-vcs", "--workspace"], cwd="strike-rs", check=False)
    ...
    result = subprocess.run(["git", "commit", "-m", "refactor(core): C5-REAL AST Pareto dominance pass"], capture_output=True, text=True)
