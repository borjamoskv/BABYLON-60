...
ADVERSARIAL_PATTERNS: List[Tuple[re.Pattern[str], str]] = [
    (re.compile(r"-----BEGIN\s+(?:[A-Z0-9-]+\s+)?PRIVATE\s+KEY-----", re.IGNORECASE), "Fuga de Clave Privada detectada"),
    (re.compile(r"(AWS_SECRET_ACCESS_KEY|aws_secret_key)\s*=\s*['\"][A-Za-z0-9/+=]{10,}['\"]", re.IGNORECASE), "Fuga de Secretos de AWS detectada"),
    (re.compile(r"sk-[A-Za-z0-9_-]{20,}"), "Fuga de Clave API de OpenAI/LLM detectada"),
    (re.compile(r"\b(password|secret|passwd|api_key)\s*=\s*['\"][^'\"]+['\"]", re.IGNORECASE), "Hardcoding de Credenciales detectado"),
    (re.compile(r"except\s*:\s*(\n\+?\s*)?(pass|\.\.\.)", re.MULTILINE), "Captura nula de excepciones (Bare except pass) detectada"),
    (re.compile(r"\b(eval|exec)\s*\("), "Inyección de ejecución dinámica no confiable (eval/exec) detectada"),
