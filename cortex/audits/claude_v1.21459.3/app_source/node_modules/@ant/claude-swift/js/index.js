...
class SwiftAddon extends EventEmitter {
  constructor() {
    ...
    // Nest-only — undefined in public builds (#if NEST_BUILD compiles it out).
    this.tearOffHalo = native.tearOffHalo;
    this.sessionHalo = native.sessionHalo;
    this.wakeScheduler = native.wakeScheduler;
    this.updater = native.updater;
    this.trayUsage = native.trayUsage;
    this.watchRecorder = native.watchRecorder;
    this.memoryPressure = native.memoryPressure;
    ...
    if (fs.existsSync(computerUseNode)) {
      ...
      // Swift binding — notifications, VM, all of it. Fail soft:
