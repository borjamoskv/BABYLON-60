...
class TransducerResult:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'features': self.features.as_dict(), 'assessment': self.assessment.as_dict(), 'ledger_entry': self.ledger_entry.as_dict(), 'history': self.history.as_dict() if self.history is not None else None}
...
class Transducer:
    ...
    def score(self, nct_id: str, with_history: bool=True) -> TransducerResult:
        ...
        if with_history:
            try:
                ...
                history = None
        payload: dict[str, Any] = {'nct_id': features.nct_id, 'model_version': assessment.model_version, 'mode': assessment.mode, 'score': assessment.score, 'tier': assessment.tier, 'raw_score': assessment.raw_score, 'expected_amendments': assessment.expected_amendments, 'features': features.as_dict(), 'fired_rules': [r.as_dict() for r in assessment.fired_rules], 'contributions': list(assessment.contributions)}
        ...
        return CopilotResult(features=features, assessment=assessment, ledger_entry=entry, history=history, module_risks=module_risks)