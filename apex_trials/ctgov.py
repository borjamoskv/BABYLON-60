...
SUBSTANTIVE_MODULES: frozenset[str] = frozenset({'Study Design', 'Outcome Measures', 'Arms and Interventions', 'Eligibility', 'Conditions', 'Study Description'})
ADMINISTRATIVE_MODULES: frozenset[str] = frozenset({'Study Status', 'Contacts/Locations', 'Study Identification', 'Sponsor/Collaborators', 'More Information', 'References', 'Document Section', 'Oversight', 'Study Documents', 'Recruitment Status'})
...
class CtGovClient:
    ...
    def search(self, condition: str | None=None, term: str | None=None, phase: str | None=None, status: str | None=None, page_size: int=20) -> list[dict[str, Any]]:
...
class AmendmentHistory:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'nct_id': self.nct_id, 'n_versions': self.n_versions, 'n_substantive_amendments': self.n_substantive, 'n_administrative_updates': self.n_administrative, 'first_date': self.first_date, 'last_date': self.last_date, 'substantive_dates': list(self.substantive_dates), 'unknown_labels': list(self.unknown_labels)}
...
def classify_history(nct_id: str, history: dict[str, Any]) -> AmendmentHistory:
    ...
    for change in changes:
        ...
        if is_substantive:
            ...
            n_administrative += 1
    return AmendmentHistory(nct_id=nct_id, n_versions=n_versions, n_substantive=n_substantive, n_administrative=n_administrative, first_date=min(dates) if dates else None, last_date=max(dates) if dates else None, substantive_dates=tuple(substantive_dates), unknown_labels=tuple(sorted(unknown)))

def _normalize_phase(phase: str) -> str:
    p = phase.strip().upper().replace(' ', '')
    mapping = {'1': 'PHASE1', '2': 'PHASE2', '3': 'PHASE3', '4': 'PHASE4', 'PHASE1': 'PHASE1', 'PHASE2': 'PHASE2', 'PHASE3': 'PHASE3', 'PHASE4': 'PHASE4', 'I': 'PHASE1', 'II': 'PHASE2', 'III': 'PHASE3', 'IV': 'PHASE4'}
