...
Inference is fully deterministic and dependency-free (Option B: pure Python TF-IDF).
Same protocol -> same probabilities, byte-for-byte.
...
def predict_module_risks(
    ...
    Uses structured features plus optional free text (eligibility and brief summary)
    for deterministic, dependency-free text prediction.
    """
    if _MODELS is None:
        return ()
