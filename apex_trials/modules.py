...
class ModuleRisk:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'module': self.module, 'label': self.label, 'probability': self.probability, 'base_rate': self.base_rate, 'lift': self.lift}
...
def _feature_map(f: StudyFeatures) -> dict[str, float]:
    ...
    return {'n_eligibility_criteria': float(f.n_eligibility_criteria), 'n_endpoints': float(f.n_primary_endpoints + f.n_secondary_endpoints), 'n_arms': float(f.n_arms), 'log_enrollment': math.log1p(f.enrollment), 'n_countries': float(f.n_countries), 'phase_ord': float(phase_ord), 'is_crossover': 1.0 if 'CROSSOVER' in f.intervention_model.upper() else 0.0, 'is_factorial': 1.0 if 'FACTORIAL' in f.intervention_model.upper() else 0.0, 'high_masking': 1.0 if f.masking.upper() in ('TRIPLE', 'QUADRUPLE') else 0.0, 'is_oncology': 1.0 if f.is_oncology else 0.0, 'is_rare': 1.0 if f.is_rare_disease else 0.0, 'has_dmc': 1.0 if f.has_dmc else 0.0, 'is_fda_regulated': 1.0 if f.is_fda_regulated else 0.0, 'log_summary_words': math.log1p(f.brief_summary_words), 'n_conditions': float(f.n_conditions), 'n_interventions': float(f.n_interventions)}
...
def predict_module_risks(features: StudyFeatures, eligibility_text: str='', brief_summary_text: str='') -> tuple[ModuleRisk, ...]:
    if _MODELS is None:
        return ()
    ...
    for key, m in _MODELS['modules'].items():
        ...
        risks.append(ModuleRisk(module=key, label=m['label'], probability=round(p, 4), base_rate=round(base, 4), lift=round(p / base, 3) if base > 0 else 0.0))
