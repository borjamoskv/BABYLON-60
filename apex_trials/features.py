...
_NUMBERED = re.compile('^\\s*(\\d+)\\s*[.)]\\s+\\S')
_BULLET = re.compile('^\\s*[-*•]\\s+\\S')
_ONCOLOGY_TERMS = ('cancer', 'carcinoma', 'tumor', 'tumour', 'oncolog', 'neoplasm', 'leukemia', 'leukaemia', 'lymphoma', 'melanoma', 'sarcoma', 'myeloma', 'glioma', 'metasta')
_RARE_TERMS = ('rare', 'orphan', 'cystic fibrosis', 'duchenne', 'amyloidosis', 'hemophilia', 'haemophilia', 'gaucher', 'pompe', 'huntington', 'als ', 'sma ', 'spinal muscular')
...
def extract_features(study: dict[str, Any]) -> StudyFeatures:
    ...
    summary_words = len(re.findall('\\w+', brief_summary))
    ...
    return StudyFeatures(nct_id=ident.get('nctId', '?'), brief_title=ident.get('briefTitle', ''), phase=_max_phase(design.get('phases', [])), study_type=design.get('studyType', '?'), n_eligibility_criteria=n_total, n_inclusion=n_inc, n_exclusion=n_exc, n_primary_endpoints=len(outcomes.get('primaryOutcomes', [])), n_secondary_endpoints=len(outcomes.get('secondaryOutcomes', [])), n_arms=len(arms_mod.get('armGroups', [])), enrollment=enrollment_val, n_sites=sites_val, n_countries=len(countries), allocation=design_info.get('allocation', 'NA'), intervention_model=design_info.get('interventionModel', 'NA'), masking=design_info.get('maskingInfo', {}).get('masking', 'NA'), is_oncology=is_onc, is_rare_disease=is_rare, therapeutic_area=area, has_dmc=has_dmc, is_fda_regulated=is_fda, brief_summary_words=summary_words, n_conditions=max(1, len(conditions)), n_interventions=max(1, n_interventions), enrollment_velocity=velocity)