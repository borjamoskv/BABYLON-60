...
def compute_entry_hash(prev_hash: str, entry_id: str, causal_taint: str, lamport_t: int, agent_id: str, payload: dict[str, Any]) -> str:
...
class LedgerEntry:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'seq': self.seq, 'id': self.id, 'prev_hash': self.prev_hash, 'entry_hash': self.entry_hash, 'payload': self.payload, 'causal_taint': self.causal_taint, 'lamport_t': self.lamport_t, 'agent_id': self.agent_id, 'created_at': self.created_at}
...
class ChainVerification:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'valid': self.valid, 'entries': self.entries, 'broken_at': self.broken_at, 'reason': self.reason}
...
class AmendmentLedger:
    ...
    def append(self, payload: dict[str, Any], causal_taint: str, agent_id: str=DEFAULT_AGENT_ID) -> LedgerEntry:
        ...
        async def _run() -> LedgerEntry:
            ...
            try:
                ...
                event = LedgerEvent(stream='apex_trials', entity_id=entity_id, event_type='risk_score', payload=payload, cortex_taint=causal_taint, source_db='apex_trials.db', source_table='assessments', source_pk=entity_id, created_at=created_at)
                res = await actor.append(event)
                async with aiosqlite.connect(self.db_path) as db:
                    db.row_factory = aiosqlite.Row
                    async with db.execute('SELECT * FROM ledger_entries WHERE event_id = ?', (res['event_id'],)) as cursor:
    ...
    def verify_chain(self) -> ChainVerification:
        ...
        async def _run() -> ChainVerification:
            ...
            try:
                ...
                entries = await self._async_count()
                broken_at = None
                match = re.search('at seq (\\d+)', reason)
                if match:
    ...
    def _row_to_entry(row: sqlite3.Row | aiosqlite.Row) -> LedgerEntry:
        ...
        return LedgerEntry(seq=int(row['seq']), id=str(row['event_id']), prev_hash=str(row['prev_hash']), entry_hash=str(row['entry_hash']), payload=json.loads(payload_json), causal_taint=str(row['cortex_taint']), lamport_t=int(row['lamport_t']), agent_id=str(row['entity_id']), created_at=str(row['created_at']))
...
class BabylonBFTLedgerAdapter:
    ...
    def append(self, payload: dict[str, Any], causal_taint: str, agent_id: str=DEFAULT_AGENT_ID) -> Any:
        try:
        ...
        entity_id = str(payload.get('nct_id', uuid.uuid4()))
        event = LedgerEvent(stream='apex_trials', entity_id=entity_id, event_type='risk_score', payload=payload, cortex_taint=causal_taint, source_db='apex_trials.db', source_table='assessments', source_pk=entity_id)
