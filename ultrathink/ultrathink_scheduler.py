...
async def propose_with_backoff(engine: ConsensusEngineStub, payload: bytes, task_id: int) -> bool:
    ...
    if os.path.exists(map_file):
        with open(map_file, encoding="utf-8") as f:
            for line in f:
                ...
                if match:
                    payloads.append(f"EXEC_VECTOR:{match.group(1)}".encode())
    if not payloads:
        log.warning("No mapped vectors found, falling back to dummy tasks.")
        payloads = [f"task-{i}".encode() for i in range(1000)]
    total_tasks: int = len(payloads)
    successes: int = 0
    t0_ns: int = time.monotonic_ns()
    tasks: list[asyncio.Task[bool]] = [
        asyncio.create_task(propose_with_backoff(engine, payloads[i], i)) for i in range(total_tasks)
    ]
