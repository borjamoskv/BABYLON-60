...
def verify_ed25519(public_key_b64: str, signature_b64: str, message: bytes) -> bool:
    if not NACL_AVAILABLE:
        logging.info('ERROR: PyNaCl not installed. Cannot verify Ed25519 signatures.')
        return False
    try:
        ...
        logging.info(f'Signature decoding error: {e}')
        return False
...
def verify_receipt(receipt_path: str) -> dict[str, Any]:
    ...
    if 'payload' not in receipt or 'payload_hash' not in receipt or 'signature' not in receipt:
        logging.info('ERROR: Receipt does not conform to v0.2 structure (payload, payload_hash, signature).')
        sys.exit(1)
    ...
    if sig_block.get('algorithm') == 'Ed25519':
        ...
        logging.info(f"ERROR: Algorithm {sig_block.get('algorithm')} is not implemented. Failing closed.")
        sig_valid = False
    report = {'receipt_id': receipt.get('receipt_id', 'unknown'), 'schema': receipt.get('schema', 'unknown'), 'payload_hash_valid': hash_valid, 'hash_primitive': hash_primitive, 'signature_valid': sig_valid, 'status': 'verified' if hash_valid and sig_valid else 'invalid'}
