...
def execute_pipeline(data_path: str | None=None, falsifiability_threshold: float=40.0) -> None:
    ...
    assert delta > falsifiability_threshold, f'FALSABILIDAD REFUTADA: El colapso del {delta:.1f}% es menor al umbral {falsifiability_threshold}%.'
    logger.info('VERIFICACIÓN C5-REAL EXITOSA. Hipótesis apta para In-Vitro.')

def main() -> None:
    if _ONCO_IMPORT_ERROR is not None:
        logging.info(f"🔴 [FATAL] cortex-onco requiere el extra 'onco' (numpy, networkx, pandas).\n    Instalar con: pip install cortex-persist[onco]\n    Error subyacente: {_ONCO_IMPORT_ERROR}", file=sys.stderr)
        sys.exit(1)
