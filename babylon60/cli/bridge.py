...
def handle_skill_cli(args: argparse.Namespace) -> None:
    ...
    if args.skill_action == 'record':
        ...
        recorder.start()
        logging.info(f"🟢 [SKILL-RECORDER] Iniciando grabación de sesión C5-REAL: '{args.name}' (Session ID: {recorder.session_id})")
        recorder.record_event(event_type=InteractionType.NAVIGATE, selector='window', value=args.target_url or 'https://babylon60.local')
        recorder.record_event(event_type=InteractionType.CLICK, selector='#main-action-btn', target_text='Ejecutar')
        session = recorder.stop()
        session_file = storage_dir / f'session_{session.session_id}.json'
        with open(session_file, 'w', encoding='utf-8') as f:
            json.dump(session.to_dict(), f, indent=2)
        logging.info(f'💾 [SKILL-RECORDER] Sesión guardada en {session_file}')
        logging.info(f'   Eventos capturados: {len(session.events)} | Hash: {session.compute_hash()[:16]}...')
    elif args.skill_action == 'compile':
        session_file = storage_dir / f'session_{args.session_id}.json'
        if not session_file.exists():
            logging.info(f'🔴 [FATAL] Sesión {args.session_id} no encontrada en {storage_dir}', file=sys.stderr)
            sys.exit(1)
        ...
        logging.info(f"🟢 [SKILL-COMPILER] Skill compilada con éxito: '{skill.metadata.name}' (ID: {skill.metadata.skill_id})")
        logging.info(f'   AST Nodes: {len(skill.ast_nodes)} | Digest: {skill.compute_digest()[:16]}...')
    elif args.skill_action == 'replay':
        skill_file = storage_dir / f'skill_{args.skill_id}.json'
        if not skill_file.exists():
            logging.info(f'🔴 [FATAL] Skill {args.skill_id} no encontrada en {storage_dir}', file=sys.stderr)
            sys.exit(1)
        ...
        logging.info(f'🟢 [SKILL-REPLAY] Ejecución finalizada: Passed={report.passed}')
        logging.info(f'   Pasos exitosos: {report.successful_steps}/{report.total_steps} | Duración: {report.total_duration_ms}ms')
        ...
        logging.info(f'🟢 [SKILL-REGISTRY] {len(session_files)} sesiones grabadas | {len(skill_files)} skills compiladas')
        for sf in skill_files:
            logging.info(f'  • {sf.stem}')

def main() -> None:
    parser = argparse.ArgumentParser(prog='cortex-bridge', description='Ignición del Puente CORTEX y BABYLON-60 Skill Synthesizer Kernel.')
    ...
    if tauri_dir is None:
        logging.info('🔴 [FATAL] No se pudo localizar el root físico C5-REAL (src-tauri/Cargo.toml).\n    cortex-bridge requiere el checkout completo del monorepo BABYLON-60\n    (no funciona desde una instalación aislada `pip install cortex-persist`).\n    Clona el repositorio completo: git clone git@github.com:borjamoskv/BABYLON-60.git', file=sys.stderr)
        sys.exit(1)
    cargo_toml = tauri_dir / 'Cargo.toml'
    logging.info('🟢 [CORTEX-BRIDGE] Transducción iniciada. Ignición C5-REAL del VoidLedger Rust.')
    try:
        bft_key = os.environ.get('CORTEX_BFT_KEY') or os.environ.get('CORTEX_VAULT_KEY')
        if not bft_key:
            logging.info('🟡 [WARNING] CORTEX_BFT_KEY o CORTEX_VAULT_KEY no detectada. El núcleo Rust fallará (Zero Static HMAC Invariant).', file=sys.stderr)
        subprocess.run(['cargo', 'run', '--manifest-path', str(cargo_toml)], check=True)
    except FileNotFoundError:
        logging.info("🔴 [FATAL] 'cargo' no está instalado o no está en PATH. Instala Rust: https://rustup.rs", file=sys.stderr)
        sys.exit(1)
    except subprocess.CalledProcessError as e:
        logging.info(f'🔴 [FATAL] El Puente CORTEX colapsó termodinámicamente. Exit Code: {e.returncode}', file=sys.stderr)
