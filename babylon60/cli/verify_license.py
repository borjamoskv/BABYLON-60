...
def verify_offline_key(key_str: str) -> bool:
    ...
    if verified:
        logging.info(f"[+] VALID LICENSE: Owner='{verified['owner']}', Tier='{verified['tier']}', Expires={verified['expires_at']}")
        return True
    else:
        logging.info('[-] INVALID LICENSE: Signature tampered, key corrupted, or expired.')
        return False

def main():
    if len(sys.argv) < 2:
        logging.info('Usage: python3 -m babylon60.cli.verify_license <SOVEREIGN_LICENSE_KEY>')
        sys.exit(1)
    key_str = sys.argv[1]
