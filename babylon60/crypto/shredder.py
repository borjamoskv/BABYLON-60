...
class CryptoShredder:
    ...
    def is_shredded(self, fact_id: int, tenant_id: str='default') -> bool:
        ...
        cursor = self._conn.execute('SELECT 1 FROM shredded_keys WHERE fact_id = ? AND tenant_id = ?', (fact_id, tenant_id))
        return cursor.fetchone() is not None

    async def is_shredded_async(self, fact_id: int, tenant_id: str='default') -> bool:
        cursor = await __import__('typing').cast(__import__('typing').Any, self._conn).execute('SELECT 1 FROM shredded_keys WHERE fact_id = ? AND tenant_id = ?', (fact_id, tenant_id))
    ...
    async def get_shredded_fact_ids_async(self, tenant_id: str='default') -> set[int]:
        cursor = await __import__('typing').cast(__import__('typing').Any, self._conn).execute('SELECT fact_id FROM shredded_keys WHERE tenant_id = ?', (tenant_id,))
        rows = await cursor.fetchall()
        return {row[0] for row in rows}

    def shred_fact(self, fact_id: int, tenant_id: str='default', reason: str='gdpr_erasure', shredded_by: str | None=None) -> ShredResult:
        if not isinstance(self._conn, sqlite3.Connection):
            raise TypeError('Use shred_fact_async for async connections')
        if self.is_shredded(fact_id, tenant_id):
            return ShredResult(fact_id=fact_id, tenant_id=tenant_id, success=True, reason=reason, was_already_shredded=True)
        try:
            ts = datetime.fromtimestamp(time.time(), tz=timezone.utc).isoformat()
            self._conn.execute('INSERT INTO shredded_keys (fact_id, tenant_id, reason, shredded_by, shredded_at) VALUES (?, ?, ?, ?, ?)', (fact_id, tenant_id, reason, shredded_by, ts))
    ...
    async def shred_fact_async(self, fact_id: int, tenant_id: str='default', reason: str='gdpr_erasure', shredded_by: str | None=None) -> ShredResult:
        if await self.is_shredded_async(fact_id, tenant_id):
            return ShredResult(fact_id=fact_id, tenant_id=tenant_id, success=True, reason=reason, was_already_shredded=True)
        try:
            ts = datetime.fromtimestamp(time.time(), tz=timezone.utc).isoformat()
            await __import__('typing').cast(__import__('typing').Any, self._conn).execute('INSERT INTO shredded_keys (fact_id, tenant_id, reason, shredded_by, shredded_at) VALUES (?, ?, ?, ?, ?)', (fact_id, tenant_id, reason, shredded_by, ts))
            self._invalidate_fact_key(fact_id, tenant_id)
            await __import__('typing').cast(__import__('typing').Any, self._conn).commit()
            logger.info('Crypto-shredded fact #%d (tenant=%s, reason=%s)', fact_id, tenant_id, reason)
            return ShredResult(fact_id=fact_id, tenant_id=tenant_id, success=True, reason=reason)
    ...
    async def shred_by_project(self, project: str, tenant_id: str='default', reason: str='project_erasure', shredded_by: str | None=None) -> ShredBatchResult:
        cursor = await __import__('typing').cast(__import__('typing').Any, self._conn).execute('SELECT id FROM facts WHERE project = ? AND tenant_id = ?', (project, tenant_id))
        rows = await cursor.fetchall()
        fact_ids = [row[0] for row in rows]
        return await self._shred_batch(fact_ids, tenant_id, reason, shredded_by)

    async def _shred_batch(self, fact_ids: list[int], tenant_id: str, reason: str, shredded_by: str | None) -> ShredBatchResult:
    ...
    def audit_shredding(self) -> dict[str, Any]:
        ...
        cursor = self._conn.execute('SELECT COUNT(*), reason, MIN(shredded_at), MAX(shredded_at) FROM shredded_keys GROUP BY reason')
        ...
        return {'total_shredded': total, 'by_reason': reasons, 'compliant': True, 'audit_timestamp': datetime.fromtimestamp(time.monotonic(), tz=timezone.utc).isoformat()}