...
def generate_and_store_master_key() -> str:
    ...
    if keyring is None:
        logger.warning(
            "OS Keychain integration unavailable. Set CORTEX_MASTER_KEY or CORTEX_VAULT_KEY manually with the generated wrapped key."
        )
        return wrapped_b64
    try:
        keyring.set_password(SERVICE_NAME, KEY_NAME, wrapped_b64)
        ...
        logger.error(
            "Could not store key in Keychain (%s). Set CORTEX_MASTER_KEY env var manually with the generated wrapped key.",
            e,
        )
    return wrapped_b64
