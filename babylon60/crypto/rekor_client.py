...
class RekorClient:
    ...
    def anchor_payload(self, payload_hash: str, signature_b64: str, public_key_pem: str) -> dict[str, Any] | None:
        data = {'kind': 'hashedrekord', 'apiVersion': '0.0.1', 'spec': {'signature': {'content': signature_b64, 'publicKey': {'content': base64.b64encode(public_key_pem.encode('utf-8')).decode('utf-8')}}, 'data': {'hash': {'algorithm': 'sha256', 'value': payload_hash}}}}
        req = urllib.request.Request(f'{self.rekor_url}/api/v1/log/entries', data=json.dumps(data).encode('utf-8'), headers={'Content-Type': 'application/json', 'Accept': 'application/json'}, method='POST')
        try:
            with urllib.request.urlopen(req, timeout=10) as response:
                if response.status in (201, 200):
                    ...
                    if resp_json:
                        ...
                        return {'uuid': uuid, 'logIndex': entry.get('logIndex'), 'integratedTime': entry.get('integratedTime')}
