...
class KapsoGateway:
    ...
    def __init__(
        self, api_key: str, phone_number_id: str, base_url: str = "https://api.kapso.ai/v1"
    ):
    ...
    async def send_message(self, message: WhatsAppMessage) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            try:
                ...
                logger.error("Kapso API HTTP Error: %s", e.response.text)
                raise
            except httpx.RequestError as e:
                logger.error("Kapso API Request Error: %s", e)
                raise
