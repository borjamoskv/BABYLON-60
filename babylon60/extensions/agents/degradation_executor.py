        ...
        async def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> _R:
            ...
            try:
                ...
                if fallback_mode == "text_only" and action is not None:
                    ...
                    try:
                        result = await fn(*degraded_args, **kwargs)  # type: ignore[arg-type]
                        if isinstance(result, AgentResult):
                            result.latency_ms = (time.perf_counter() - t0) * 1000
                        ...
                        return result
                    except (ValueError, TypeError, KeyError, RuntimeError, OSError, AssertionError) as inner_exc:  # noqa: BLE001
                        raise AgentDegradedError(
                            cause=inner_exc,
...
async def _persist_to_cortex(
    ...
    except (ValueError, TypeError, KeyError, RuntimeError, OSError, AssertionError) as persist_exc:  # noqa: BLE001
        logger.debug(
