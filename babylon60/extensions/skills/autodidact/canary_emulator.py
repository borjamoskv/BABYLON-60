...
class GPTCanaryDetector:
    ...
    async def benchmark_endpoint(
        ...
        if result.is_err():
            logger.error("❌ Canary benchmark failed: %s", result.error)  # type: ignore[union-attr]
            return {"status": "FAIL", "error": result.error}  # type: ignore[union-attr]
...
class GlasswingVulnerabilityScanner:
    ...
    async def scan_and_patch(self, code_block: str, language: str = "python") -> dict[str, Any]:
        ...
        if result.is_err():
            logger.error("❌ Mythos scan failed: %s", result.error)  # type: ignore[union-attr]
            return {"status": "FAIL", "error": result.error}  # type: ignore[union-attr]
