...
class SovereignGate:
    ...
    def __init__(
        ...
        if isinstance(self._secret, str):
            self._secret = self._secret.encode("utf-8")  # type: ignore[assignment]
    ...
    def request_approval(
        ...
        challenge = hmac.new(
            self._secret,  # type: ignore[call-overload]
    ...
    def approve_interactive(self, action_id: str) -> bool:
        ...
        logging.getLogger(__name__).info(f"  Action:  {action.description}")
        logging.getLogger(__name__).info(f"  Level:   {action.level.value}")
        logging.getLogger(__name__).info(f"  Project: {action.project or 'N/A'}")
        if action.command:
            cmd_str = " ".join(action.command)
            if len(cmd_str) > 100:
                cmd_str = cmd_str[:100] + "..."
            logging.getLogger(__name__).info(f"  Command: {cmd_str}")
        logging.getLogger(__name__).info(f"  ID:      {action_id}")
        logging.getLogger(__name__).info(f"{'=' * 60}")
