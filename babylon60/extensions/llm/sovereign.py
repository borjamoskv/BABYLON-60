...
Usage::
    ...
    async with SovereignLLM() as llm:
        ...
        logging.getLogger(__name__).info(result.content)      # Always has content
        logging.getLogger(__name__).info(result.provider)     # Which provider answered
        logging.getLogger(__name__).info(result.is_local)     # True if local fallback was used
        logging.getLogger(__name__).info(result.is_template)  # True if zero-connectivity template
        logging.getLogger(__name__).info(result.latency_ms)   # Actual latency measured
"""
