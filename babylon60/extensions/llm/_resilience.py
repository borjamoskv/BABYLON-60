...
class CircuitBreaker:
    ...
    def __init__(self, provider_name: str, failure_threshold: int = 50, recovery_timeout: int = 60):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if isinstance(e, httpx.HTTPStatusError) and e.response.status_code in (
                ...
                logger.error(
                    "LLM Call [FATAL/FAIL-FAST] -> Status: %d | Latency: %.2fs",
            ...
            if not is_retryable(e) or attempt == max_attempts:
                ...
                logger.error(
                    "LLM Call [FAIL] -> Status: %s | Latency: %.2fs | Attempt: %d/%d",
                    status,
                    latency,
                    attempt,
                    max_attempts,
            ...
            logger.warning(
                "LLM Call [RETRY] -> Error: %s | Next try in %.2fs (Attempt %d/%d)",
                type(e).__name__,
                sleep_s,
                attempt,
                max_attempts,
