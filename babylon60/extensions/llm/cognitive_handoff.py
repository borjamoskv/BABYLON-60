...
class CognitiveHandoff:
    ...
    def get_drm_route(self, tolerance_variance: float) -> dict[str, Any]:
        ...
        logger.info(
            ...
            route["provider"],
            route["temperature"],
            route["reasoning_mode"].value if route["reasoning_mode"] else "Standard",  # type: ignore[attr-defined]
        )
        return route
    ...
    def _involves_axiomatics(
        ...
        if belief.confidence_score >= 0.95:
            return True
        return any(b.confidence_score >= 0.95 for b in context)
    ...
    def _format_belief_for_prompt(
        ...
        parts = [
            ...
            f"Confidence: {belief.confidence_score}\n"
        ...
        if context:
            ...
            for b in context[:20]:  # Cap context to prevent token explosion
                parts.append(f"- [{b.id}] ({b.confidence_score}) {b.content}\n")
