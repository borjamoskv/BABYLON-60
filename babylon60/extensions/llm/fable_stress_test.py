...
async def run_stress_test(concurrency: int, total_requests: int):
    ...
    logging.getLogger(__name__).info(f"Total Requests: {total_requests}")
    logging.getLogger(__name__).info(f"Concurrency Limit: {concurrency}")
    logging.getLogger(__name__).info(f"Wall Time: {total_wall_time:.3f}s")
    logging.getLogger(__name__).info(f"Successful: {len(successes)}")
    logging.getLogger(__name__).info(f"Failed: {len(errors)}")
    if successes:
        logging.getLogger(__name__).info(f"Average Latency per task: {avg_latency:.3f}s")
    if errors:
        logging.getLogger(__name__).info(f"Sample Error: {errors[0]['error']}")
