...
class SovereignQuotaManager:
    ...
    def __init__(
        ...
        configured_path = db_path or os.environ.get("CORTEX_LLM_QUOTA_DB", "~/.babylon60/quota.db")
        self.db_path = Path(configured_path).expanduser()

        env_capacity = os.environ.get("CORTEX_LLM_QUOTA_CAPACITY")
        self.capacity = float(env_capacity) if env_capacity else float(capacity)

        env_max_waiters = os.environ.get("CORTEX_LLM_MAX_WAITERS")
