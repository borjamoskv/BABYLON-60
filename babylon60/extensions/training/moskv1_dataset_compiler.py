...
_ANERGY_PATTERNS: list[re.Pattern[str]] = [
    ...
    for p in [
        r"\b(hola|buenos días|buenas tardes|hey|hi there)\b",
        r"\b(espero que|hope this helps|let me know if)\b",
        r"\b(aquí tienes|here you go|here is the)\b",
        r"\b(por supuesto|of course|sure thing|certainly)\b",
        r"\b(no dudes en|feel free to|don't hesitate)\b",
        r"\b(claro|por supuesto|absolutely)\b",
        r"\b(es importante recordar|it's important to note)\b",
        r"\b(en resumen|to summarize|in conclusion)\b",
        r"\b(como puedes ver|as you can see)\b",
        r"\b(vale la pena|it's worth noting)\b",
        r"\b(me alegra|I'm glad|I'm happy to)\b",
        r"\b(sin más preámbulos|without further ado)\b",
        r"\b(recuerda que|remember that|keep in mind)\b",
        r"^(ok,?\s|okay,?\s|alright,?\s)",
        r"\b(básicamente|basically|essentially)\b",
    ]
]
...
class MOSKV1DatasetCompiler:
    ...
    def _split_dataset(
        ...
        if not val and shuffled:
            val = [shuffled[0]]
        if not test and shuffled:
            test = [shuffled[0]]
