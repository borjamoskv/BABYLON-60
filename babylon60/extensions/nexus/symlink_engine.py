...
class SymlinkEngine:
    ...
    def _safe_backup(self, target_path: str) -> None:
        ...
        logger.warning("[Nexus] Backing up physical entity: %s -> %s", target_path, backup_path)
    ...
    def _enforce_link(self, source_path: str, target_path: str) -> bool:
        ...
        if not os.path.exists(source_path):
            logger.error("[Nexus] Canonical source missing: %s", source_path)
            return False
        ...
        logger.info("[Nexus] OP_BIND_NEXUS: %s -> %s", target_path, source_path)
        return True
    ...
    def propagate(self, target_workspaces: list[str], artifacts: list[str]) -> dict[str, bool]:
        ...
        for workspace in target_workspaces:
            ...
            if not os.path.isdir(os.path.abspath(workspace)):
                logger.warning("[Nexus] Target workspace unreachable: %s", workspace)
        ...
        for workspace in target_workspaces:
            for artifact in artifacts:
                ...
                if os.path.exists(target) and not os.path.islink(target):
                    logger.critical(
                        "[Nexus] INV_NEXUS_LINK VIOLATED: %s is a physical entity, not a symlink.",
                        target,
                    )
                    return False
        return True
