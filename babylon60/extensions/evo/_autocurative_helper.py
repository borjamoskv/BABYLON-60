...
from babylon60.extensions.evo._autocurative_state import HealingEvent, HealingPhase
from babylon60.extensions.evo.repair_strategies import RepairResult
...
async def execute_with_healing(
    ...
    **kwargs: Any,
    ...
    for attempt in range(agent.config.max_healing_attempts):
        ...
        _check_circuit_breaker(agent, breaker, subsystem)

        agent._phase = HealingPhase.EXECUTING
        try:
            result = await _execute_task(agent, task, *args, **kwargs)
            return _handle_success(agent, breaker, subsystem, attempt, result)
        except (ValueError, TypeError, KeyError, OSError, RuntimeError) as error:
            last_error = error
            await _handle_failure(agent, breaker, subsystem, error, ctx, attempt, cycle_start)

    return _handle_exhaustion(agent, subsystem, last_error)


def _check_circuit_breaker(agent: AutoCurativeAgent, breaker: Any, subsystem: str) -> None:
...
async def _execute_task(agent: AutoCurativeAgent, task: Any, *args: Any, **kwargs: Any) -> Any:
...
def _handle_success(
    agent: AutoCurativeAgent, breaker: Any, subsystem: str, attempt: int, result: Any
) -> Any:
...
def _handle_exhaustion(
    agent: AutoCurativeAgent, subsystem: str, last_error: Exception | None
) -> Any:
    agent._phase = HealingPhase.IDLE
    logger.error(
