...
class CDPSovereignWallet:
    ...
    Wallet seed persists to ~/.babylon60/ with restricted file permissions (0600).
    """
    ...
    def __init__(
        ...
        seed_path: str = "~/.babylon60.agents_wallet_seed.json",
    ):
...
if __name__ == "__main__":
    ...
    async def run_wallet_test():
        logging.getLogger(__name__).info("--- Testing CDP Sovereign Wallet V3 ---")
        wallet = CDPSovereignWallet(network_id="base-sepolia")
        success = wallet.initialize()

        if success:
            balance = await wallet.get_balance("eth")
            logging.getLogger(__name__).info(f"✅ Wallet OK. Balance: {balance} ETH")
            logging.getLogger(__name__).info(f"   Tools loaded: {len(wallet.tools)}")
            ...
            logging.getLogger(__name__).info("❌ Init failed. Check ENVs and deps.")
