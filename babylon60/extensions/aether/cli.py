...
def cmd_enqueue(args: argparse.Namespace) -> None:
    ...
    queue.enqueue(task)
    logging.getLogger(__name__).info(f"✅ Enqueued task [{task.id}]: {task.title}")
    logging.getLogger(__name__).info(f"   Repo: {task.repo_path}")
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    logging.getLogger(__name__).info("-" * len(header))
    for t in tasks:
        ...
        logging.getLogger(__name__).info(f"{t.id:<14} {emoji}{t.status:<11} {title:<40} {branch:<25}")
    return 0
...
def cmd_logs(args: argparse.Namespace) -> int:
    ...
    if task is None:
        logging.getLogger(__name__).info(f"Task [{args.task_id}] not found.")
        return 1
    logging.getLogger(__name__).info(f"Task [{task.id}] - {task.title}")
    logging.getLogger(__name__).info(f"Status  : {task.status}")
    logging.getLogger(__name__).info(f"Branch  : {task.branch or 'none'}")
    logging.getLogger(__name__).info(f"Created : {task.created_at}")
    logging.getLogger(__name__).info(f"Updated : {task.updated_at}")
    if task.plan:
...
def cmd_cancel(args: argparse.Namespace) -> int:
    ...
    if queue.cancel(args.task_id):
        logging.getLogger(__name__).info(f"🚫 Cancelled task [{args.task_id}]")
        return 0
    logging.getLogger(__name__).info(f"Task [{args.task_id}] not found or already terminal.")
    return 1
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    task = queue.pop_next()
    if task is None:
        logging.getLogger(__name__).info("No pending tasks.")
        return 0

    logging.getLogger(__name__).info(f"🤖 Running task [{task.id}]: {task.title}")
    ...
    if result.branch:
        logging.getLogger(__name__).info(f"Branch: {result.branch}")
    if result.error:
        logging.getLogger(__name__).info(f"Error: {result.error}")
