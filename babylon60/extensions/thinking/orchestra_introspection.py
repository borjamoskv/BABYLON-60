...
class OrchestraIntrospectionMixin:
    ...
    def stats(self) -> dict[str, Any]:
        ...
        top_winner = max(winner_counts, key=lambda k: winner_counts[k]) if winner_counts else None
        ...
        return {
            ...
            "mode_distribution": {
                mode: sum(1 for r in self.history if r.mode == mode)  # type: ignore[reportAttributeAccessIssue]
                for mode in {r.mode for r in self.history}  # type: ignore[reportAttributeAccessIssue]
            },
        }
