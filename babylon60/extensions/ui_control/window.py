...
class WindowEngine:
    ...
    async def _set_window_property(
        self, target: AppTarget, prop: str, value: str, delay: float = 0.3
    ) -> InteractionResult:
        """Helper to set a window property via AppleScript."""
        script = f"""
        tell application "{target.name}" to activate
        delay {delay}
        tell application "System Events"
            tell process "{target.name}"
                set {prop} of window 1 to {value}
            end tell
        end tell
        """
        try:
    ...
    async def move(self, target: AppTarget, x: int, y: int) -> InteractionResult:
        ...
        return await self._set_window_property(target, "position", f"{{{x}, {y}}}")
    ...
    async def resize(self, target: AppTarget, width: int, height: int) -> InteractionResult:
        ...
        return await self._set_window_property(target, "size", f"{{{width}, {height}}}")

    async def minimize(self, target: AppTarget) -> InteractionResult:
        """Minimizes the frontmost window of the target app."""
        return await self._set_window_property(
            target, 'value of attribute "AXMinimized"', "true", delay=0.2
        )
