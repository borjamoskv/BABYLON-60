...
class AltozanoAnalyzer:
    ...
    def deconstruct_track(self, filepath: str | Path) -> dict[str, Any]:
        ...
        logger.info("[Altozano] Extracting structural invariants from: %s", path.name)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        logging.getLogger(__name__).info(yaml.dump(res, default_flow_style=False))
    else:
        logging.getLogger(__name__).info("Usage: python altozano.py <path_to_wav>")
