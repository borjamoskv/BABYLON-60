...
def _print_swarm(engine: EvolutionEngine) -> None:
    ...
    logging.getLogger(__name__).info(f"  🧬 CORTEX EVOLUTION ENGINE - Cycle {status['cycle']}")
    logging.getLogger(__name__).info("═══════════════════════════════════════════════════════")
...
async def _run_n_cycles(engine: EvolutionEngine, n: int) -> None:
    ...
    if report:
        logging.getLogger(__name__).info(json.dumps(report.to_dict(), indent=2))
...
def main() -> None:
    ...
    if args.once:
        ...
        logging.getLogger(__name__).info("🧬 Evolution Engine - Press Ctrl+C to stop\n")
        try:
            asyncio.run(_run_forever(engine))
        except KeyboardInterrupt:
            engine.stop()  # type: ignore[reportAttributeAccessIssue]
            _print_swarm(engine)
            logging.getLogger(__name__).info("🛑 Evolution Engine halted.")
            sys.exit(0)
