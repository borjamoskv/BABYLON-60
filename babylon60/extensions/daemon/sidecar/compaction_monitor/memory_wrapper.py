...
def malloc_trim(pad: int = 0) -> int:
    ...
    if not HAS_MALLOC_TRIM:
        return 0
    try:
        res = _libc.malloc_trim(pad)  # type: ignore[union-attr]
        return res
    except (ValueError, TypeError, OSError, KeyError):
        return 0
...
class MallInfo2:
    ...
    def from_c() -> "MallInfo2":
        ...
        if not HAS_MALLINFO2:
            return MallInfo2()
        try:
            raw = _libc.mallinfo2()  # type: ignore[union-attr]  # type: ignore[reportOptionalMemberAccess]
            return MallInfo2(
                arena=raw.arena,
                ordblks=raw.ordblks,
                smblks=raw.smblks,
...
if __name__ == "__main__":
    ...
    try:
        malloc_trim()
        logging.getLogger(__name__).info("malloc_trim succeeded")
    except OSError as exc:
        logging.getLogger(__name__).info("malloc_trim failed:", exc)
