...
class IHelpPurgeDaemon:
    ...
    async def phagocytize(self, payload: str, source_agent: str) -> dict[str, Any]:
        ...
        logger.warning(
            "[%s] 🛑 ANERGY DETECTED 🛑\nSource: %s\nAntigen Hash: %s\nThermodynamic Action: Payload obliterated. Saved %s bytes (%s tokens) of GC/LLM evaluation overhead.",
            self.agent_id,
            source_agent,
            payload_hash[:16],
            waste_bytes,
        ...
        try:
            ...
            priv_key_b64: str | None = km.get_private_key_b64(self.agent_id)
            if not priv_key_b64:
