...
class _FloatingPointVisitor(ast.NodeVisitor):
    ...
    def visit_Name(self, node: ast.Name) -> None:
        if node.id == "float":
            self.findings.append(
                AntipatternFinding(
                    scanner="FloatingPoint",
                    severity="high",
                    file=self.rel,
                    line=node.lineno,
                    message="Usage of float detected (ANT-203)",
                    fix_hint="Use decimal.Decimal for financial/scoring logic",
                )
            )
        self.generic_visit(node)
        ...
        if node.id == "Any":
            self.findings.append(
                AntipatternFinding(
                    ...
                    file=self.rel,
                    line=node.lineno,
                    message="Stochastic typing `Any` detected (ANT-021)",
                    fix_hint="Use explicit typing or Union",
                )
            )
        self.generic_visit(node)
...
def _scan_single_file(
    ...
    _FloatingPointVisitor(rel, findings).visit(tree)
    _AnyTypeVisitor(rel, findings).visit(tree)
