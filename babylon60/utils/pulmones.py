        ...
        for candidate in candidates:
            try:
                ...
                if candidate != self.db_path:
                    logger.warning('🫁 [PULMONES] Primary DB unavailable at %s; falling back to %s', self.db_path, candidate)
    ...
    def enqueue(self, func_name: str, args: tuple, kwargs: dict[str, typing.Any], delay: float=60.0) -> None:
        ...
        try:
            with babylon60.database.core.connect_sync(self.db_path) as conn:
                conn.execute('INSERT INTO fallback_queue (target_func, payload, next_retry_at) VALUES (?, ?, ?)', (func_name, payload, next_retry))
            ...
            if self.db_path != self._fallback_path:
                try:
                    ...
                    with babylon60.database.core.connect_sync(self.db_path) as conn:
                        conn.execute('INSERT INTO fallback_queue (target_func, payload, next_retry_at) VALUES (?, ?, ?)', (func_name, payload, next_retry))
                    logger.warning('🫁 [PULMONES] Primary queue readonly; payload for %s enqueued via fallback DB.', func_name)
                    return
