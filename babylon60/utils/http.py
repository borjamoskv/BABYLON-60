...
class HttpRetryMixin:
    ...
    async def _post_with_retry(
        self, url: str, headers: dict[str, str], payload: dict[str, Any], label: str = "POST"
    ) -> dict[str, Any]:
    ...
    async def _do_http_call(
        self, method: str, url: str, headers: dict[str, str], payload: dict[str, Any] | None, label: str
    ...
    async def _request_with_retry(
        self, method: str, url: str, headers: dict[str, str], payload: dict[str, Any] | None = None, label: str = ""
    ) -> dict[str, Any]:
        import httpx
...
async def _do_standalone_post(
    client: Any, url: str, headers: dict[str, str], payload: dict[str, Any], provider: str, label: str
) -> dict[str, Any] | Exception:
    import httpx
...
async def post_with_retry(
    ...
    for attempt in range(max_retries):
        ...
        delay = base_delay_val + random.uniform(0.1, 2.0) ** (attempt + 1)
        logger.warning(
            "HTTP POST 429 [%s] %s. Retry %d/%d in %.1fs...", provider, label, attempt + 1, max_retries, delay
        )
        await asyncio.sleep(delay)
