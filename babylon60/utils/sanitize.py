...
def sanitize_project_name(project: str) -> str:
    ...
    if not _PROJECT_RE.match(project):
        raise ValueError(
            f"Invalid project name: '{project}'. Must be 1-128 chars, alphanumeric with _-. allowed, starting with alphanumeric."
        )
    return project
...
def sanitize_tenant_id(tenant_id: str) -> str:
    ...
    if not _TENANT_RE.match(tenant_id):
        raise ValueError(
            f"Invalid tenant_id: '{tenant_id}'. Must be 1-64 chars, lowercase alphanumeric with _- allowed."
        )
    return tenant_id
