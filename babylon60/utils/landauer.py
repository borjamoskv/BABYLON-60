...
class LandauerAnalyzer(ast.NodeVisitor):
    ...
    def _push_node(self, node, node_type):
        node_info = {'name': getattr(node, 'name', '<anonymous>'), 'type': node_type, 'start_line': node.lineno, 'end_line': getattr(node, 'end_lineno', node.lineno), 'complexity': 1}
...
def calculate_calcification(file_path: Path) -> dict | None:
    try:
        ...
        return {'file': str(file_path.name), 'loc': loc, 'complexity': analyzer.complexity, 'score': round(calcification_score, 2), 'is_parasite': calcification_score > 50, 'nodes': sorted(analyzer.node_metrics, key=lambda x: x['score'], reverse=True)}
