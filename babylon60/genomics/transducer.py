...
class GenomicStateTransducer:
    GENE_TO_PRIMITIVE_MAP: dict[str, str] = {'TP53': 'ONC-046', 'BRCA1': 'ONC-151', 'BRCA2': 'ONC-152', 'ATM': 'ONC-136', 'ATR': 'ONC-137', 'EGFR': 'ONC-017', 'KRAS': 'ONC-018', 'MYC': 'ONC-019', 'PIK3CA': 'ONC-020', 'PTEN': 'ONC-047', 'TERT': 'ONC-163'}

    @classmethod
    def transduce_variant_records(cls, variants: list[GenomicVariantRecord], base_state: dict[str, int] | None=None, target_region_mb: float=38.0) -> dict[str, Any]:
        ...
        return {'state_matrix': state, 'tmb_result': tmb_res, 'apobec_result': apobec_res, 'mutated_genes': sorted(mutated_genes), 'causal_taint': 'borjamoskv:genomic_state_transducer_c5'}

    @classmethod
    def transduce_full_profile(cls, params: FullProfileParams | None=None, *, variants: list[GenomicVariantRecord] | None=None, loh_events: int=0, total_regions: int=1, wgd_detected: bool=False, ecdna_records: list[dict[str, Any]] | None=None, base_state: dict[str, int] | None=None, target_region_mb: float=38.0) -> dict[str, Any]:
        if params is None:
            if variants is None:
                raise ValueError('[C5-FAIL] Either params or variants must be provided.')
            params = FullProfileParams(variants=variants, loh_events=loh_events, total_regions=total_regions, wgd_detected=wgd_detected, ecdna_records=ecdna_records, base_state=base_state, target_region_mb=target_region_mb)
        ...
        if params.ecdna_records:
            ...
            for rec in params.ecdna_records:
                amp_res = GenomicEvaluationEngine.evaluate_ecdna_amplicon(amplicon_id=str(rec['amplicon_id']), oncogenes=list(rec['oncogenes']), copy_number=int(rec['copy_number']), circular_confirmed=bool(rec['circular_confirmed']), rna_fold_change=float(rec['rna_fold_change']))
        ...
        return {'state_matrix': state_matrix, 'tmb_result': base_transduction['tmb_result'], 'apobec_result': base_transduction['apobec_result'], 'hrd_result': hrd_res, 'ecdna_results': ecdna_results, 'mutated_genes': base_transduction['mutated_genes'], 'causal_taint': 'borjamoskv:full_genomic_profile_transducer_c5'}