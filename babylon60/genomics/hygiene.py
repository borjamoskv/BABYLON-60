    ...
    def validate_sequence(cls, seq_id: str, sequence: str, quality_scores: str | None=None) -> dict[str, Any]:
        ...
        if quality_scores is not None:
            ...
            if len(quality_scores) != seq_len:
                raise ValueError(f"[C5-FAIL] FASTQ quality length ({len(quality_scores)}) does not match sequence length ({seq_len}) for ID '{seq_id}'.")
        ...
        return {'seq_id': seq_id, 'length': seq_len, 'gc_content': gc_content, 'sha3_256': sha3_hash, 'causal_taint': 'borjamoskv:fasta_validator_c5', 'is_valid': True}