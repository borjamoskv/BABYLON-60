...
class CortexOntologyLedger:
    ...
    def _verify_topology(self) -> None:
        ...
        for p in paths:
            if not os.path.exists(p):
                logging.info(f'[-] FATAL: Ontological path missing: {p}')
    ...
    def crystallize_session(self, delta: SessionDelta) -> None:
        ...
        logging.info(f'[+] CORTEX PERSIST: Ontología cristalizada con éxito para {self.project_id}')
if __name__ == '__main__':
    if len(sys.argv) < 2:
        logging.info('Uso: python3 cortex_persist.py <PROJECT_ID> <CONVERSATION_ID>')
        sys.exit(1)
    pid = sys.argv[1]
    cid = sys.argv[2] if len(sys.argv) > 2 else 'CLI-FALLBACK'
    delta = SessionDelta(task_desc='Manual CORTEX Persist trigger via CLI', last_file='babylon60/core/cortex_persist.py', conversation_id=cid, changes=['Forced synchronization of BFT state'], decisions=['CORTEX memory boundary manual check'])
