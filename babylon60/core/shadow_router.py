...
class ShadowRouter:
    ...
    def __init__(self, signer: Ed25519Signer, config: RouteConfig | None=None) -> None:
        ...
        self.circuit_breaker = CircuitBreaker(failure_threshold=self.config.circuit_breaker_threshold, reset_timeout=self.config.circuit_breaker_reset_seconds)
...
async def demo() -> None:
    ...
    logging.info('Decision Receipt (T0):')
    logging.info(json.dumps(t0_receipt, indent=2))
    logging.info('\nExecution Receipt (T1):')
    logging.info(json.dumps(t1_receipt, indent=2))
