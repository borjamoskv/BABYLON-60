...
class Moskv1Kernel:
    ...
    async def bft_state_loop(self) -> None:
        logging.info('[MOSKV-1] KERNEL IGNITION: BFT_STATE_LOOP ACTIVATED.')
        try:
            while True:
                claim = await self._write_queue.get()
                if claim.confidence not in ('C4', 'C5'):
                    logging.info(f'[-] Anergía detectada en {claim.claim_id}. Purgando.')
                ...
                try:
                    ...
                    try:
                        await db.execute('INSERT INTO master_ledger (claim_id, payload, prev_hash, current_hash, cortex_taint, lamport_t) VALUES (?, ?, ?, ?, ?, ?)', (claim.claim_id, json.dumps(claim.payload), claim.prev_hash, current_hash, taint_signature, claim.lamport_t))
                        self._last_hash = current_hash
                        logging.info(f'[+] Cristalizado: {claim.claim_id} -> {current_hash[:8]}')
                    finally:
                        await db.close()
                except sqlite3.IntegrityError:
                    logging.info(f'[!] Idempotency Lock disparado para {claim.claim_id}. Entropía abortada.')
                except sqlite3.DatabaseError as e:
                    raise RuntimeError('FAIL-FAST: BFT Ledger corrompido.') from e
                self._write_queue.task_done()
        except asyncio.CancelledError:
            logging.info('[MOSKV-1] KERNEL SHUTDOWN: SIGTERM RECIBIDO.')
