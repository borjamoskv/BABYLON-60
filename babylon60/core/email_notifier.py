...
class PurchaseNotifier:
    ...
    def notify_purchase(self, customer_email: str, tier: str, amount_eur: int, license_key: str, session_id: str) -> dict[str, Any]:
        ...
        # 1. Log to local persistent notifications JSON
        self._append_to_log(alert_data)

        # 2. Print high-visibility C5-REAL terminal alert
        print(f"\n🚨 [NUEVA COMPRA DETECTADA] ¡Ingreso de €{amount_eur}! Cliente: {customer_email} ({tier})")
        print(f"   Aviso enviado a: {self.target_email}")
        print(f"   Clave Emitida: {license_key}\n")

        # 3. SMTP Direct Dispatcher if credentials configured
    ...
    def _send_smtp_email(self, data: dict[str, Any], smtp_user: str, smtp_pass: str) -> bool:
        try:
            ...
            body = (
                ...
                f"• Cliente: {data['customer_email']}\n"
                f"• Plan: {data['tier']}\n"
                f"• Importe: €{data['amount_eur']} EUR\n"
                f"• ID de Sesión: {data['session_id']}\n"
                f"• Clave Emitida: {data['license_key']}\n"
