...
def enforce_no_coredump() -> None:
    try:
        ...
        logging.info(f'[OPSEC] RLIMIT_CORE rejection: {exc}', file=sys.stderr)
        _kill_self()
...
def enforce_anti_debug() -> None:
    ...
    try:
        ...
        if result != 0:
            logging.info('[OPSEC] Debugger detected — SIGKILL.', file=sys.stderr)
            _kill_self()
    except OSError as exc:
        logging.info(f'[OPSEC] ptrace syscall failed: {exc}', file=sys.stderr)
        _kill_self()
