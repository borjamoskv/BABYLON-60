...
- **Tolerancia Flotante en Benchmarks SIMD (INV_C5_SIMD_REL_EPSILON):** Al contrastar acumulaciones vectorizadas SIMD (ARM64 NEON `vfmaq_f64`, AVX-512) frente a trayectorias escalares en tests de estrés o PoCs, debido a la no-asociatividad intrínseca de la suma en IEEE 754, las aserciones NUNCA deben exigir discrepancia absoluta infinitesimal ($\Delta < 10^{-12}$) sobre sumatorias de gran cardinalidad ($N > 10^3$). La aserción DEBE calibrarse mediante el error relativo normalizado $\frac{|E_{\text{SIMD}} - E_{\text{scalar}}|}{\max(|E_{\text{scalar}}|, 1.0)} < 10^{-11}$, certificando la fidelidad a nivel de épsilon de máquina sin inducir falsos negativos por reordenamiento de carriles vectoriales.
...
- **Desacople del Corpus de Investigación Teórica**:
  - Los ensayos de isomorfismo histórico/social (ej. colapso de Mesopotamia, comoditización de morfismos), manifiestos discursivos de singularidad y artículos reflexivos deben residir exclusivamente en el repositorio hermano satélite: **`C5-RESEARCH-FOUNDATIONS`** (`../C5-RESEARCH-FOUNDATIONS`).
...
- **Manejo de Fallos por Sandbox y Códigos Sexagesimales:** El *gate* biométrico emite códigos de salida deterministas tipados (60: sin evaluación, 61: `ERR_NOT_INTERACTIVE` / sandbox, 62: `ERR_CLAMSHELL_LOCKED`, 63: `ERR_OPERATOR_ABORT`, 64: `ERR_SECURE_ENCLAVE`). Si un script o llamador enjaulado recibe código 61, activa automáticamente el Trampolín GUI vía `osascript` (o RPC al daemon de Ring-1) para heredar el WindowServer sin requerir intervención manual del operador.
...
- **Oráculos Formales AOT en Silicio (INV_C5_LEAN_AOT_ORACLE):** Todo verificador o tribunal formal en Lean 4 consumido por el consenso BFT isostático (LARSA-120) o planificadores en tiempo de ejecución DEBE compilarse como ejecutable C nativo autónomo (`[[lean_exe]]` en `lakefile.toml`). Queda estrictamente prohibido invocar `lean --run` o el entorno interactivo de Lean en bucles de evaluación de Ring-0/Ring-1. El ejecutable debe procesar la traza física en modo streaming $O(1)$ (vía `IO.FS.Handle` recursivo), transduciendo la causalidad sin instanciar árboles sintácticos en el heap.
...
## 🔒 Invariante de Ignición Soberana (Bootstrapping Trinity)
Todo script de inicialización o *setup* (Python, Bash, Rust) que genere el entorno de BABYLON-60 (`.env`) DEBE superar de forma determinista tres compuertas:
