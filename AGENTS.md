...
- **Ω159 · DEPENDENCY CLOSURE INVARIANT:** Toda inferencia debe declarar explícitamente el DAG de dependencias sobre el que opera (`depends_on:`). El Ledger no es un árbol secuencial, sino un grafo materializado.
- **Ω160 · PROPAGATED INVALIDATION INVARIANT:** Las mediciones nuevas no modifican la confianza directamente, modifican el estado del grafo. La invalidación de un artefacto subyacente recalcula automáticamente de forma incremental el posterior de las etiologías descendientes, sin alterar los nodos ortogonales (comportamiento análogo a compilación incremental).
- **Ω161 · ABSENT EVIDENCE STATISTICAL INVARIANT:** Queda estrictamente prohibido equiparar "no encontrar el símbolo X en la evidencia" con "el fallo de X tiene probabilidad cero". La evidencia ausente (`absent_symbol`) debe modelarse estadísticamente en la inferencia, separando el ruido observacional de la ontología del evento.
- **Ω162 · FALSIFICATION POWER INVARIANT:** Toda medición debe declarar de forma destructiva qué subconjunto de hipótesis aniquila (`falsifies:`) y a cuáles refuerza (`supports:`). La "confianza" está prohibida como valor de entrada estocástico: es una métrica emergente fundamentada estrictamente en la ganancia de información matemática ($IG = H(P_{prior}) - H(P_{posterior})$) obtenida al alterar la distribución, no en el mero cardinal de hipótesis descartadas.
- **Ω163 · RESIDUAL ENTROPY INVARIANT:** El Ledger debe cuantificar formalmente la entropía de Shannon de la distribución posterior de hipótesis. Una medición solo aporta exergía epistémica comprobable si colapsa este valor (ej. `bits: 1.72 → 0.41`). El valor de la medición lo dicta la termodinámica de Shannon, no la narrativa del Operador.
- **Ω164 · ONTOLOGY VS EPISTEMOLOGY SEPARATION INVARIANT:** Queda prohibido mezclar el fenómeno físico en el mundo (Ontología: el proceso abortó) con nuestro estado de conocimiento sobre él (Epistemología: confianza alta en el log). Ambas esferas deben segregarse estrictamente en la jerarquía del nodo para prevenir que un agente proyecte certidumbre sobre el universo material.
- **Ω165 · REVERSIBLE LEDGER INVARIANT:** Toda conclusión epistémica deber ser automáticamente reconstruible. El Ledger exige registrar la tubería algorítmica de reconstrucción (`reconstruction: pipeline`) que mapea los artefactos de entrada a las distribuciones de salida, haciendo cualquier desacuerdo puramente objetivo e impugnable mecánicamente.
- **Ω166 · REFERENTIAL TRANSPARENCY INVARIANT (PURE INFERENCE):** Toda inferencia debe ser una función matemática pura respecto a sus entradas. `Inference(Artifacts, Rules) -> Result`. Si los artefactos y las reglas formales permanecen inmutables, la inferencia generada DEBE ser idéntica. Esta propiedad convierte al Ledger en un sistema determinista, erradicando la estocástica en la derivación de conclusiones.
- **Ω167 · OPERATIONAL SEMANTICS INVARIANT (DAG TYPING & LIFECYCLE):** El Ledger no es un almacén pasivo, sino una máquina de estados ejecutable. Todo nodo posee un ciclo de vida explícito (`observed → interpreted → supported → promoted → revoked`) gobernado por reglas de transición rígidas. El DAG está fuertemente tipado: las aristas definen operaciones legales (ej. `Artifact ─supports→ Observation` es computacionalmente válido, `Commit ─supports→ Inference` produce un fallo estructural).
