...
def test_handoff_rejection_empty_instruction():
    response = client.post("/handoff", json={"source_agent": "A1", "target_agent": "A2", "instruction": ""})
...
def test_handoff_valid_event():
    response = client.post(
        "/handoff",
        json={"source_agent": "moskv-1-apex", "target_agent": "claude-cowork", "instruction": "refactor bft"},
    )
    assert response.status_code == 200
