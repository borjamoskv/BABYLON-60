    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch.object(SovereignLicenseGate, '__init__', lambda self: None):
            with patch.object(SovereignLicenseGate, 'get_status') as mock_status:
                ...
                mock_status.return_value = LicenseStatus(active=True, tier=Tier.PRO_SWARM, owner='test@cortex.dev', ops_today=50, ops_limit=1000000, expires_at=2000000000, signature='SIG_TEST_C5')
                print_status()
