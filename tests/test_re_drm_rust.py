...
def test_re_drm_rust_bft_verification() -> None:
    if os.path.exists(DB_PATH):
        try:
            ...
            pass
    logging.info('[*] Launching native Rust BFT verifier...')
    env = os.environ.copy()
    env['PYO3_USE_ABI3_FORWARD_COMPATIBILITY'] = '1'
    env['PYO3_PYTHON'] = sys.executable
    res = subprocess.run(['cargo', 'run', '--manifest-path', 'strike_rs/Cargo.toml', '--bin', 're_drm_896_bft'], cwd=PROJECT_ROOT, capture_output=True, text=True, env=env)
    ...
    logging.info(f'[+] Verified in DB: Unanimous={unanimous}, Byzantine-Tolerant={tolerant}')
    assert unanimous + tolerant == 896, 'All 896 entries must be resolved under quorums'
    conn.close()
    logging.info('[+] Test successfully verified C5-REAL integration.')
