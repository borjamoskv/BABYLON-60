...
def test_coordinate_hygiene_bed_vcf_conversions() -> None:
    ...
    with pytest.raises(ValueError):
        GenomeCoordinateHygiene.bed_to_vcf('chr1', 100, 100)
...
def test_fasta_sequence_validation_and_crypto_hash() -> None:
    ...
    assert len(val['sha3_256']) == 64
    assert val['causal_taint'] == 'borjamoskv:fasta_validator_c5'
    with pytest.raises(ValueError):
        FASTASequenceValidator.validate_sequence('seq_bad', 'ACGTZZZ')

def test_tmb_evaluation_engine() -> None:
    variants = [GenomicVariantRecord(chrom='chr1', pos_0based=1000 + i, pos_1based=1001 + i, ref_allele='A', alt_allele='G', variant_type='SNV', quality=40.0, metadata={'gene': 'TP53' if i == 0 else 'GENE_X'}) for i in range(400)]
...
def test_apobec_enrichment_engine() -> None:
    variants = [GenomicVariantRecord(chrom='chr1', pos_0based=2000 + i, pos_1based=2001 + i, ref_allele='C', alt_allele='T', variant_type='SNV', quality=35.0, metadata={'trinucleotide_context': 'TCA'}) for i in range(10)] + [GenomicVariantRecord(chrom='chr2', pos_0based=3000 + i, pos_1based=3001 + i, ref_allele='A', alt_allele='G', variant_type='SNV', quality=35.0, metadata={'trinucleotide_context': 'AAA'}) for i in range(5)]
...
def test_loh_hrd_and_ecdna_engine() -> None:
    ...
    ecdna: ECDNAAmpliconResult = GenomicEvaluationEngine.evaluate_ecdna_amplicon(amplicon_id='AMP_MYC_01', oncogenes=['MYC', 'PVT1'], copy_number=25, circular_confirmed=True, rna_fold_change=150.0)
...
def test_genomic_transducer_integration_with_oncology_transducer() -> None:
    variants = [GenomicVariantRecord('chr17', 7673766, 7673767, 'C', 'T', 'SNV', 45.0, metadata={'gene': 'TP53'}), GenomicVariantRecord('chr17', 43044294, 43044295, 'G', 'A', 'SNV', 45.0, metadata={'gene': 'BRCA1'}), GenomicVariantRecord('chr7', 55242464, 55242465, 'T', 'G', 'SNV', 50.0, metadata={'gene': 'EGFR'})] + [GenomicVariantRecord('chr1', 10000 + i, 10001 + i, 'C', 'T', 'SNV', 35.0, metadata={'trinucleotide_context': 'TCA'}) for i in range(400)]
    profile = GenomicStateTransducer.transduce_full_profile(variants=variants, loh_events=20, total_regions=35, wgd_detected=True, ecdna_records=[{'amplicon_id': 'AMP_MYC_CIRC', 'oncogenes': ['MYC'], 'copy_number': 30, 'circular_confirmed': True, 'rna_fold_change': 200.0}])
    state_matrix = profile['state_matrix']
    assert state_matrix.get('ONC-046') == 1
    assert state_matrix.get('ONC-151') == 1
    assert state_matrix.get('ONC-017') == 1
    assert state_matrix.get('ONC-019') == 1
    assert state_matrix.get('ONC-146') == 1
    assert state_matrix.get('ONC-148') == 1
    assert state_matrix.get('ONC-143') == 1
    assert state_matrix.get('ONC-154') == 1
    G = nx.DiGraph()
    G.add_edges_from([('ONC-046', 'ONC-143'), ('ONC-143', 'ONC-146'), ('ONC-017', 'ONC-019'), ('ONC-154', 'ONC-019')])
