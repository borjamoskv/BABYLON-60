...
@pytest.fixture
async def ephemeral_ledger(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_idempotent_writes(ephemeral_ledger: BFTLedgerActor):
    """
...
@pytest.mark.asyncio
async def test_hash_chain_corruption_detection(ephemeral_ledger: BFTLedgerActor):
    """
    ...
    # Write valid entries
    for i in range(3):
        await ephemeral_ledger.append(LedgerEvent(
            stream="test",
...
@pytest.mark.asyncio
async def test_cascading_rollback_defense(ephemeral_ledger: BFTLedgerActor):
    """
    ...
    """
    # 1. Valid write
    res1 = await ephemeral_ledger.append(LedgerEvent(
        stream="test",
        entity_id="1",
        event_type="CREATED",
        payload={},
        source_db="a", source_table="b", source_pk="c",
    ...
    res3 = await ephemeral_ledger.append(LedgerEvent(
        stream="test",
        entity_id="3",
        event_type="CREATED",
        payload={},
