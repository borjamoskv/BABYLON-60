...
def test_zero_network_valid_localhost():
    response = client.post(
        "/api/inference/local/generate",
        json={"prompt": "Test", "model": "test", "base_url": "http://127.0.0.1:11434/v1"},
    )
...
def test_zero_network_valid_localhost_name():
    response = client.post(
        "/api/inference/local/generate",
        json={"prompt": "Test", "model": "test", "base_url": "http://localhost:11434/v1"},
    )
    assert response.status_code == 503
...
def test_zero_network_invalid_domain():
    response = client.post(
        "/api/inference/local/generate", json={"prompt": "Test", "model": "test", "base_url": "http://openai.com/v1"}
    )
...
def test_zero_network_attacker_bypass():
    response = client.post(
        "/api/inference/local/generate",
        json={"prompt": "Test", "model": "test", "base_url": "http://localhost.attacker.com/v1"},
    )
    assert response.status_code == 403
