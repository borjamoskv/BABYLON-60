...
def _route(prompt: str = "Explain quantum gravity"):  # type: ignore
...
def test_init_fails_fast_without_key_material(monkeypatch) -> None:  # type: ignore
...
def test_commitments_are_keyed_and_deterministic(monkeypatch) -> None:  # type: ignore
...
def test_hashes_commit_to_real_content(monkeypatch) -> None:  # type: ignore
    monkeypatch.setenv("CORTEX_SHADOW_HMAC_KEY", "clave-operador-A")
    d1, _ = _route()
    d2, _ = _route()
...
def test_simulation_is_declared_not_fabricated(monkeypatch) -> None:  # type: ignore
    monkeypatch.setenv("CORTEX_SHADOW_HMAC_KEY", "clave-operador-A")
    d, e = _route()
...
def test_receipts_carry_verifiable_signatures(monkeypatch) -> None:  # type: ignore
    monkeypatch.setenv("CORTEX_SHADOW_HMAC_KEY", "clave-operador-A")
    from babylon60.core.shadow_router import ShadowRouter
