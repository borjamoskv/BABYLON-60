...
class StateSpaceModel:
    ...
    def _discretize_zoh(self) -> Tuple[List[List[float]], List[List[float]]]:
        ...
        """
        A_bar = [
            [(1.0 if i == j else 0.0) + self.delta * self.A[i][j] for j in range(self.state_dim)]
...
def main() -> None:
    ...
    assert len(out[0]) == 4
    print(
        f"[*] Causal-Determinist: SSM Primitive Processed 1000 tokens in O(N). Output shape: ({len(out)}, {len(out[0])})"
    )
