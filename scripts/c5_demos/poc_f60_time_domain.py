...
if __name__ == "__main__":
    ...
    event_1 = CrystallizedEvent(
        payload="Mutación A", causal_proof=CausalWitness(motor.head_hash), lamport_t=motor.current_lamport + 1
    )
    motor.mutate_state(event_1)

    event_2 = CrystallizedEvent(
        payload="Mutación B", causal_proof=CausalWitness(motor.head_hash), lamport_t=motor.current_lamport + 1
    )
