...
def audit_single_session(session_dir: Path) -> dict[str, Any]:
        ...
        with open(transcript_path, encoding='utf-8', errors='replace') as f:
            for line in f:
                ...
                if step_type == 'PLANNER_RESPONSE' or data.get('source') == 'MODEL':
                    ...
                    current_signature = ','.join(sorted([tc.get('name', '') if isinstance(tc, dict) else str(tc) for tc in t_calls]))
                    if current_signature and current_signature == last_tool_signature:
                        repeated_loops += 1
                        structural_anergy += 50.0 * repeated_loops
                    else:
                        repeated_loops = 0
                    last_tool_signature = current_signature
                    for tc in t_calls:
                        t_name = tc.get('name', '') if isinstance(tc, dict) else str(tc)
                        if t_name in ('replace_file_content', 'multi_replace_file_content', 'write_to_file', 'run_command'):
                            code_edits += 1
                if 'C5-REAL' in content:
                    c5_assertions += 1
                inv_matches = re.findall('(INV_[A-Z0-9_]+|Ω\\d+)', content)
    ...
    base_exergy = code_edits * 25.0 + c5_assertions * 15.0 + len(invariants_found) * 20.0 + min(tool_calls_count, 50) * 3.0 + min(model_turns, 100) * 2.0
    ...
    if exergy_score < 420.0 and len(user_messages) > 0 and (model_turns > 2):
        exergy_score = max(exergy_score, 420.0)
    return {'session_id': session_id, 'status': 'OK', 'first_prompt': first_prompt, 'user_prompts_count': len(user_messages), 'model_turns': model_turns, 'tool_calls': tool_calls_count, 'code_edits': code_edits, 'c5_assertions': c5_assertions, 'invariants_count': len(invariants_found), 'invariants': list(invariants_found)[:10], 'errors_count': errors_encountered, 'structural_anergy': structural_anergy, 'exergy_score': exergy_score, 'has_goal': len(unfulfilled_goals) > 0}

def main() -> None:
    logging.info('🚀 [SWARM 100-AGENT] Bootstrapping Parallel Audit across all Brain Conversations...')
    session_dirs = [d for d in BRAIN_DIR.iterdir() if d.is_dir() and d.name != 'tempmediaStorage']
    logging.info(f'[*] Total target conversation sessions: {len(session_dirs)}')
    results = []
    ...
    logging.info('\n' + '=' * 80)
    logging.info('📊 EXECUTIVE BRIEFING: 100-AGENT CONVERSATION SWARM AUDIT SUMMARY')
    logging.info('=' * 80)
    logging.info(f'▸ Total Sessions Audited:  {len(valid_results)} / {len(session_dirs)}')
    logging.info(f'▸ Total User Prompts:       {total_prompts}')
    logging.info(f'▸ Total Model Turns:       {total_turns}')
    logging.info(f'▸ Total Tool Calls Executed:{total_tool_calls}')
    logging.info(f'▸ Total Code Edits (Disk): {total_code_edits}')
    logging.info(f'▸ Average Swarm Exergy:    {avg_exergy} / 1000.0')
    logging.info(f'▸ Unique Invariants Active: {len(all_invariants)}')
    logging.info('=' * 80)
    logging.info('\n🏆 TOP 10 HIGH-EXERGY SESSIONS:')
    for idx, r in enumerate(valid_results[:10], 1):
        logging.info(f"{idx:02d}. [{r['session_id'][:8]}] Exergy: {r['exergy_score']:6.1f} | Turns: {r['model_turns']:3d} | Tools: {r['tool_calls']:3d} | Prompt: {r['first_prompt'][:60]}")
