...
def main() -> None:
    ...
    parser.add_argument("--phase", type=int, choices=[0, 1, 2, 3], help="Ejecutar una fase específica completa (0-3).")
    parser.add_argument("--all", action="store_true", help="Ejecutar la cascada completa de todas las fases (0 a 3).")
    parser.add_argument("--script", type=str, help="Ejecutar un script individual por su nombre de archivo.")
    
    args = parser.parse_args()

    if not (args.all or args.phase is not None or args.script):
        print("=== CORTEX-OMEGA: PIPELINE RUNNER ===")
        print("Uso: python3 scripts/run_pipeline.py [--all] [--phase <0-3>] [--script <nombre>]")
        print("\nFases Disponibles:")
    ...
    for script in scripts_to_run:
        ...
        if args.all and script in ["01_cdp_transducer.py", "22_iter_5000.py", "23_iter_ultrathink.py", "30_audit_loop.py"]:
            print(f"⏩ [SKIP] Saltando {script} en cascada general para evitar bloqueos/esperas de puerto o bucles infinitos.")
            continue
