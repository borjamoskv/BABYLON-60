...
def get_db_connection(path: Path=DB_PATH) -> sqlite3.Connection:
    conn = babylon60.database.core.connect_sync(path)
...
def execute_pulse() -> dict[str, Any]:
    ...
    for ext in ['*.py', '*.rs', '*.ts', '*.md']:
        for f in PROJECT_ROOT.rglob(ext):
            if '.git' in str(f) or 'node_modules' in str(f) or '.venv' in str(f) or ('.cortex' in str(f)) or ('anvil_yung' in str(f)) or ('target' in str(f)) or ('tests/' in str(f)) or f.name.endswith('_primitives.py'):
                continue
            try:
