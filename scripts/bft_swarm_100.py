...
async def main() -> None:
    logging.info('🔋 Igniting BFT Swarm Mitosis: 100 Concurrent Agents...')
    if not DB_PATH.exists():
        logging.info('❌ Ledger DB does not exist. Run exergy optimizer first.')
        return
    ...
    logging.info('Total Agents : 100')
    logging.info(f'Successes    : {metrics.successes}')
    logging.info(f'Deadlocks    : {metrics.deadlocks} (SQLite WAL contention)')
    logging.info(f'Other Errors : {metrics.failures}')
