...
def check_wayback(url: str) -> None:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if len(data) > 1:
                logging.info(f'[!] Latest captures found for {url}:')
                for row in data[1:]:
                    logging.info(f'  - Timestamp: {row[1]}, URL: {row[2]}, Status: {row[4]}')
            else:
                logging.info(f'[✓] No captures found for {url} via Wayback Machine.')
