...
def main() -> None:
    ...
    if not run_cmd(
        "Ruff Format (Python)", [sys.executable, "-m", "ruff", "format", "."]
    ):
        # Fallback to ruff command directly if sys.executable -m ruff fails
        if not run_cmd("Ruff Format (Global)", ["ruff", "format", "."]):
            success = False
