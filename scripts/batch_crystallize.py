...
def batch_crystallize(matrices: list[dict[str, str]]) -> None:
    """
    [L3] Σ13 - Batch Crystallize
    Estructura y escribe todas las reglas o matrices YAML en un solo pulso físico,
    inyectando el hash CORTEX_TAINT.
    """
    ontology_dir = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "cortex", "ontology"
    )
