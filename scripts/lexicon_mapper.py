...
def main() -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        base_nodes = conn.execute(
            "SELECT concept_hash, canonical_name FROM lexicon_nodes WHERE canonical_name IN ('ACTION', 'ENTITY', 'EXERGY', 'ANERGY', 'BYZANTINE_FAULT_TOLERANCE', 'TRUTH_C5')"
        ).fetchall()
        edges = conn.execute("SELECT source_hash, target_hash, relation_type FROM lexicon_edges").fetchall()
        asts = conn.execute(
            "SELECT concept_hash, canonical_name FROM lexicon_nodes WHERE canonical_name LIKE 'AST::%' ORDER BY lamport_t LIMIT 3"
        ).fetchall()
        invs = conn.execute(
            "SELECT concept_hash, canonical_name FROM lexicon_nodes WHERE canonical_name LIKE 'INVARIANT::%' ORDER BY lamport_t LIMIT 3"
        ).fetchall()
