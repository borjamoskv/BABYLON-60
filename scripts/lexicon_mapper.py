...
def main() -> None:
    ...
    if not db_path.exists():
        logging.info('FATAL: cortex_lexicon.db not found.')
        return
    with sqlite3.connect(db_path) as conn:
        ...
        logging.info('### Mapeo Topológico del DAG Criptográfico')
        logging.info(f'**Total Nodos Físicos:** {total_nodes}')
        logging.info(f'**Total Aristas Causales:** {total_edges}')
        logging.info('\n```mermaid')
        logging.info('graph TD')
        logging.info('    classDef base fill:#2B3BE5,stroke:#0A0A0A,stroke-width:2px,color:#fff;')
        logging.info('    classDef ast fill:#0A0A0A,stroke:#2B3BE5,stroke-width:1px,color:#fff;')
        logging.info('    classDef inv fill:#E52B2B,stroke:#0A0A0A,stroke-width:1px,color:#fff;')
        for h, name in base_nodes:
            logging.info(f'    {h[:8]}["{name}"]:::base')
        for h, name in asts:
            logging.info(f'    {h[:8]}["{name}"]:::ast')
        for h, name in invs:
            logging.info(f'    {h[:8]}["{name}"]:::inv')
        for src, tgt, rel in edges:
            logging.info(f'    {src[:8]} -->|{rel}| {tgt[:8]}')
        logging.info('```')
