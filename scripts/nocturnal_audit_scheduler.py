...
def execute_audit_iteration(iteration_num: int) -> dict[str, Any]:
    ...
    logging.info('\n==================================================')
    logging.info(f'⚡ NOCTURNAL AUDIT ITERATION #{iteration_num} | {timestamp}')
    logging.info('==================================================')
    ...
    with open(LOG_FILE, 'a', encoding='utf-8') as f:
        f.write(log_entry)
    return {'iteration': iteration_num, 'timestamp': timestamp, 'status': status, 'planes_passed': [align_ok, sync_ok, exergy_ok, pytest_ok, secret_ok].count(True)}
...
def main():
    ...
    for i in range(1, max_iterations + 1):
        ...
        if res['status'] != 'SUCCESS':
            logging.info(f'[-] AUDIT FAILURE at iteration #{i}. Halting.')
            sys.exit(1)
