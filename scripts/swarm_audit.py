...
async def main() -> None:
    ...
    if not target_file.exists():
        logging.info('target_conversations.txt not found')
        return
    with open(target_file) as f:
        conv_ids = [line.strip() for line in f if line.strip()]
    logging.info(f'Spawning {len(conv_ids)} audit agents...')
    ...
    with open(report_path, 'w') as f:
        ...
        for res in results:
            f.write(res)
    logging.info(f'Audit complete. Report generated at {report_path}')
