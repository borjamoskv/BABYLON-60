...
def levenshtein(a: list[str], b: list[str]) -> int:
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            d[i, j] = min(float(d[i - 1, j]) + 1, float(d[i, j - 1]) + 1, float(d[i - 1, j - 1]) + (0 if a[i - 1] == b[j - 1] else 1))
    return int(d[n, m])
