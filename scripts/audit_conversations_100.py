...
async def audit_agent_task(agent_id: int, uuid_dir: Path) -> None:
    ...
    if success:
        logging.info(f'[🟢] Agent {agent_id:03d} audited {uuid_dir.name[:8]}: {score:.1f}/1000.0')
    else:
        logging.info(f'[🔴] Agent {agent_id:03d} FAILED to log {uuid_dir.name[:8]}')

async def main() -> None:
    logging.info('🔋 Igniting Cognitive Audit Swarm: 100 Agents...')
    if not BRAIN_DIR.exists():
        logging.info('❌ Brain dir not found.')
        return
    ...
    logging.info(f'Targeting {len(target_dirs)} conversations for maximum exergy audit without crystallization...')
    tasks = []
