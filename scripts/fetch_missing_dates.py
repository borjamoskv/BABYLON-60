...
def main() -> None:
    ...
    if not dataset_path.exists():
        logging.info('ERROR: dataset.json not found.', flush=True)
        return
    rows = json.loads(dataset_path.read_text(encoding='utf-8'))
    ncts = [r['nct_id'] for r in rows]
    logging.info(f'Loaded {len(rows)} trials from dataset.json', flush=True)
    study_dates: dict[str, dict[str, str | None]] = {}
    for i in range(0, len(ncts), 200):
        batch = ncts[i:i + 200]
        params = {'filter.ids': ','.join(batch), 'fields': 'NCTId,StartDateStruct,CompletionDateStruct,PrimaryCompletionDateStruct', 'pageSize': '200'}
        url = f'{V2}?{urllib.parse.urlencode(params)}'
        logging.info(f'Fetching batch {i // 200 + 1}/{len(ncts) // 200 + 1}...', flush=True)
        try:
            data = _get_api(url)
            ...
            logging.info(f'Warning: batch failed: {e}', flush=True)
        time.sleep(0.5)
    logging.info(f'Fetched dates for {len(study_dates)} studies.', flush=True)
    updated_count = 0
    for row in rows:
        nct = row['nct_id']
    ...
    logging.info(f'Successfully updated dataset.json: {updated_count} rows updated.', flush=True)
