...
class LexiconBFTActor:
    ...
    async def start(self) -> None:
        # Single writer pattern running in a separate thread to not block event loop
        asyncio.create_task(asyncio.to_thread(self._sync_writer))

    def _sync_writer(self) -> None:
        # INV_BFT_02: WAL mode and busy_timeout=5000ms
        with sqlite3.connect(self.db_path, timeout=5.0) as conn:
            ...
            while self.running or not self.queue.empty():
                try:
                    # Non-blocking get for the thread
                ...
                # BFT rule: get next lamport from disk
                cur = conn.cursor()
                ...
                # Incremental anchors - commit per operation or batch. We commit per op for BFT strictness
                conn.commit()
                self.ops += 1
