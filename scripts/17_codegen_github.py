...
DOMAINS = [
    ...
    "DEPLOYMENT", "CODE_SCANNING_ALERT", "DISCUSSION", "REPOSITORY", "PACKAGE"
]

CATEGORIES = [
    "Mutación Autónoma", "Destrucción Anergía", "Mitosis Celular", "Bucle Adversarial",
    "Bloqueo Termodinámico", "Bypass Causal", "Rechazo Soberano", "Contención Epistémica",
    "Causalidad Base-60", "Ruteo Multidimensional"
]

ACTION_VERBS = [
    "Transducir", "Aniquilar", "Cristalizar", "Falsar", "Confrontar",
    "Auditar", "Interceptar", "Purgar", "Sintetizar", "Aislar"
]
...
def save_to_sqlite(primitives, db_path):
    ...
    for p in primitives:
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (p["id"], p["domain"], p["archetype_id"], p["name"], p["description"], p["taint_hash"], p["timestamp"]))
