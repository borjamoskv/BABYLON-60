...
if command -v lean >/dev/null 2>&1; then
    ...
    echo "[i] Note: Lean 4 binary ('lean' / 'lake') not installed on local host environment."
    echo "[i] Static axiom check passed."
    echo "[i] To install Lean 4 locally for full formal elaboration, run:"
