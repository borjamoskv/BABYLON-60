...
def main():
    ...
    logger.info(
        f"📊 Objetivo: {TOTAL_TARGET:,} transacciones BFT en {num_chunks} bloques de {CHUNK_SIZE:,} txs cada uno."
    )
    ...
    for idx in range(1, num_chunks + 1):
        ...
        logger.info(
            f"  ├── Chunk #{idx:02d}: {rep['tx_count']:,} txs en {rep['elapsed_sec']}s ({rep['throughput_tx_s']:,} tx/s)"
        )
