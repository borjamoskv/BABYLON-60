...
ISOMORFISMOS_PATH = os.path.expandvars(
    os.environ.get(
        "CORTEX_ISOMORFISMOS_PATH", os.path.join(VAULT_DIR, "isomorfismos_cruzados_1000_primitivas.yaml.enc")
    )
)
...
def bootstrap_cortex() -> None:
    ...
    for path in [MATRIZ_PATH, ISOMORFISMOS_PATH]:
        if not os.path.exists(path):
            print(
                f"\x1b[1;31m[CORTEX APOPTOSIS]\x1b[0m Essential Config Missing: {path}. Causal-Determinist Fail-Fast.",
                file=sys.stderr,
            )
            sys.exit(1)
