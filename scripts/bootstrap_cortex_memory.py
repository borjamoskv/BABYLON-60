...
def bootstrap_cortex() -> None:
    logging.info('[CORTEX] Iniciando bootstrap de persistencia de base de datos...')
    if not VAULT_KEY:
        logging.info('\x1b[1;31m[CORTEX APOPTOSIS]\x1b[0m CORTEX_VAULT_KEY is missing. C5-REAL Fail-Fast.', file=sys.stderr)
        sys.exit(1)
    master_ledger = os.path.join(_BASE, 'bft', 'master_ledger.db')
    if os.path.exists(master_ledger) or os.path.exists(DB_PATH):
        logging.info(f'\x1b[1;31m[CORTEX APOPTOSIS]\x1b[0m Freezing Lock: Ledger {master_ledger} or {DB_PATH} already exists. Cannot mutate historical RO state.', file=sys.stderr)
        sys.exit(1)
    for path in [MATRIZ_PATH, ISOMORFISMOS_PATH]:
        if not os.path.exists(path):
            logging.info(f'\x1b[1;31m[CORTEX APOPTOSIS]\x1b[0m Essential Config Missing: {path}. C5-REAL Fail-Fast.', file=sys.stderr)
            sys.exit(1)
    ...
    conn.commit()
    conn.close()
    logging.info('[CORTEX] SQLite persistida exitosamente.')
    logging.info(f'[CORTEX] Nodos Primitivos (L1) instanciados: {primitives_inserted}')
    logging.info(f'[CORTEX] Enlaces Isomorfos (L2) instanciados: {isomorphisms_inserted}')
