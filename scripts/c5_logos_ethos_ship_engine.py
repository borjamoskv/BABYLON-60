...
class PhysicalMembraneState:
    ...
    def __init__(self, state_type: str, payload_hash: str, lamport_clock: int) -> None:
        ...
        if state_type == 'C4_Simulated_Buffer':
            raise RuntimeError('[SIGKILL_State_Purge] C4-SIM state rejected by Causal-Determinist physical membrane during SHIP.')
        ...
        for prev_h, payload, clock, agent, taint_h in rows:
            if prev_h != expected_prev:
                raise AssertionError(
                    f'[SIGKILL_State_Purge] Chain break detected! '
                    f'Expected prev {expected_prev}, got {prev_h}'
                )
            # Lamport strict monotonicity verification
            if clock <= prev_lamport and prev_lamport != 0:
                raise AssertionError(
                    f'[SIGKILL_State_Purge] Lamport monotonicity violated! '
                    f'clock={clock} <= prev={prev_lamport}'
                )
            calc_t = self._compute_taint(prev_h, payload, clock, agent)
            if taint_h != calc_t:
                raise AssertionError(
                    f'[SIGKILL_State_Purge] Taint hash mismatch! '
                    f'Expected {calc_t}, got {taint_h}'
                )
