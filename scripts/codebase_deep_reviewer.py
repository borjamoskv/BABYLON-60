...
def main() -> None:
    ...
    logging.info('=== CODEBASE DEEP REVIEW SUMMARY ===')
    logging.info(f'Total Python Files: {total_files}')
    logging.info(f'Total Lines of Code: {total_lines}')
    logging.info(f'Files with Syntax Errors: {len(syntax_fails)}')
    logging.info(f'Files with Invariant/Quality Warnings: {len(warns)}')
    logging.info('-' * 50)
    for r in warns:
        flag = '🔴' if r['status'] == 'FAIL' else '🟡'
        logging.info(f"{flag} {r['path']} ({r['lines']} lines)")
