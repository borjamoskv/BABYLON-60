    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ExceptHandler):
            if node.type is None:
                ...
                # Exception check for BFT invariants (INV_C5_19)
                if "bft" in str(rel_path) or "ledger" in str(rel_path):
                    issues.append(
                        f"Line {node.lineno}: Broad 'except {node.type.id}:' in BFT module (INV_C5_19 violation)."
                    )
            ...
            for child in ast.walk(node):
                if isinstance(child, ast.Call):
                    if (
                        ...
                        and child.func.attr in ("connect", "execute")
                        and isinstance(child.func.value, ast.Name)
                        and child.func.value.id == "sqlite3"
                    ):
                        issues.append(
                            f"Line {child.lineno}: Synchronous sqlite3 call inside async def {node.name} (INV_BFT_02 violation)."
                        )
