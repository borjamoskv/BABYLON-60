...
def run_step(name: str, cmd: list[str]) -> bool:
    ...
    try:
        res = subprocess.run(
            cmd, env=env_vars, check=True, capture_output=True, text=True
        )
...
def main():
    ...
    steps = [
        ...
        (
            "10_codegen_constants",
            [".venv/bin/python", "scripts/10_codegen_constants.py"],
        ),
        ...
        (
            "16_codegen_primitives",
            [".venv/bin/python", "scripts/16_codegen_primitives.py"],
        ),
        ("17_codegen_github", [".venv/bin/python", "scripts/17_codegen_github.py"]),
        ("Go Auto-Format", ["gofmt", "-w", "./primitives"]),
        ("Python Auto-Format", ["ruff", "format", "."]),
        (
            "Tauri Rust Auto-Format",
            ["cargo", "fmt", "--manifest-path", "src-tauri/Cargo.toml"],
        ),
        (
            "Strike-RS Rust Auto-Format",
        ...
        (
            "Strike-RS Rust Clippy",
            [
                "cargo",
                "clippy",
                "--manifest-path",
                "strike-rs/Cargo.toml",
                "--",
                "-D",
                "warnings",
            ],
        ...
        (
            "Strike-RS Rust Tests",
            ["cargo", "test", "--manifest-path", "strike-rs/Cargo.toml"],
        ),
    ...
    if failed:
        ...
        print(
            f"✅ PIPELINE COMPLETADO EXITOSAMENTE. Tiempo total: {elapsed_global:.4f}s"
        )
        sys.exit(0)
