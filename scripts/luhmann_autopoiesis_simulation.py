...
class LuhmannAutopoiesisSimulation:
    ...
    def __init__(self, seed: int = 42, steps: int = 500) -> None:
        ...
        self.env_state: Dict[str, Any] = {
            "citizens": [{"id": f"citizen_{i}", "frustration": 0.0, "has_cert": random.random() > 0.3} for i in range(100)],
            "total_attempts": 0,
            "dissipated_atp": 0.0
        }
    ...
    def run(self) -> Dict[str, Any]:
        for step in range(self.steps):
            ...
            if citizen["has_cert"] and random.random() > 0.4:  # 60% chance signature applet works
                ...
                self.system_state["internal_queue"].append({
                    "dossier_id": dossier_id,
                    "action": "SUBSANACION_CHECK"
                })
            else:
        ...
        results = {
            ...
            "coupling_success_rate": round(self.successful_couplings / self.env_state["total_attempts"] if self.env_state["total_attempts"] > 0 else 0, 5)
        }
