    ...
    def run(self) -> dict[str, Any]:
        for _step in range(self.steps):
            ...
            if citizen['has_cert'] and random.random() > 0.4:
                ...
                self.system_state['dossiers_db'][dossier_id] = {'owner': citizen['id'], 'status': 'INITIATED', 'step_count': 1}
        ...
        results = {'steps_simulated': self.steps, 'successful_couplings': self.successful_couplings, 'failed_couplings': self.failed_couplings, 'total_attempts': self.env_state['total_attempts'], 'autopoietic_loops_executed': self.system_state['loops_run'], 'environment_frustration_mean': round(avg_frustration, 5), 'environment_dissipated_atp': round(self.env_state['dissipated_atp'], 5), 'system_registered_dossiers': total_dossiers, 'system_entropy': system_entropy, 'coupling_success_rate': round(self.successful_couplings / self.env_state['total_attempts'] if self.env_state['total_attempts'] > 0 else 0, 5)}
        ...
        if AmendmentLedgerClass is not None:
            try:
                ...
                entry = ledger.append(payload=results, causal_taint='borjamoskv:luhmann_autopoiesis', agent_id='borjamoskv')
                ledger_seq = entry.seq
                ledger_event_id = entry.id
                ledger_entry_hash = entry.entry_hash
                ledger_prev_hash = entry.prev_hash
            except (RuntimeError, ValueError, OSError) as e:
                logging.info(f'[-] Ledger write failed: {e}')
                import traceback
                traceback.print_exc()
        else:
            logging.info('[-] AmendmentLedger not imported')
        output = {'metadata': {'author': 'borjamoskv', 'timestamp': '2026-07-17T19:25:00Z', 'cortex_taint': f'borjamoskv:luhmann_autopoiesis:{results_hash[:16]}'}, 'results': results, 'results_hash': results_hash, 'ledger_metadata': {'seq': ledger_seq, 'event_id': ledger_event_id, 'entry_hash': ledger_entry_hash, 'prev_hash': ledger_prev_hash}}
        return output
