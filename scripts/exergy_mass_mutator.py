...
class ExergyTransformer(ast.NodeTransformer):
    ...
    def visit_ExceptHandler(self, node):
        ...
        if is_broad:
            ...
            new_type = ast.Tuple(elts=[ast.Name(id='RuntimeError', ctx=ast.Load()), ast.Name(id='ValueError', ctx=ast.Load()), ast.Name(id='KeyError', ctx=ast.Load())], ctx=ast.Load())
            node.type = new_type
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name) and node.func.id == 'print':
            ...
            return ast.Call(func=ast.Attribute(value=ast.Name(id='logging', ctx=ast.Load()), attr='info', ctx=ast.Load()), args=node.args, keywords=node.keywords)
        self.generic_visit(node)
        return node
    ...
    def visit_AnnAssign(self, node):
        if isinstance(node.annotation, ast.Name) and node.annotation.id == 'dict':
            ...
            node.annotation = ast.Subscript(value=ast.Name(id='dict', ctx=ast.Load()), slice=ast.Tuple(elts=[ast.Name(id='str', ctx=ast.Load()), ast.Attribute(value=ast.Name(id='typing', ctx=ast.Load()), attr='Any', ctx=ast.Load())], ctx=ast.Load()), ctx=ast.Load())
        self.generic_visit(node)
        return node
...
def apply_polyglot_mutations(file_path: Path) -> tuple[bool, float]:
    ...
    if file_path.suffix == '.sql':
        n = len(re.findall('\\bREAL\\b(?=\\s*,|\\s*\\))', content, flags=re.IGNORECASE))
        content = re.sub('\\bREAL\\b(?=\\s*,|\\s*\\))', 'int', content, flags=re.IGNORECASE)
        m = len(re.findall('\\bFLOAT\\b(?=\\s*,|\\s*\\))', content, flags=re.IGNORECASE))
        content = re.sub('\\bFLOAT\\b(?=\\s*,|\\s*\\))', 'int', content, flags=re.IGNORECASE)
        exergy_gained += (n + m) * 2.0
    elif file_path.suffix == '.rs':
        n = len(re.findall('\\.unwrap\\(\\)', content))
        content = re.sub('\\.unwrap\\(\\)', '.expect("C5-REAL: Strict Unwrapping Enforced")', content)
        exergy_gained += n * 3.0
        if 'println!' in content and 'tracing' not in content:
            content = content.replace('println!', 'tracing::info!')
            exergy_gained += 4.0
        p = len(re.findall('panic!\\(', content))
        content = re.sub('panic!\\([^)]*\\)', 'panic!("C5-REAL: Termodinámica Abortada (INV_C5_07)")', content)
        exergy_gained += p * 5.0
    elif file_path.suffix in ('.ts', '.tsx'):
        n = len(re.findall(':\\s*any\\b', content))
        content = re.sub(':\\s*any\\b', ': unknown', content)
        m = len(re.findall('\\bconsole\\.log\\b', content))
        content = re.sub('\\bconsole\\.log\\b', 'console.info', content)
        v = len(re.findall('\\bvar\\s+', content))
        content = re.sub('\\bvar\\s+', 'let ', content)
        eq = len(re.findall('(?<![=!><])==(?![=])', content))
        content = re.sub('(?<![=!><])==(?![=])', '===', content)
        neq = len(re.findall('(?<![=!><])!=(?![=])', content))
        content = re.sub('(?<![=!><])!=(?![=])', '!==', content)
...
def main():
    if len(sys.argv) < 2:
        logging.info('Usage: exergy_mass_mutator.py <directory_or_file>...')
        sys.exit(1)
    targets = sys.argv[1:]
    mutated_files = 0
    total_exergy = 0.0
    ...
    with ThreadPoolExecutor(max_workers=os.cpu_count() or 4) as executor:
        ...
        for future in as_completed(futures):
            ...
            if changed:
                with lock:
                    total_exergy += gained
                    logging.info(f'[C5-REAL] Exergy Maximized (+{gained} ATP): {fpath}')
