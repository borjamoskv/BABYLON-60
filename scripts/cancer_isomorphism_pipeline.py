...
def simulate_boolean_network(
    G: nx.DiGraph, initial_state: dict[str, int], steps: int = 30, perturbed_nodes: dict[str, int] | None = None
...
def main() -> None:
    ...
    assert caida_atractor > UMBRAL_FALSACION, (
        f"[ERROR C5-REAL] La intervención teórica solo alcanzó {caida_atractor:.1f}% de colapso. No supera el umbral crítico ({UMBRAL_FALSACION}%). Hipótesis REFUTADA. No derivar a ensayo In-Vitro."
    )
    print(
        "[ÉXITO C5-REAL] Hipótesis topológica VALIDAD. La intervención supera el umbral termodinámico requerido para someterse a ensayo In-Vitro (CRISPR/Cas9)."
    )
