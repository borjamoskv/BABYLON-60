...
def create_crypto_vectors():
    ...
    cbor_bytes = cbor2.dumps(event_basic, canonical=True)
    cbor_hex = cbor_bytes.hex()
    sha3_hash = hashlib.sha3_256(cbor_bytes).hexdigest()

    event_basic["_cbor_hex"] = cbor_hex
    event_basic["_sha3_256"] = sha3_hash
    
    with open(f"{vectors_dir}/event_basic.json", "w") as f:
        json.dump(event_basic, f, indent=2)
    print(f"Crypto vectors written. SHA3: {sha3_hash}")
    
def test_replay_corruption():
