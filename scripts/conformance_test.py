...
def run_cmd(cmd: list[str], env: dict[str, str] | None=None) -> str:
    ...
    if result.returncode != 0:
        logging.info(f"Error executing {' '.join(cmd)}:\n{result.stderr}\n{result.stdout}")
...
def check_wheel_contents() -> None:
    ...
    if not wheels:
        logging.info('ERROR: No wheels found in dist/')
        sys.exit(1)
    ...
    for f in files:
        ...
        if root_dir in invalid_roots:
            logging.info(f"ERROR: Wheel contains invalid root directory '{root_dir}' -> {f}")
            sys.exit(1)
    logging.info('Wheel verification PASSED. The distribution is clean.')
...
def create_crypto_vectors() -> None:
    ...
    event_basic: dict[str, object] = {'event_id': 'test_event_1', 'timestamp': 1690000000000, 'payload': {'action': 'ping', 'data': 'pong'}}
    ...
    logging.info(f'Crypto vectors written. SHA3: {sha3_hash}')
...
def test_replay_corruption() -> None:
    ...
    ledger.conn.execute('INSERT INTO state_log (mutation_hash, agent_id, payload, ts, causal_taint) VALUES (?, ?, ?, ?, ?)', (m_hash, mutation.agent_id, canonicalize_cbor(mutation.payload), mutation.timestamp, mutation.causal_taint))
    ledger.conn.commit()
    if not ledger.audit_integrity():
        logging.info('Initial integrity check failed!')
        sys.exit(1)
    ...
    if ledger.audit_integrity():
        logging.info('ERROR: Corruption was NOT detected!')
        sys.exit(1)
    logging.info('Replay verification PASSED. Corruption correctly triggers false on audit_integrity().')
