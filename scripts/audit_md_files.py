...
def audit_file(file_path: Path) -> dict[str, Any]:
    ...
    for match in link_pattern.finditer(content):
        ...
        if link_target.startswith('http://') or link_target.startswith('https://') or link_target.startswith('mailto:') or link_target.startswith('#'):
            continue
    ...
    word_count = len(re.findall('\\w+', content))
    code_block_count = len(re.findall('```', content)) // 2
    status = 'OK'
    issues = []
    if size_bytes == 0:
        status = 'EMPTY'
    ...
    return {'path': str(rel_path), 'size_bytes': size_bytes, 'lines': len(lines), 'words': word_count, 'h1_count': h1_count, 'links_count': links_found, 'broken_links': broken_links, 'code_blocks': code_block_count, 'has_c5': has_c5, 'has_cortex_taint': has_cortex_taint, 'has_invariants': has_inv, 'status': status, 'issues': issues}
...
def main() -> None:
    ...
    logging.info('=== MOSKV-1 APEX MD AUDIT SUMMARY ===')
    logging.info(f'Total Markdown Files: {total_files}')
    logging.info(f'Total Disk Volume: {total_bytes / 1024:.2f} KB ({total_lines} lines, {total_words} words)')
    logging.info(f'Files with Issues/Warnings: {len(warn_files)}')
    logging.info(f'Empty Files: {len(empty_files)}')
    logging.info('-' * 50)
    for r in results:
        flag = '🔴' if r['status'] == 'EMPTY' else '🟡' if r['issues'] else '🟢'
        logging.info(f"{flag} {r['path']} ({r['lines']} lines, {r['words']} words, {r['size_bytes']} bytes)")
