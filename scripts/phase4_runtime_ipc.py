...
def analyze_ipc_and_runtime(target_dir: str) -> None:
    ...
    exclude_dirs = {".venv", "node_modules", "__pycache__", ".git", "target", "out", "dist", "bin", "obj", ".mypy_cache", ".pytest_cache", ".ruff_cache"}
    ...
    for root, dirs, files in os.walk(target_dir):
        ...
        for file in files:
            ...
            try:
                ...
                logging.error(f"Error estructural parseando {rel_path}: {e}. Ejecutando purga SIGKILL.")
                os.kill(os.getpid(), signal.SIGKILL)
