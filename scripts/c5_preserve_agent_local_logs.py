...
def harvest_logs() -> None:
    logging.info('[*] ULTRATHINK P0: Harvesting local Agent Code CLI session logs from ~/.agent_persist/...')
    TARGET_LOGS_DIR.mkdir(parents=True, exist_ok=True)
    if not SOURCE_AGENT_DIR.exists():
        logging.info('[-] Source directory ~/.agent_persist/projects does not exist.')
        return
    jsonl_files = list(SOURCE_AGENT_DIR.rglob('*.jsonl'))
    logging.info(f'[+] Found {len(jsonl_files)} local Agent Code session logs.')
    catalog_entries = []
    ...
    with open(INDEX_FILE, 'w', encoding='utf-8') as f:
        f.write('\n'.join(md_lines))
    logging.info(f'[+] Successfully harvested and indexed {len(catalog_entries)} local Agent Code logs.')
