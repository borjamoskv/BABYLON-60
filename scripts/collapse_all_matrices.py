...
if not os.path.exists(agents_md_path):
    logging.info('AGENTS.md no existe.')
    exit(1)
...
if not matrices:
    logging.info('[-] FATAL: No se encontraron matrices flotantes.')
else:
    ...
    if res.returncode == 0:
        logging.info(f'[+] COMMIT SUCCESS: {res.stdout.strip()}')
    else:
        logging.info(f'[-] COMMIT NO NECESARIO: {res.stdout.strip()} {res.stderr.strip()}')