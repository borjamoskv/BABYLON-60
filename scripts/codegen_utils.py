...
CODEGEN_CONFIGS: Dict[str, Dict[str, Any]] = {
    ...
    "Neuro": {
        ...
        "go_sim": [
            ...
            "\tvec.ActionTorque[i] = vec.AttentionWeight[i] * (float64((id.Code+uint64(i))%10) + 1.0)",
    ...
    "Kimi": {
        ...
        "go_sim": [
            ...
            "\tvec.PromptSize[i] = math.Max(0.0, vec.PromptSize[i] + float64((id.Code+uint64(i))%50) - 25.0)",
            "\tvec.CacheHits[i] = vec.CacheHits[i]*0.99 + 0.01*float64((id.Code+uint64(i))%2)",
            "\tvec.BftValidationCount[i] += uint64((id.Code+uint64(i))%5)",
            "}",
        ],
        "rust_sim": [
            "for i in 0..64 {",
