...
DPO_REGISTRY = {'X_Twitter': 'privacy@x.com', 'Meta_Facebook_Instagram': 'privacy@support.facebook.com', 'Google_Alphabet': 'data-protection-office@google.com', 'LinkedIn': 'dpo-team@linkedin.com', 'TikTok': 'privacy@tiktok.com', 'Reddit': 'privacy@reddit.com', 'Telegram': 'support@telegram.org', 'Discord': 'privacy@discord.com', 'OpenAI': 'dsar@openai.com', 'Microsoft': 'privacydpo@microsoft.com'}
...
def generate_erasure_dossier(operator_identity: str='Borja Moskv', operator_email: str='operator@cortex.local') -> dict[str, str]:
    ...
    for platform, dpo_email in DPO_REGISTRY.items():
        ...
        letter = GDPR_TEMPLATE.format(operator_identity=operator_identity, operator_email=operator_email, platform_name=platform, dpo_email=dpo_email, date_iso=now_str, cortex_taint=taint, sha256_proof=proof)
...
if __name__ == '__main__':
    logging.info('[C5-REAL] Generating GDPR Art. 17 Erasure Dossiers...')
    results = generate_erasure_dossier()
    logging.info(f'🟢 Generated {len(results)} formal erasure dossiers in gdpr_dossiers/')