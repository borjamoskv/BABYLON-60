...
def run_autocognition_audit() -> None:
    logging.info('[*] C5-REAL AUTOCOGNITION-Ω: Ingesting active session transcript...')
    ...
    logging.info(f'[+] Transcript Steps: {total_steps} | Total Approx Tokens: {total_tokens}')
    logging.info(f'[+] Structured Tokens: {structured_tokens} ({exergy_ratio * 100:.1f}%) | Narrative Tokens: {narrative_tokens} ({anergy_ratio * 100:.1f}%)')
    logging.info(f'[+] Command Repeat Index: {cmd_repeat_index} ({repeat_commands} duplicated shell executions)')
    logging.info(f'[+] Tool Errors: {tool_errors}')
    ...
    with open(AUDIT_FILE, 'w', encoding='utf-8') as f:
        f.write(audit_yaml)
    logging.info(f'[+] Crystallized Autocognitive Audit Report at: {AUDIT_FILE}')
    logging.info('[*] Sealing Autocognition crystal into Memory Vault (`cortex_memory.db`)...')
    ...
    conn.commit()
    conn.close()
    logging.info('[+] Successfully sealed AUTOCOGNITION-OMEGA audit inside Memory Vault.')
