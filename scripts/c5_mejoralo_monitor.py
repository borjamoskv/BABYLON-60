...
LEDGER_TABLES = ['master_ledger', 'ledger_entries', 'state_log', 'ledger', 'jetsam_async_ledger', 'ontology', 'L1_primitive_nodes', 'L2_isomorphism_edges', 'L3_inference_cache', 'voice_turns', 'audit_ledger', 'ttft_log', 'throughput_log', 'autonomic_daemon_log', 'ttft_metrics']

def _git(args: list[str]) -> str:
    for _attempt in range(2):
        result = subprocess.run(['git', '-c', 'commit.gpgsign=false', *args], cwd=str(ROOT_DIR), capture_output=True, text=True)
...
def audit_git_entropy() -> dict[str, str | int | list[str]]:
    ...
    return {'head': head, 'branch': branch, 'commits': commit_count, 'last_tag': last_tag[0] if last_tag else 'NONE', 'dirty_count': len(dirty_files), 'dirty_files': dirty_files}
...
def kinetic_purge_protocol() -> None:
    logging.info('[KINETIC PURGE] Ejecutando Brutalismo Cinético (INV_C5_20)...')
...
def c5_real_colapso() -> None:
    logging.info('=' * 60)
    logging.info(' MOSKV-1 APEX — C5-REAL STATE MONITOR (MEJORALO)')
    logging.info('=' * 60)
    kinetic_purge_protocol()
    git_report = audit_git_entropy()
    logging.info(f"\n[GIT] HEAD: {git_report['head']} | Branch: {git_report['branch']}")
    logging.info(f"[GIT] Commits: {git_report['commits']} | Tag: {git_report['last_tag']}")
    logging.info(f"[GIT] Entropía: {git_report['dirty_count']} archivos mutados")
    ...
    for db_name, tables in db_census.items():
        if isinstance(tables, dict) and 'ERROR' not in tables:
            ...
            logging.info(f'  {db_name:30s} → {db_total:>8,} nodos ({len(tables)} tablas)')
        else:
            logging.info(f'  {db_name:30s} → ERROR')
    logging.info(f"  {'TOTAL':30s} → {total_nodes:>8,} nodos")
    ruff_report = audit_ruff()
    logging.info(f"\n[RUFF] Errores: {ruff_report['total_errors']} | Fixable: {ruff_report['fixable']}")
    test_report = audit_tests()
    logging.info(f"[TEST] Archivos de test: {test_report['test_files']} | Estado: {test_report['status']} | Passed: {test_report.get('passed', 0)}")
    status_hash = crystallize_status({'git': git_report, 'bft': db_census, 'ruff': ruff_report, 'tests': test_report})
    logging.info(f'\n[HASH] STATUS.md SHA3-256: {status_hash[:24]}...')
    logging.info('[GIT SENTINEL] Forzando colapso...')
    ...
    logging.info(f'[GIT SENTINEL] Colapso: {final_hash}')
    logging.info('=' * 60)
    logging.info(f' MEJORALO COMPLETADO | {total_nodes:,} nodos BFT | {final_hash}')
    logging.info('=' * 60)
