...
def harvest_logs() -> None:
    logging.info('[*] ULTRATHINK P0: Harvesting local Claude Code CLI session logs from ~/.claude/projects/...')
    TARGET_LOGS_DIR.mkdir(parents=True, exist_ok=True)
    if not SOURCE_CLAUDE_DIR.exists():
        logging.info('[-] Source directory ~/.claude/projects does not exist.')
        return
    jsonl_files = list(SOURCE_CLAUDE_DIR.rglob('*.jsonl'))
    logging.info(f'[+] Found {len(jsonl_files)} local Claude Code session logs.')
    if not jsonl_files:
        return
    catalog_entries = []
