...
def test_ledger_append_and_trace() -> None:
    ...
    n1 = ledger.mut_append_node(
        parent_id=genesis_id, claim="Init ontology spec", payload_hash=core_calc_sha256("payload 1")
    )
    ...
    n2 = ledger.mut_append_node(
        parent_id=n1.node_id, claim="Add verify script", payload_hash=core_calc_sha256("payload 2")
    )
...
def test_ledger_fail_fast_on_missing_parent() -> None:
    ...
    with pytest.raises(ValueError, match="not found in DAG ontology"):
        ledger.mut_append_node(parent_id=fake_parent, claim="Orphan node", payload_hash=core_calc_sha256("dummy"))
...
def test_ledger_fail_fast_on_invalid_claim() -> None:
    ...
    with pytest.raises(ValueError, match="claim must be non-empty and <= 64 chars"):
        ledger.mut_append_node(parent_id=ledger.genesis_id, claim=long_claim, payload_hash=core_calc_sha256("dummy"))
