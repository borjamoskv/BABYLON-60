...
async def _execute_bridge_phase(
    ctx: SovereignContext, phase: Phase, bridge_cmd: str, success_details: dict | None = None
) -> PipelineResult:
    """Helper to execute a bridge command for a specific phase."""
    t0 = time.monotonic()
    try:
        await asyncio.to_thread(ctx.bridge.execute, bridge_cmd)
        ...
        if success_details:
            result.details = success_details
        return result
    except (RuntimeError, ValueError, OSError, ImportError) as e:
        logger.error("%s phase failed: %s", phase.name.capitalize(), e)
...
async def _phase_fabrication(ctx: SovereignContext) -> PipelineResult:
    ...
    return await _execute_bridge_phase(
        ctx,
        Phase.FABRICATION,
        "aether-1",
        {"status": "Aether-1 materialized artifacts successfully"},
    )


async def _phase_orchestration(ctx: SovereignContext) -> PipelineResult:
    """Phase 2 - Keter-omega for multi-cloud readiness."""
    return await _execute_bridge_phase(ctx, Phase.ORCHESTRATION, "keter-omega")
...
async def _phase_security(ctx: SovereignContext) -> PipelineResult:
    """Phase 5 - Run cryptographically verified security scans."""
    t0 = time.monotonic()
...
async def _phase_experience(ctx: SovereignContext) -> PipelineResult:
    ...
    return await _execute_bridge_phase(ctx, Phase.EXPERIENCE, "impactv-1")
