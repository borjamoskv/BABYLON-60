...
class LLMProvider(BaseProvider):
    ...
    def __init__(
        ...
        if is_external and "localhost" not in prov_url and "127.0.0.1" not in prov_url:
            ...
            if (is_hybrid_bft and prov_name == "gemini" and not is_subagent) or allow_external:
                logger.info(f"⚡ [HYBRID-BFT] Allowing external {prov_name.upper()} instantiation.")
            else:
                _mask(prov_name)
                logger.warning(
                    "🛑 [ZERO-NETWORK] Core LLMProvider trapped external instantiation. Forcing local autarchy (Ollama)."
                )
                cfg["provider"] = "ollama"
        ...
        _mask(self._model)
        logger.info("LLM [READY] -> Internal Node Active")
    ...
    async def _execute_completion(
        ...
        try:
            ...
            _mask(self._provider)
            _mask(err_text)
            logger.error("LLM API Failure [%s]: <redacted>", e.response.status_code)
            if wrap_errors:
            ...
            _mask(self._provider)
            logger.error("LLM Parse Error: %s", type(e).__name__)
            if wrap_errors:
                from babylon60.utils.errors import CortexError
    ...
    async def stream(
        ...
        if os.environ.get("CORTEX_LIVE_ROUTING") == "1":
            _get_hud().print(
                "[bold magenta]⚡ COGNITIVE STREAM[/bold magenta] [dim]➜[/dim] Processing stream intent..."
            )
    ...
    def _resolve_model(self, intent: IntentProfile) -> str:
        ...
        if self._intent_model_map:
            ...
            if resolved != self._model:
                logger.debug("LLM meta-route resolved")
            return resolved
        return self._model

    async def _acquire_quota(self) -> None:
        if self._provider not in ["ollama", "lmstudio"]:
            await _get_quota_manager().acquire(tokens=1, fast_reject=False)
