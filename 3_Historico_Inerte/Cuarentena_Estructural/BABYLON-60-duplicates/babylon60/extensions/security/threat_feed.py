...
class ThreatFeedEngine:
    ...
    def __init__(self, data_dir: str = "~/.babylon60", hmac_key: str | None = None) -> None:
        ...
        self._load_custom_signatures()  # type: ignore[attr-defined]  # pyright: ignore[reportAttributeAccessIssue]
    ...
    async def update_daily(self) -> ThreatFeedReport:
        ...
        for feed_name, url in self.FEED_URLS.items():
            ...
            extracted = await self._fetch_feed_signatures(feed_name, url, report)  # type: ignore[attr-defined]
            for sig in extracted:
                if sig["id"] not in existing_ids:
                    new_sigs.append(sig)
                    existing_ids.add(sig["id"])
        if new_sigs:
            self._add_new_signatures(new_sigs)  # type: ignore[attr-defined]
