...
class BCI_Daemon:
    ...
    async def _handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            ...
            if action in self.action_handlers:
                ...
                logging.getLogger(__name__).info(f"[BCI] ⚠️ Unknown action: {action}")
    ...
    async def start(self):
        ...
        logging.getLogger(__name__).info("[BCI] Listening for binary intent. No words. No entropy.")
...
async def mock_handler(instruction: str, payload: str):
    ...
    logging.getLogger(__name__).info(f"      -> Payload size: {len(payload)} bytes")
    logging.getLogger(__name__).info(f"      -> Content chunk: {payload[:50]}...")
