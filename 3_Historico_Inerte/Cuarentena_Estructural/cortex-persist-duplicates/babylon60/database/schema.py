...
CREATE TRIGGER IF NOT EXISTS prevent_diffuse_reasoning_insert
BEFORE INSERT ON facts
FOR EACH ROW
WHEN (NEW.confidence IN ('C1', 'C2') AND NEW.fact_type != 'causal_void')
BEGIN
    SELECT RAISE(ABORT, '[Fail-Fast K1] CAUSAL VIOLATION: Diffuse reasoning (C1/C2) detected. Missing info MUST be modeled as a causal_void (Inv 18 & 19).');
END;
"""
...
CREATE TRIGGER IF NOT EXISTS prevent_compaction_log_update BEFORE UPDATE ON compaction_log
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: compaction_log is append-only'); END;

CREATE TRIGGER IF NOT EXISTS prevent_compaction_log_delete BEFORE DELETE ON compaction_log
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: compaction_log is append-only'); END;
"""
...
CREATE TRIGGER IF NOT EXISTS prevent_replay_admissions_update BEFORE UPDATE ON ledger_replay_admissions
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: ledger_replay_admissions is append-only'); END;

CREATE TRIGGER IF NOT EXISTS prevent_replay_admissions_delete BEFORE DELETE ON ledger_replay_admissions
BEGIN SELECT RAISE(ABORT, 'Immunitas-Omega: ledger_replay_admissions is append-only'); END;
"""
