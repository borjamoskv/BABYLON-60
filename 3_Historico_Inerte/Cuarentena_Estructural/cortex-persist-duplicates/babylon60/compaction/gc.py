    ...
    async def run_gc(self, batch_size: int = 500, force: bool = False) -> dict[str, Any]:
        ...
        async with self.engine.session() as conn:
            ...
            try:
                ...
                # [VEC-0] Run apoptosis sweep for orphaned embeddings
                from babylon60.engine.core.vec_apoptosis import VecApoptosisEngine

                apoptosis = VecApoptosisEngine(conn)
                apoptosis_results = await apoptosis.run_apoptosis()
                for _, count in apoptosis_results.items():
                    stats["deleted_embeddings"] += count
