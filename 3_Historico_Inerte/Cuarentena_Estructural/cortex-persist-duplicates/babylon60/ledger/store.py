...
class LedgerStore:
    ...
    def _init_db(self) -> None:
        with self.tx() as conn:
            self._create_ledger_events_schema(conn)
            self._create_ledger_checkpoints_schema(conn)
            self._create_enrichment_jobs_schema(conn)
            self._create_replay_admissions_schema(conn)
            ...
            self._create_indexes_and_triggers(conn)

    def _create_ledger_events_schema(self, conn: sqlite3.Connection) -> None:
        conn.executescript(
            """
            ...
            CREATE UNIQUE INDEX IF NOT EXISTS idx_ledger_events_prev_hash ON ledger_events(prev_hash);

            CREATE TRIGGER IF NOT EXISTS ledger_events_verify_prev_hash_genesis
            BEFORE INSERT ON ledger_events
            FOR EACH ROW
            WHEN (SELECT COUNT(*) FROM ledger_events) = 0
            BEGIN
                SELECT RAISE(ABORT, 'genesis prev_hash must be GENESIS')
                WHERE NEW.prev_hash IS NULL OR NEW.prev_hash != 'GENESIS';
            END;

            CREATE TRIGGER IF NOT EXISTS ledger_events_verify_prev_hash
            BEFORE INSERT ON ledger_events
            FOR EACH ROW
            WHEN (SELECT COUNT(*) FROM ledger_events) > 0
            BEGIN
                SELECT RAISE(ABORT, 'prev_hash mismatch: does not match current ledger tip')
                WHERE NEW.prev_hash IS NULL OR NEW.prev_hash != (
            ...
            BEFORE UPDATE OF event_id, ts, tool, actor, action, payload_json, prev_hash, hash, correlation_id, trace_id, created_at ON ledger_events
            BEGIN
    ...
    def _create_ledger_checkpoints_schema(self, conn: sqlite3.Connection) -> None:
        conn.executescript(
            """
    ...
    def _create_enrichment_jobs_schema(self, conn: sqlite3.Connection) -> None:
        conn.executescript(
            """
    ...
    def _create_replay_admissions_schema(self, conn: sqlite3.Connection) -> None:
        conn.executescript(
            """
    ...
    def _create_indexes_and_triggers(self, conn: sqlite3.Connection) -> None:
        conn.executescript(
            """
            ...
            BEGIN
                INSERT OR REPLACE INTO ledger_enrichment_jobs (
                    id, job_id, event_id, fact_id, job_type, status,
                    attempts, priority, next_attempt_ts, next_attempt_at,
                    last_error, created_at, updated_at
                )
                VALUES (
                    NEW.id, NEW.job_id, NEW.event_id, NEW.fact_id, NEW.job_type, NEW.status,
                    NEW.attempts, NEW.priority, NEW.next_attempt_ts, NEW.next_attempt_at,
                    NEW.last_error, NEW.created_at, NEW.updated_at
                );
            END;
