...
def verify_bundle(bundle_path: str) -> None:
    ...
    async def _verify():
        console.print(
            f"[bold blue]Starting Independent Cryptographic Verification:[/bold blue] {bundle_path}"
        )
        ...
        try:
            ...
            if not ledger_file.exists() or not pk_file.exists():
                console.print(
                    "[red]✗ ERROR: Invalid bundle format. Missing ledger or public key.[/red]"
                )
                return
            ...
            for row in ledger_data:
                ...
                if expected_audit_id != row["audit_id"]:
                    console.print(
                        f"[red]✗ ROW TAMPERING DETECTED: Audit ID {row['audit_id']}[/red]"
                    )
                    return
            ...
            for prev_hash, signature, external_anchor, batch_rows in batches:
                ...
                if prev_hash != expected_prev_hash:
                    console.print(
                        f"[red]✗ CHAIN BROKEN at batch starting with {batch_rows[0]['audit_id']}[/red]"
                    )
                    return
                ...
                try:
                    ...
                    console.print(
                        f"[red]✗ INVALID SIGNATURE for batch starting with {batch_rows[0]['audit_id']}[/red]"
                    )
                    return
                ...
                if external_anchor:
                    ...
                    if rekor_uuid:
                        ...
                        if not rekor_data:
                            console.print(
                                f"[yellow]⚠ Rekor verification failed for UUID {rekor_uuid}[/yellow]"
                            )
