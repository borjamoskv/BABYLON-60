...
class ImportResolutionLedger:
    ...
    @staticmethod
    def _get_previous_hash(lines: list[str]) -> tuple[str, list[str]]:
        non_empty = [line.strip() for line in lines if line.strip()]
        if not non_empty:
            return "GENESIS", []
        try:
            last_entry = json.loads(non_empty[-1])
            return last_entry.get("entry_hash", "GENESIS"), non_empty
        except (json.JSONDecodeError, KeyError, TypeError):
            return "GENESIS", non_empty

    def _build_payload(self, entry: dict, prev_hash: str) -> str:
    ...
    def _finalize_session_entry(
        self, entry: dict, entry_hash: str, payload: str, non_empty: list[str]
    ) -> None:
        """Finalize a SESSION_END entry by computing the Merkle root and signing it."""
        session_hashes = []
        ...
        try:
            with open(self.filepath, "a+", encoding="utf-8") as f:
                try:
                    ...
                    f.seek(0)
                    lines = f.readlines()
                    prev_hash, non_empty = self._get_previous_hash(lines)

                    entry["prev_hash"] = prev_hash
                    payload = self._build_payload(entry, prev_hash)
                    entry_hash = cortex_hash(payload.encode("utf-8"))
                    entry["entry_hash"] = entry_hash

                    if entry["event"] == "SESSION_END":
                        self._finalize_session_entry(entry, entry_hash, payload, non_empty)
