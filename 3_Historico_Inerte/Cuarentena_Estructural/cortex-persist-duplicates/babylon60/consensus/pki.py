...
class TrustMatrix:
    ...
    def get_peer_key(self, agent_id: str) -> ed25519.Ed25519PublicKey | None:
    ...
    def revoke_agent(self, agent_id: str) -> None:
        ...
        logger.critical("[TrustMatrix] AGENT REVOKED: %s. Mutational rights severed.", agent_id)
    ...
    def process_handshake(self, payload_str: str) -> bool:
        ...
        try:
            ...
            if agent_id in self._revoked:
                logger.warning("[TrustMatrix] Rejected handshake from REVOKED agent: ***id")
                return False

            if token not in self._valid_bootstrap_tokens:
                logger.warning(
                    "[TrustMatrix] Rejected handshake. Invalid/consumed bootstrap token: %s", token
                )
                return False
            ...
            try:
                ...
                logger.warning("[TrustMatrix] Malformed public key from ***id")
                return False
            ...
            try:
                ...
                logger.error("[TrustMatrix] Signature verification failed for agent: ***id")
                return False
            ...
            logger.info("[TrustMatrix] Handshake ACCEPTED. Agent joined fabric: ***id")
            return True
