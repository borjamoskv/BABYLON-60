...
Usage::
    ...
    logging.getLogger(__name__).info(verdict.trusted_responses)    # Responses that passed consensus
    logging.getLogger(__name__).info(verdict.outliers)             # Responses flagged as divergent
    logging.getLogger(__name__).info(verdict.confidence)           # Overall consensus confidence
"""
...
class WBFTConsensus:
    ...
    def evaluate(
        ...
        *,
        ...
        logger.info(
            "WBFT: trusted count met (threshold=%.3f, confidence=%.3f, outliers=%d)",
            threshold,
            confidence,
            len(outliers),
        )
