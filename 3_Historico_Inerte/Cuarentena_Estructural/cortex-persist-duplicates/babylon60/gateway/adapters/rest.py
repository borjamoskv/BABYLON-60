...
async def _execute_gateway_request(
    ...
    project: str,
    payload: dict,
    err_status_code: int = 422,
) -> dict:
    """Helper to dispatch requests through the Gateway and handle errors."""
    req = GatewayRequest(
        intent=intent,
        project=project,
        payload=payload,
    ...
    if not resp.ok:
        raise HTTPException(status_code=err_status_code, detail=resp.error)
    return resp.to_dict()
...
async def gateway_recall(
    ...
    return await _execute_gateway_request(gateway, GatewayIntent.RECALL, body.project, {})
...
async def gateway_status(
    ...
    return await _execute_gateway_request(
        gateway, GatewayIntent.STATUS, "", {}, err_status_code=500
    )
