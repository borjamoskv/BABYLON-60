...
class EpistemicPolicyNetwork:
    ...
    def route(
        ...
        if is_security_validation:
            logger.info(
                "[P0] Router: Security Validation task detected. Routing to Local MLX Engine (Autarquía)."
            )
            return "mlx-local-engine"  # type: ignore
        if requested_model and "sonnet" in requested_model.lower():
            logger.error(
                "[P0] Router: Sonnet 5 (or any Sonnet variant) is blacklisted due to Thermodynamic Anergy. Forcing bypass."
            )
            requested_model = None

        if requested_model and requested_model in self.models:
            # Bypass policy if a valid model is explicitly requested
            return requested_model  # type: ignore
