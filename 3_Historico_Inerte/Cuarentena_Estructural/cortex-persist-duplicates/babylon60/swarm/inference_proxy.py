...
async def proxy_inference(req: InferenceRequest):
    ...
    if not public_key:
        logger.critical("[InferenceProxy] REJECTED. Unknown or revoked agent: %s", req.agent_id)
    ...
    try:
        ...
        logger.critical(
            "[InferenceProxy] REJECTED. Invalid cryptographic signature from %s: %s",
            req.agent_id,
            e,
        )
    ...
    logger.info("[InferenceProxy] Authorized inference request from ***id")
    ...
    try:
        ...
        logger.error("[InferenceProxy] Upstream LLM failure: %s", e)
...
class InferenceProxyDaemon:
    ...
    def start(self):
        ...
        self._thread.start()
        logger.info("[InferenceProxy] Started Zero-Trust Proxy on 127.0.0.1:%s", self.port)
