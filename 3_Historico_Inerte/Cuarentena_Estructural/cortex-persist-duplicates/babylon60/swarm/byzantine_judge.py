    ...
    def evaluate_proposals(
        ...
        for prop in proposals:
            ...
            if not all([agent_id, code, signature_b64, timestamp]):
                logger.warning("Proposal from ***id")
                if agent_id:
            ...
            if not pub_key_b64:
                logger.warning("[AUDIT] JIT Cryptographic Enrollment for Agent: ***id")
            ...
            try:
                ...
                logger.warning("[SECURITY] Signature validation failed structurally: %s", e)
                is_valid = False

            if not is_valid:
                logger.error(
                    "[SECURITY] Cryptographic spoofing or corruption detected for agent %s. Slashed.",
                    agent_id,
                )
            ...
            try:
                ...
                logger.info("Evaluating verified AST from agent ***id")
            ...
            except JITSandboxViolation:
                logger.warning("Agent ***id")
                ...
                logger.critical("Host System Error evaluating agent %s: %s", agent_id, e)
                raise RuntimeError(f"BFT Consensus halted. Host execution degraded: {e}")

        if winning_agent:
            logger.info("🏆 Consensus reached. Winner: %s", winning_agent)
