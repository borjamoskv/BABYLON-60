...
class EntropyDaemon:
    ...
    async def _daemon_loop(self):
        ...
        while self._running:
            try:
                ...
                logger.error("[EntropyDaemon] Failure during hygiene: %s", e)
    ...
    async def _hygiene_sweep(self):
        ...
        if recovered > 0:
            logger.info("[EntropyDaemon] Cleanup successful. Recovered space: %.2f KB", recovered)
        ...
        try:
            ...
            if quarantined:
                logger.warning("[EntropyDaemon] ISOLATED: %s files by Ouroboros.", len(quarantined))
        except Exception as e:  # noqa: BLE001
            logger.warning("[EntropyDaemon] Ouroboros Immune failure: %s", e)
