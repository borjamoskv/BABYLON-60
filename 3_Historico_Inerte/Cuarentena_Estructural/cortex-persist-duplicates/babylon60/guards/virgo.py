...
class VirgoContextGuard:
        ...
        if is_valid_sig and agent_id and agent_public_key:
            ...
            try:
                ...
                if registered_key and registered_key != agent_public_key:
                    is_valid_sig = False
                    logger.error(
                        "Agent key mismatch: key passed %s... does not match registered key %s...",
                        agent_public_key[:16],
                        registered_key[:16],
                    )
            except Exception as e:  # noqa: BLE001
                logger.debug("Could not load KeyManager metadata for agent key verification: %s", e)
    ...
    async def check(
        ...
        *,
        ...
        """
        logger.debug("VIRGO CHECK CALLED! project=%s, nonce=%s", project, meta.get('nonce'))
