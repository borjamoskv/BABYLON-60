...
class DuressGuard:
    ...
    LOCK_FILE = Path("~/.gemini/config/.apoptosis_seed").expanduser()
    ...
    def execute_apoptosis(cls) -> None:
        ...
        try:
            ...
            db_path_str = os.environ.get("CORTEX_DB_PATH", "cortex_ledger.db")
            db_path = Path(db_path_str)
            if db_path.exists():
                with connect(str(db_path), timeout=5) as conn:
            ...
            logger.error("Failed to commit apoptosis to ledger: %s", e)
        ...
        try:
            ...
            cls.LOCK_FILE.chmod(0o400)
        except Exception as e:  # noqa: BLE001
            logger.error("Failed to execute apoptosis lock: %s", e)
