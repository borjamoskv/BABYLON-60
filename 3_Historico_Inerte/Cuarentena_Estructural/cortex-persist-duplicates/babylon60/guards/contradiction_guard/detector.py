...
MAX_CANDIDATES = 10
MIN_OVERLAP_SCORE = Decimal("0.10")  # Jaccard threshold for keyword overlap
EMBEDDING_BOOST_WEIGHT = Decimal("0.3")  # Max boost from embedding similarity
...
def _score_candidate(
    ...
    cosine_sim = Decimal(str(cosine_similarity(new_embedding, existing_embedding)))
    if cosine_sim > Decimal("0.5"):
    ...
    if cosine_sim > Decimal("0.8") and _jaccard(new_tokens, existing_tokens) < Decimal("0.2"):
    ...
    return ConflictCandidate(
        ...
        overlap_score=min(score, Decimal("1.0")),
