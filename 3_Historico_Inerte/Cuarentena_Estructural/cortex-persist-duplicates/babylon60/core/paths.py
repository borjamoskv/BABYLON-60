...
Environment variables::
    ...
    CORTEX_MODELS_DIR   → ~/.babylon60/models (default)
"""
...
CORTEX_DIR: Path = Path(
    os.environ.get("MOSKV_DIR", os.environ.get("CORTEX_DIR", Path.home() / ".babylon60"))
)
...
if _db_name_override == _DEFAULT_DB_NAME and not CORTEX_DB.exists() and _legacy_db.exists():
    ...
    try:
        ...
        logging.getLogger("babylon60.paths").error(
            "Failed to migrate cortex.db to runtime.db: %s", e
        )
