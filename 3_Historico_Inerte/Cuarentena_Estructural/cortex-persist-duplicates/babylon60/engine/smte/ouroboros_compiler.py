...
class OuroborosCompiler(EngineAwareMixin):
    ...
    async def _amputate_entity(self, target_path: Path, analysis: dict[str, Any]) -> bool:
        logger.info(
            "[L-EPI GUARD] FATAL: Epistemic Limerence detected in %s. dead_code_ratio=%.2f, limerence_penalty=%.2f. Automatic amputation initiated.",
            target_path.name,
            analysis["dead_code_ratio"],
            analysis["limerence_penalty"],
        )
        target_path.unlink()
    ...
    async def _compress_entity(
        self, target_path: Path, source: str, analysis: dict[str, Any]
    ) -> bool:
        logger.info(
            "Limerence detected: Maintenance Cost %s. Compressing graph...",
        ...
        try:
            ...
            logger.info(
                "OuroborosCompiler: Entity %s compressed successfully (Maximized Exergy).",
                target_path.name,
            )
    ...
    async def compile_entity(self, target_file: str | Path) -> bool:
        ...
        if analysis.get("must_amputate"):
            return await self._amputate_entity(target_path, analysis)

        if not analysis.get("is_limerent"):
            logger.info("Entity is not limerent (C <= U). No compilation needed.")
            return True

        return await self._compress_entity(target_path, source, analysis)
