...
def enforce_weismann_barrier(target_kernel_file: str, mutator_callback) -> bool:
    ...
    if not original_path.exists():
        logger.error("Kernel file %s not found.", original_path)
        return False
    ...
    with tempfile.TemporaryDirectory(prefix="cortex_somatic_clone_") as clone_dir:
        ...
        logger.info("[WEISMANN] Injecting mutation into somatic clone: %s", mutant_file)
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error("[WEISMANN] Mutant failed compilation!\n%s", result.stderr)
                return False

        except (ValueError, TypeError, KeyError, OSError, RuntimeError) as e:
            logger.error("[WEISMANN] Gauntlet execution error: %s", e)
            return False
        ...
        try:
            ...
            logger.info(
                "[WEISMANN] SUCCESS: Kernel overwritten with superior mutation -> %s", original_path
            )
            return True
        except (ValueError, TypeError, KeyError, OSError, RuntimeError) as e:
            logger.error("[WEISMANN] Hot-Swap failed: %s", e)
            return False
