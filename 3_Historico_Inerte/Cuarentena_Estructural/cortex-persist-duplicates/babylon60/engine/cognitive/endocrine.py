...
class EndocrineRegistry:
    ...
    def _sync_stress_hormone(
        self, metric_val: float, max_metric: float, reason_template: str
    ) -> None:
        """Helper to sync systemic Cortisol based on an external stress metric."""
        stress = min(1.0, metric_val / max_metric)
        current = self._hormones.get(HormoneType.CORTISOL, 0.0)  # type: ignore[reportAttributeAccessIssue]
        if stress > current:
            self.pulse(
                HormoneType.CORTISOL,
                (stress - current) * 0.5,
                reason=reason_template.format(metric_val),
            )

    def sync_with_calcification(self, index: float) -> None:
        """Ω₅-H: Sync systemic Cortisol with project Calcification Index."""
        self._sync_stress_hormone(index, 100.0, "Calcification Stress (Index: {:.2f})")

    def sync_with_free_energy(self, total_f: float) -> None:
        """Ω₅-F: Sync systemic Cortisol with Variational Free Energy."""
        self._sync_stress_hormone(total_f, 10.0, "High Free Energy Stress (F: {:.2f})")
