...
class ExergyCostScheduler:
    ...
    def __init__(self, policy_path: Path | str | None = None):
        self.policy_path = Path(policy_path or DEFAULT_POLICY_PATH).expanduser()
        self.policy = {}
        self._load_policy()
    ...
    def _load_policy(self) -> None:
        ...
        if not self.policy_path.exists():
            logger.warning(
                "[COST-SCHEDULER] Policy path %s not found. Using empty policy.", self.policy_path
            )
            self.policy = {}
            return
        ...
        try:
            ...
            logger.info("[COST-SCHEDULER] Loaded policy from %s", self.policy_path)
        except (ValueError, TypeError, KeyError, OSError, RuntimeError) as e:
            logger.error("[COST-SCHEDULER] Failed to load policy: %s", e)
            self.policy = {}
...
class BackendConfig:
    ...
    def total_cost(self, domain: str) -> float:
        ...
        # HEURISTIC ONLY (FIND-006): scheduler ranking signal, not a billing
        # figure. If this ever feeds invoicing, switch to decimal.Decimal.
