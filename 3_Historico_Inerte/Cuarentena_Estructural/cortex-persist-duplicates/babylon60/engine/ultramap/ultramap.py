...
try:
    ...
    HAS_RUST = False
    logger.warning("Failed to load CORTEX-RS: %s. Falling back to Python mmap.", e)
...
class UltramapSubstrate:
    ...
    def __init__(self, capacity=10000):
        ...
        if HAS_EVOLUTION_LEDGER:
            ...
            try:
                ...
                logger.info(
                    "EVO-LEDGER Active. Head: %s… Seq: %s",
                    self._evolution_ledger.head_hash[:12],
                    self._evolution_ledger.sequence,
                )
            except Exception as e:  # noqa: BLE001
                logger.warning("EVO-LEDGER init failed (non-fatal): %s", e)

        logger.info(
            "ULTRAMAP-Ω Initialized. Capacity: %s agents. O(1) Memory Active. (Rust: %s)",
            self.capacity,
            HAS_RUST,
        )
    ...
    def close(self):
        if hasattr(self, "_checkpoint_manager") and self._checkpoint_manager is not None:
            try:
                ...
                logger.warning("EVO-LEDGER Checkpoint sync failed: %s", e)
            self._checkpoint_manager = None
    ...
    def calculate_exergy_distance(self, agent_idx: int, target_hash: str) -> float:
        ...
        target_int = decode_base60(cortex_hash_truncated(target_hash.encode(), length=16))
    ...
    def _emit_ledger_event(
        ...
        try:
            ...
            logger.warning("EVO-LEDGER write failed (non-fatal): %s", e)
...
if __name__ == "__main__":
    ...
    logger.info("C5-REAL: Exergy Distance Calculated: %.2f Joules", joules)
    logger.info("UESS v2 Update Success: %s", success)
    logger.info("Agent 0 State: %s", umap.get_agent_state(0))
