...
def _fast_sha3(buffer: bytes | memoryview) -> str:
    ...
    return bytes_to_base60(hashlib.sha3_256(buffer).digest())
...
def generate_secure_taint_token(
    ...
    if not nonce:
        ...
        nonce = bytes_to_base60(uuid.uuid4().bytes)
...
def _query_agent_key_sync(conn, agent_id: str, tenant_id: str) -> str | None:
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT public_key FROM agents WHERE id = ? AND tenant_id = ? AND is_active = 1",
            (agent_id, tenant_id),
        )
        row = cursor.fetchone()
...
async def _query_agent_key_async(conn, agent_id: str, tenant_id: str) -> str | None:
    try:
...
def _parse_taint_token(token: str) -> dict | None:
    parts = token.split(":")
    if len(parts) < 6:
...
def _verify_token_timestamp(timestamp_str: str) -> bool:
    try:
...
async def verify_taint_token(
    conn, token: str | None, content: str, tenant_id: str = "default"
) -> bool:
    ...
    if os.environ.get("CORTEX_NO_TAINT_ENFORCE") == "1" or (
        os.environ.get("CORTEX_TESTING") == "1" and os.environ.get("CORTEX_NO_TAINT_ENFORCE") != "0"
    ):
        logger.info("[TaintEngine] Bypassing taint token verification in test/dev environment.")
        return True
    ...
    parsed = _parse_taint_token(token)
    if not parsed:
        return False
    ...
    # Allow system_bypass token in test environments
    if agent_id == "system" and signature == "system_bypass":
        import os

        if os.environ.get("CORTEX_NO_TAINT_ENFORCE") == "1" or (
            os.environ.get("CORTEX_TESTING") == "1"
            and os.environ.get("CORTEX_NO_TAINT_ENFORCE") != "0"
        ):
            logger.info("[TaintEngine] Allowing system_bypass token in test/dev environment.")
            return True

    # 1. Verify Timestamp Expiration Window
    if not _verify_token_timestamp(timestamp_str):
        return False
    ...
    if is_async:
        public_key_b64 = await _query_agent_key_async(conn, agent_id, tenant_id)
    else:
        public_key_b64 = _query_agent_key_sync(conn, agent_id, tenant_id)
...
async def _execute_saga_guards(
    conn, token: str | None, content: str, tenant_id: str = "default"
) -> None:
    # [C5-REAL] Taint Engine Enforced strictly
    ...
    try:
        _, _, _ = MemoryFirewall.screen_content(content)
    ...
    is_valid = await verify_taint_token(conn, token, content, tenant_id)
    if not is_valid:
...
def _translate_homoglyphs(text: str, homoglyph_map: dict[str, str]) -> str:
...
def _fast_pii_bypass(content: str, content_lower: str) -> bool:
    p_b = "borja"
    p_f = "fernandez"
    p_a = "angulo"
    ...
    if "%" in content:
        return False
    if HOMOGLYPH_PREFILTER_RE.search(content):
        return False
    if any(hc in content_lower for hc in PII_HEX_CHUNKS):
        return False
    if any(bc in content for bc in PII_BASE64_CHUNKS):
        return False
...
def _check_pii_leak(content: str, content_lower: str) -> bool:
    import re
    ...
    for layer in layers:
        normalized_content = _translate_homoglyphs(_strip_accents(layer.lower()), homoglyph_map)
...
async def enforce_taint_check(
    conn, token: str | None, content: str, tenant_id: str = "default"
) -> None:
    """Enforces the CORTEX-TAINT check. Raises TaintValidationError if invalid."""
    content_lower = content.lower()

    if _fast_pii_bypass(content, content_lower):
        await _execute_saga_guards(conn, token, content, tenant_id)
        return

    if _check_pii_leak(content, content_lower):
    ...
    # [C5-REAL] Taint Engine Enforced strictly (No bypass)
    await _execute_saga_guards(conn, token, content, tenant_id)
...
class MHCAntigenRouter:
    ...
    def _load_static_antigens(self):
        ...
        try:
            ...
            logger.info("[MHC] Loaded and cached SOTA static antigens from %s", sota_path.name)
        except json.JSONDecodeError as e:
            logger.error("[MHC] UltraThink P0: JSON syntax corruption in static antigens: %s", e)
            raise TaintValidationError("SOTA invariant file corrupted. Apoptosis triggered.") from e
        except (OSError, KeyError, TypeError) as e:
            logger.error("[MHC] UltraThink P0: Structural failure loading static antigens: %s", e)
    ...
    def _load_dynamic_antigens(self):
        ...
        try:
            ...
            logger.info("[MHC] Loaded dynamic antigens from %s", self.dynamic_antigens_path)
        except Exception as e:  # noqa: BLE001
            logger.error("[MHC] Failed to load dynamic antigens: %s", e)
    ...
    def _save_dynamic_antigens(self):
        ...
        try:
            ...
            logger.info("[MHC] Successfully persisted antigens to %s", self.dynamic_antigens_path)
        except Exception as e:  # noqa: BLE001
            logger.error("[MHC] Failed to persist dynamic antigens: %s", e)
    ...
    def present_antigen(self, payload: str) -> str | None:
        ...
        for agent_id, antigen_pattern in self._t_cells.items():
            if antigen_pattern.search(payload):
                logger.info("[MHC] Antigen match! Signature %s triggers ***id", payload_hash)
                return agent_id

        logger.warning("[MHC] No T-Cell match for antigen signature %s", payload_hash)
        return None
    ...
    def record_miss(self, payload: str, resolved_agent_id: str) -> bool:
        ...
        logger.info(
            "[MHC] Miss recorded for signature: '%s...' -> resolved to %s (hits: %s/%s)",
            sig[:40],
            resolved_agent_id,
            hits,
        ...
        if hits >= self.promotion_threshold:
            ...
            logger.info(
                "[MHC] 🔥 PROMOTING ANTIGEN to active mesh: %s -> %s", pattern, resolved_agent_id
            )
...
def secure_state_commit(content: str, metadata: dict) -> tuple:
    ...
    logger.info("[TaintEngine] Enforcing OP_GIT_SENTINEL for agent ***id")
    ...
    logger.info("[TaintEngine] State collapsed successfully. Ledger Hash: %s", hash_ledger)
