...
class HotSwapManager:
    ...
    def swap_weights(self, actor_weights: np.ndarray, critic_weights: np.ndarray):
        ...
        logger.info("Exported candidate model to %s", checkpoint_path)

        # Atomic rename to prevent mid-read corruption during Legion inference
        try:
            os.rename(checkpoint_path, self.onnx_model_path)
            logger.info("Atomic Hot-Swap Complete: Version %s activated.", self.current_version)
    ...
    def process_azkartu_logs(self):
        ...
        for _idx, _ts, _rec_id, payload_bytes in pending:
            try:
                ...
                ingested += 1
            except Exception as e:  # noqa: BLE001 - Deliberate fault-isolation boundary for background worker loops
                logger.error("Azkartu Ingestion failed on payload: %s", e)

        if ingested > 0:
            logger.debug("Azkartu Ingestion: Parsed %s C5-REAL experiences.", ingested)
    ...
    async def run_training_loop(self):
        ...
        while self.running:
            ...
            if len(self.replay) >= self.batch_size and ticks % update_interval == 0:
                ...
                logger.info("Retrained on %s samples.", self.batch_size)
