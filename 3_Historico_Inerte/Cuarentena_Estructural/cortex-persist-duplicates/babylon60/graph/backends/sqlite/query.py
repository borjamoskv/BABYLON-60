...
class SQLiteQueryMixin:
    ...
    async def _get_graph_entities(
        ...
        from babylon60.graph.crypto_graph import decrypt_node_name

        query_entities = "SELECT id, name, entity_type, project, mention_count, meta FROM entities WHERE tenant_id = ?"
        ...
        for row in entity_rows:
            entities.append(
                {
                    "id": row[0],
                    "name": decrypt_node_name(row[5], row[1], tenant_id),
                    "type": row[2],
                    "project": row[3],
                    "weight": row[4],
                }
    ...
    def get_graph_sync(
        ...
        if project:
            entity_rows = self.conn.execute(  # type: ignore[reportAttributeAccessIssue]
                "SELECT id, name, entity_type, project, mention_count, meta "
    ...
    async def query_entity(
        ...
        from babylon60.graph.crypto_graph import decrypt_node_name, get_blind_name

        if not name or not name.strip():
            return None

        blind_name = get_blind_name(name, tenant_id)

        q_ent = (
            "SELECT id, name, entity_type, project, mention_count, meta FROM entities "
            "WHERE name = ? AND tenant_id = ?"
        )
        params_ent = [blind_name, tenant_id]
        if project:
            q_ent += " AND project = ?"
        ...
        q_conn = """SELECT e.name, e.entity_type, er.relation_type, er.weight, e.meta
        ...
        entity["connections"] = [
            {"name": decrypt_node_name(c[4], c[0], tenant_id), "type": c[1], "relation": c[2], "weight": c[3]}
    ...
    def query_entity_sync(
        ...
        q = "SELECT id, name, entity_type, project, mention_count, meta FROM entities WHERE name = ? AND tenant_id = ?"
        if project:
            row = self.conn.execute(  # type: ignore[reportAttributeAccessIssue]
                q + " AND project = ?", (blind_name, tenant_id, project)
            ).fetchone()
        else:
            row = self.conn.execute(  # type: ignore[reportAttributeAccessIssue]
                q + " ORDER BY mention_count DESC LIMIT 1", (blind_name, tenant_id)
            ).fetchone()
        ...
        connections = self.conn.execute(  # type: ignore[reportAttributeAccessIssue]
            """SELECT e.name, e.entity_type, er.relation_type, er.weight, e.meta
               FROM entity_relations er
