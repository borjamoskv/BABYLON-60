...
class SQLiteStoreMixin:
    ...
    async def upsert_entity(
        ...
        from babylon60.graph.crypto_graph import encrypt_node_meta, get_blind_name

        blind_name = get_blind_name(name, tenant_id)
        meta = encrypt_node_meta(name, tenant_id)

        query_select = (
            "SELECT id, mention_count FROM entities WHERE name = ? AND project = ? "
            "AND tenant_id = ?"
        )
        params = (blind_name, project, tenant_id)
        ...
        query_insert = (
            ...
            "first_seen, last_seen, mention_count, meta) VALUES (?, ?, ?, ?, ?, ?, 1, ?)"
        )
        params_insert = (blind_name, entity_type, project, tenant_id, timestamp, timestamp, meta)
        if self._is_async:
    ...
    def upsert_entity_sync(
        ...
        cursor = self.conn.execute(
            ...
            "last_seen, mention_count, meta) VALUES (?, ?, ?, ?, ?, ?, 1, ?)",
            (blind_name, entity_type, project, tenant_id, timestamp, timestamp, meta),
        )
        return cursor.lastrowid
