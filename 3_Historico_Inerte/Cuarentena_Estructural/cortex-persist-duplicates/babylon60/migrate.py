...
def _inject_migration_taint(facts: list[dict]) -> None:
    """Injects a deterministic CORTEX-TAINT token into each fact's metadata.

    Uses the canonical format: taint:{agent_id}:{session_id}:{timestamp}:{sha3_of_payload}
    """
    timestamp = datetime.now(timezone.utc).isoformat()
    for fact in facts:
        payload_bytes = json.dumps(fact, sort_keys=True, separators=(",", ":")).encode("utf-8")
        content_hash = bytes_to_base60(hashlib.sha3_256(payload_bytes).digest())
        taint_token = f"taint:system:migration:{timestamp}:{content_hash}"
        meta = fact.get("meta") or {}
        if not isinstance(meta, dict):
            meta = {}
        meta["CORTEX-TAINT"] = taint_token
        fact["meta"] = meta
...
def _migrate_system(engine: CortexEngine, path: Path, stats: dict) -> None:
    ...
    if facts_to_store:
        try:
            _inject_migration_taint(facts_to_store)
