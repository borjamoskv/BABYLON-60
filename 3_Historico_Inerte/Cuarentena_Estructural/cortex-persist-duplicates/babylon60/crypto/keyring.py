...
Enforces Argon2id key wrapping before delegating to OS Keychain or .env.
"""
...
def _wrap_key(raw_key: bytes, passphrase: bytes) -> str:
    """Wraps the 32-byte master key with Argon2id-derived KEK and AES-GCM."""
...
def _unwrap_key(blob_b64: str, passphrase: bytes) -> bytes | None:
    """Unwraps the base64 encoded blob to retrieve the L0 Master Key."""
    try:
    ...
    if blob.startswith(b"v1:"):
        ...
        try:
            ...
            logger.error(
                "Failed to unwrap L0 Master Key: Invalid KDF Passphrase or tampered payload."
            )
    ...
    if len(blob) == _AES_KEY_LENGTH:
        ...
        if keyring is not None and not os.environ.get("CORTEX_TESTING"):
            ...
            if not os.environ.get("CORTEX_TESTING"):
                logger.warning(
                    "CORTEX_MASTER_KEY env var holds legacy plaintext. Update it with the new wrapped format:\n%s",
                    wrapped_b64,
                )
        return blob
...
def get_master_key() -> bytes | None:
    """Read the wrapped master key from the OS keychain or fallback to env var and unwrap it."""
    key_b64 = None
    if keyring is not None and not os.environ.get("CORTEX_TESTING"):
        try:
...
def generate_and_store_master_key() -> str:
    """Generate a new AES-256 master key, wrap it via Argon2id, and store it in the OS Keychain."""
    ...
    if keyring is None:
        logger.warning(
            ...
            "Set CORTEX_MASTER_KEY or CORTEX_VAULT_KEY manually with the generated wrapped key."
        )
        return wrapped_b64
    ...
    try:
        keyring.set_password(SERVICE_NAME, KEY_NAME, wrapped_b64)
        logger.info(
            "[C5-REAL] Successfully vaulted new wrapped CORTEX_MASTER_KEY in OS Keychain via KDF-0."
        )
    except _keyring_error_types as e:
        logger.error(
            "Could not store key in Keychain (%s). "
            "Set CORTEX_MASTER_KEY env var manually with the generated wrapped key.",
            e,
        )
