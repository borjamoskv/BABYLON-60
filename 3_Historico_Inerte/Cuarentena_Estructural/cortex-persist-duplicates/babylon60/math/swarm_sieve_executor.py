...
def execute_sieve_chunk(start_n: int, count: int, worker_id: int):
    logger.info(
        "[Worker-%s] Iniciando chunk. Rango: %s a %s", worker_id, start_n, start_n + count - 1
    )
    ...
    try:
        for i in range(count):
            ...
            logger.info(
                "[Worker-%s] Inyectado [N=%s] -> MILLENNIUM-RIEMANN-%06d | Hash: %s | Taint: %s...",
                worker_id,
                n,
                n,
                node.hash,
        ...
        conn.rollback()
        logger.error("[Worker-%s] Apoptosis de Chunk: %s", worker_id, e)
        return False
    finally:
        conn.close()
...
def orchestrate_swarm(start_n: int, total_count: int, num_workers: int):
    logger.info(
        "Levantando Ouroboros Swarm V3. Total: %s ceros, Workers: %s", total_count, num_workers
    )
