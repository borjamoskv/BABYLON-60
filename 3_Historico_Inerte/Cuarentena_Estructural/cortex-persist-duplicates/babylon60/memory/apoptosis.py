...
class ApoptosisEngine:
    ...
    def scan_for_necrosis(cls, nodes: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for node in nodes:
            ...
            if "CORTEX-TAINT" not in node.get("metadata", {}):
                logger.warning(
                    "Necrosis detected: Node %s lacks CORTEX-TAINT.", node.get("id", "UNKNOWN")
                )
            ...
            if confidence != cls.MIN_TAINT_CONFIDENCE:
                logger.warning(
                    "Necrosis detected: Sub-optimal confidence '%s' in Node %s.",
                    confidence,
                    node.get("id", "UNKNOWN"),
                )
                necrotic_nodes.append(node)
            ...
            if hours_elapsed > cls.DECAY_THRESHOLD_HOURS and not node.get("metadata", {}).get(
                ...
                logger.warning(
                    "Necrosis detected: Temporal drift (%.1fh) on Node %s.",
                    hours_elapsed,
                    node.get("id", "UNKNOWN"),
                )
                necrotic_nodes.append(node)
    ...
    def trigger_apoptosis(cls, nodes: list[dict[str, Any]]) -> int:
        ...
        for node in necrotic_nodes:
            ...
            logger.info(
                "Apoptosis triggered: Purging memory node %s to recover Exergy.",
                node.get("id", "UNKNOWN"),
            )
