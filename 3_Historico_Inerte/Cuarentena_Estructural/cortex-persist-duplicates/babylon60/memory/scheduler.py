...
try:
    from babylon60.extensions.hypervisor.belief_object import (  # pyright: ignore[reportAssignmentType,reportMissingImports]
        BeliefConfidence,
        BeliefObject,
    )
except ImportError:
...
class MemoryScheduler:
    ...
    def admit(self, belief: Any, contamination_risk: float | None = None) -> bool:
        ...
        if state is None:
            state = getattr(belief, "status", None)
        if state is not None:
            status_str = str(getattr(state, "value", state)).lower()
    ...
    def _extract_confidence_value(self, belief: Any) -> float:
        ...
        conf_val = getattr(belief, "confidence_score", None)
        if conf_val is None:
