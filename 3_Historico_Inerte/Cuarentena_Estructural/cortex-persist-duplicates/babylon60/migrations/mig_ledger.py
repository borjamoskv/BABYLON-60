...
def _migration_014_vote_ledger_refinement(conn: sqlite3.Connection):
    ...
    conn.executescript("""
        ...
        CREATE TRIGGER IF NOT EXISTS vote_ledger_no_update
        BEFORE UPDATE ON vote_ledger
        BEGIN
            SELECT RAISE(ABORT, 'ledger is append-only: UPDATE forbidden');
        END;

        CREATE TRIGGER IF NOT EXISTS vote_ledger_no_delete
...
def _migration_026_ledger_replay_admission(conn: sqlite3.Connection):
    ...
    conn.executescript("""
        ...
        CREATE TRIGGER IF NOT EXISTS ledger_replay_no_update
        BEFORE UPDATE ON ledger_replay_admissions
        BEGIN
            SELECT RAISE(ABORT, 'ledger is append-only: UPDATE forbidden');
        END;

        CREATE TRIGGER IF NOT EXISTS ledger_replay_no_delete
        BEFORE DELETE ON ledger_replay_admissions
        BEGIN
            SELECT RAISE(ABORT, 'ledger is append-only: DELETE forbidden');
        END;
    """)
...
# DOWNGRADE TARGET: 26
def _migration_027_satellite_ledger(conn: sqlite3.Connection):
    """Add satellite tables to contextualize the immutable ledger."""
    conn.executescript("""
    ...
    """)
    logger.info("Migration 027: Added satellite ledger context tables (agent_registry, sessions)")
