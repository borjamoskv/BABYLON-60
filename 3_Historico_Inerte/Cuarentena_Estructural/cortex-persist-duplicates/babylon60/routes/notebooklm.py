...
async def notebooklm_digest(
    ...
    if str(target_file) != str(base_dir) and not str(target_file).startswith(
        str(base_dir) + os.sep
    ):
...
async def notebooklm_fragment(
    ...
    if str(target_dir) != str(base_dir) and not str(target_dir).startswith(str(base_dir) + os.sep):
        raise HTTPException(status_code=400, detail="Path traversal detected")
...
async def notebooklm_sync(
    ...
    if drive_path:
        ...
        requested = os.path.realpath(os.path.expanduser(drive_path))

        if requested != home and not requested.startswith(home + os.sep):
            from fastapi import HTTPException
