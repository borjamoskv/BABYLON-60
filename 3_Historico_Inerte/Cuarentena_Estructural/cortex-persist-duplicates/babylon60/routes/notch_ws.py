...
__all__ = [
    ...
    "router",
    "HUB",
    "notify_notch_thinking",
    "notify_notch_idle",
    "notify_notch_mode",
    "notify_notch_model",
    "notify_notch_pruning",
    "notify_notch_halo",
    "flash_notch_halo",
    "notify_notch_mutation",
    "notify_notch_error",
    "notify_notch_pruned",
]
...
async def notify_notch_halo(color_hex: str, active: bool) -> None:
    ...
    await notch_hub.broadcast(f"halo:{status}:{color_hex}")


async def flash_notch_halo(color_hex: str, duration: float = 0.5) -> None:
    """Flash a colored halo on the notch for a brief duration, then turn it off."""
    try:
        await notify_notch_halo(color_hex, True)
        await asyncio.sleep(duration)
        await notify_notch_halo(color_hex, False)
    except Exception as exc:  # noqa: BLE001
        logger.warning("Failed to flash notch halo: %s", exc)


async def notify_notch_mutation() -> None:
    """Flash magenta/purple color to indicate a physical state mutation (C5-REAL write/commit)."""
    asyncio.create_task(flash_notch_halo("#E040FB", 0.5))


async def notify_notch_error() -> None:
    """Flash bright red color to indicate an execution error or Saga Abort."""
    asyncio.create_task(flash_notch_halo("#FF1744", 0.8))


async def notify_notch_pruned() -> None:
    """Flash cyan/aqua color to indicate that a fact was pruned/forgotten (Entropy Shockwave)."""
    asyncio.create_task(flash_notch_halo("#00E5FF", 0.6))
