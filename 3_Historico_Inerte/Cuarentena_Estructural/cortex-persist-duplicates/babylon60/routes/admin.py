...
def _validate_export_path(path: str | None, project: str, lang: str) -> Path:
    ...
    try:
        ...
        base_dir_str = os.path.abspath(os.getcwd())
        target_path_str = os.path.abspath(os.path.expanduser(path))
        if target_path_str != base_dir_str and not target_path_str.startswith(
            base_dir_str + os.sep
        ):
            raise HTTPException(
                status_code=400,
...
async def create_api_key(
    ...
    if existing_keys:
        ...
        bootstrap_token = os.environ.get("MOSKV_BOOTSTRAP_TOKEN") or os.environ.get(
            "CORTEX_BOOTSTRAP_TOKEN"
        )
        ...
        if bootstrap_token:
            expected_auth = f"Bearer {bootstrap_token}"
            if authorization != expected_auth:
                raise HTTPException(
                    status_code=401,
                    detail="Unauthorized bootstrap attempt. MOSKV_BOOTSTRAP_TOKEN is configured but not matched.",
                )
        elif not is_local:
            raise HTTPException(
                status_code=403,
                detail="Sovereign Bootstrap must be initiated from localhost or authorized via MOSKV_BOOTSTRAP_TOKEN.",
            )
