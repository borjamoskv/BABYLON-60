...
async def get_epistemic_dependency_graph(
    ...
    Searches for 'inject_axiom' actions created by 'autodidact', scoped to the tenant.
    """
    ...
    query = """
        ...
        WHERE action = 'inject_axiom' AND actor_role = 'autodidact' AND tenant_id = ?
        ORDER BY timestamp ASC
    """

    async with pool.acquire() as conn:
        async with conn.execute(query, (auth.tenant_id,)) as cursor:
