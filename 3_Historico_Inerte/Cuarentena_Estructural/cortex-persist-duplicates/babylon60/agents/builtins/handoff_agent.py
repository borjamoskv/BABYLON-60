...
class HandoffAgent(BaseAgent):
    ...
    def __init__(
        ...
        self._handoff_dir = handoff_dir or Path("~/.babylon60/handoffs").expanduser()
    ...
    async def _handle_handoff_request(self, message: AgentMessage) -> None:
        ...
        try:
            ...
            logger.exception(f"HandoffAgent - save_handoff failed: {exc}")
            await self._reply(
                message,
    ...
    async def _handle_task_request(self, message: AgentMessage) -> None:
        ...
        if op == "load":
            ...
            try:
                ...
                await self._reply(
                    message, {"error": "Internal server error"}, kind=MessageKind.TASK_RESULT
                )
        else:
            await self._reply(
                message,
