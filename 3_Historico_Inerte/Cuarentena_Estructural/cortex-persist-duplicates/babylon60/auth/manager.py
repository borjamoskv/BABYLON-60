...
class AuthManager:
    ...
    def __init__(self, backend: BaseAuthBackend | str | None = None) -> None:
        ...
        try:
            ...
            if hash_fn is not None:
                self._dummy_hash = hash_fn("ctx_dummy_key_to_initialize_hashing_parameters")
            else:
                raise AttributeError
        except AttributeError:
            if HAS_ARGON2:
                self._dummy_hash = PasswordHasher().hash(
    ...
    async def hash_key_argon2id_async(self, key: str) -> str:
        ...
        loop = asyncio.get_running_loop()
        # 1. Try Rust cortex_rs if available
        try:
            hash_fn = getattr(cortex_rs, "hash_password", None)  # noqa: B009
            if hash_fn is not None:
        ...
        # 2. Try Python argon2-cffi if available
        if HAS_ARGON2:
            ph = PasswordHasher()
            return await loop.run_in_executor(self._executor, ph.hash, key + AUTH_PEPPER)

        # 3. Fallback to stub (insecure) if neither is available
        logger.warning(
            "cortex_rs.hash_password and Python argon2-cffi missing. Falling back to stub."
        )
    ...
    async def authenticate_async(self, raw_key: str) -> AuthResult:
        ...
        if legacy_row and legacy_row.get("hash_algo", "sha256") == "sha256":
            ...
            try:
                loop = asyncio.get_running_loop()
                is_stub = "$stub$" in target_hash

                # 1. Try verify with Rust first if target_hash doesn't contain "$stub$"
                if not is_stub:
                    try:
                ...
                # 2. Try verify with Python argon2-cffi if not verified yet and target_hash is not stub
                if not is_valid and not is_stub and HAS_ARGON2:
                    try:
                        ph = PasswordHasher()
                        is_valid = await loop.run_in_executor(
                            self._executor,
                        ...
                        logger.exception("Python argon2 verification failed")

                # 3. Fallback to verification of legacy/stub hashes
                if not is_valid:
