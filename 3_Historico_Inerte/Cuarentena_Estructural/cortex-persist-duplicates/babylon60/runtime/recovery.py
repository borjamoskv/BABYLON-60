...
class RecoveryKernel:
    ...
    def recover(self) -> RuntimeState:
        ...
        if not state:
            ...
            logger.info("[+] Loaded Snapshot | Version: %s | Hash: %s", state.version, state.hash)

        # 2. Replay causality strictly forward
        pending_events = self.ledger.query_from(state.version)
        if pending_events:
            logger.info("[~] Replaying %s orphaned events...", len(pending_events))

        for event in pending_events:
            state.apply_event(event)

        logger.info("[✓] Recovery Complete | Final Hash: %s", state.hash)
        return state
