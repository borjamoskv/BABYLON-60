...
class HomeostaticController:
    ...
    def _emit_shadow_telemetry(self, metrics: HomeostasisMetrics, diff: ExecutionDiff) -> None:
        ...
        if metrics.drift_category == "PATHOLOGICAL_DRIFT":
            logger.error("[SHADOW-MODE] PATHOLOGICAL DRIFT DETECTED: %s", log_data)
        elif metrics.requires_action:
            logger.warning("[SHADOW-MODE] WARNING DRIFT DETECTED: %s", log_data)
        else:
            logger.info("[SHADOW-MODE] EXPECTED DRIFT: %s", log_data)
