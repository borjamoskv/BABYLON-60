...
def run_cronos_analysis(transcripts, rolling_window=5) -> None:
    ...
    logger.info("Generated CRONOS report in %s", CRONOS_REPORT)
...
def ingest_all_transcripts(log_path: str = LOG_FILE):
    ...
    for t in sorted_transcripts:
        try:
            ...
            logger.info("  ERROR processing %s: %s", t, e)
    logger.info(
        "Processed %s/%s transcripts → %s events", processed, len(transcripts), total_events
    )
