...
def die(msg: str):
    logging.getLogger(__name__).info(f"[C5-REAL PREFLIGHT FATAL] {msg}", file=sys.stderr)
    sys.exit(1)
...
def run_preflight():
    logging.getLogger(__name__).info("[C5-REAL PREFLIGHT] Initializing artifact audit...")
    ...
    logging.getLogger(__name__).info(f"Project version defined as: {project_version}")
    ...
    if not sdist_path.name.endswith(f"-{project_version}.tar.gz"):
        die(
            f"Sdist version mismatch. Expected to end with -{project_version}.tar.gz, got {sdist_path.name}"
        )

    # Inspect Wheel contents
    logging.getLogger(__name__).info(f"[C5-REAL PREFLIGHT] Scanning Wheel: {wheel_path.name}")
    with zipfile.ZipFile(wheel_path, "r") as zf:
        check_entropy(zf.namelist(), "Wheel")

    # Inspect Sdist contents
    logging.getLogger(__name__).info(f"[C5-REAL PREFLIGHT] Scanning Sdist: {sdist_path.name}")
    with tarfile.open(sdist_path, "r:gz") as tf:
        check_entropy(tf.getnames(), "Sdist")

    logging.getLogger(__name__).info("[C5-REAL PREFLIGHT] Audit PASSED. Artifacts are mathematically sound for PyPI.")
