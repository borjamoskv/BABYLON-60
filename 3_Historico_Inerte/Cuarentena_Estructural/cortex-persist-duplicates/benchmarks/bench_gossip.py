...
async def run_gossip_benchmark(node_count=6):
    logging.getLogger(__name__).info(f"🚀 Initializing Gossip Federation Convergence Benchmark with {node_count} nodes...")
    ...
    try:
        ...
        logging.getLogger(__name__).info("🧬 State injected at node_0. Starting clock for convergence monitoring...")
        ...
        if fully_converged:
            ...
            logging.getLogger(__name__).info(f"- Convergence Latency: {duration_ms:.2f} ms")
        else:
            logging.getLogger(__name__).info("❌ Failure: Did not converge within timeout.")
