...
from babylon60.extensions.evolution_ledger import ControlVector, EvolutionLedger
...
if __name__ == "__main__":
    ...
    logging.getLogger(__name__).info("VECTOR A: Evolution Ledger Concurrency Stress Test")
    logging.getLogger(__name__).info("=" * 60)
    ...
    logging.getLogger(__name__).info("\n[ Performance ]")
    logging.getLogger(__name__).info(f"Elapsed:     {result['elapsed_sec']:.3f} s")
    logging.getLogger(__name__).info(f"Throughput:  {result['ops_per_sec']:.1f} ops/sec")
    logging.getLogger(__name__).info(f"Avg Latency: {result['avg_latency_ms']:.3f} ms/write")

    logging.getLogger(__name__).info("\n[ Ledger Integrity ]")
    logging.getLogger(__name__).info(f"Status: {result['integrity_report'].get('status')}")
    if result["integrity_report"].get("errors"):
        logging.getLogger(__name__).info(f"Errors Detected: {len(result['integrity_report']['errors'])}")
        logging.getLogger(__name__).info(f"Sample Error: {result['integrity_report']['errors'][0]}")

    logging.getLogger(__name__).info("\n[ Checkpoint Matrix ]")
    logging.getLogger(__name__).info(f"Status: {result['checkpoint_report'].get('status')}")
    logging.getLogger(__name__).info(f"Chunks Verified: {result['checkpoint_report'].get('verified_chunks')}")
