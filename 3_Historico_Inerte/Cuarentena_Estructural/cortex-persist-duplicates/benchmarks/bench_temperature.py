...
async def run_trial(
    ...
    *,
    ...
    for i in range(runs):
        ...
        if verbose:
            logging.getLogger(__name__).info(f"     run {i + 1:02d} | {ms:6.0f}ms | {len(output):4d} chars", flush=True)
...
def print_report(result: GateResult) -> None:
    ...
    logging.getLogger(__name__).info(f"\n{bar}")
    logging.getLogger(__name__).info(f"  TEMPERATURE DETERMINISM GATE - {result.model}")
  ...
  Result: {status}""")
    ...
    if result.violations:
        ...
        for v in result.violations:
            logging.getLogger(__name__).info(f"    ✗ {v}")
...
def export_json(result: GateResult, path: str) -> None:
    ...
    logging.getLogger(__name__).info(f"  📄 JSON report: {path}")
...
async def main(argv: list[str] | None = None) -> int:
    ...
    logging.getLogger(__name__).info("🌡️  CORTEX Temperature Determinism Benchmark")
    logging.getLogger(__name__).info(f"   Model  : {args.model}")
    logging.getLogger(__name__).info(f"   Runs   : {runs} per temperature")
    logging.getLogger(__name__).info(f"   Mode   : {'QUICK' if args.quick else 'FULL'}")
    ...
    try:
        ...
        logging.getLogger(__name__).info(f"\n⚠️  {exc}")
        if args.allow_skip:
            logging.getLogger(__name__).info("   --allow-skip active: exiting 0 (skip gracefully)")
            return 0
        return 2
