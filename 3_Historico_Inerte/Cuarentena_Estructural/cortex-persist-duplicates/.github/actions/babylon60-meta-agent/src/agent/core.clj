(ns agent.core
  "MÖBIUS — Where Code IS Data IS Code. Sovereign Clojure Agent.
   ...
   Dispatch (via BABYLON60_ACTION env var):
...
(def ^:private label-rules
  ...
  [{:pattern #"babylon60_rs/"       :label "rust-core"    :priority :high}
   {:pattern #"babylon60/engine/"   :label "engine"       :priority :high}
   {:pattern #"babylon60/guards/"   :label "security"     :priority :high}
   {:pattern #"babylon60/isa/"      :label "isa"          :priority :high}
   ...
   {:pattern #"babylon60/mcp/"      :label "mcp"          :priority :high}
   {:pattern #"babylon60/ledger/"   :label "ledger"       :priority :high}])
...
(def ^:private issue-triage-rules
  ...
  [{:match #"(?i)bug|crash|error|broken"   :label "bug"         :priority :high}
   ...
   {:match #"(?i)rust|cargo|babylon60_rs"     :label "rust-core"   :priority :medium}
...
(defn -main
  "Entry point — dispatched by GitHub Actions via BABYLON60_ACTION env var.
  ...
  [& _args]
  (let [ctx      (gh/make-ctx)
        action   (keyword (or (System/getenv "BABYLON60_ACTION") "reflexion"))
