...
async def main():
    logging.getLogger(__name__).info("🚀 INICIANDO IGNICIÓN DEL SWARM 10k (FASE 2)")
    ...
    logging.getLogger(__name__).info(f"🧬 Generadas {len(tasks)} tareas ejecutables. Preparando SwarmCommander...")
    ...
    logging.getLogger(__name__).info(f"📡 Bus soberano inicializado con {commander.bus.num_shards} shards.")
    logging.getLogger(__name__).info("🌪️ Despachando tareas (Bucketed Dispatch con Control Cinético)...")
    ...
    logging.getLogger(__name__).info(f"✅ Despacho paralelo completado en {dispatch_time:.4f}s")
    logging.getLogger(__name__).info(
        f"📊 Latencia media de enrutamiento: {(dispatch_time / len(tasks)) * 1000:.4f}ms por tarea"
    )

    report = await commander.get_density_report()
    logging.getLogger(__name__).info(f"📈 Reporte de Densidad Termodinámica: {report}")

    if report["agents"] == len(tasks):
        logging.getLogger(__name__).info("💎 HOMEOSTASIS CONFIRMADA: 100% de agentes cristalizados y despachados.")
    else:
        logging.getLogger(__name__).info(
            f"⚠️ DIVERGENCIA: {report['agents']} agentes en L2 vs {len(tasks)} tareas solicitadas."
        )

    logging.getLogger(__name__).info("🧹 Aniquilando y consolidando estado...")
    await commander.consolidate_and_annihilate()

    total_time = time.perf_counter() - start_time
    logging.getLogger(__name__).info(f"🚀 FASE 2 COMPLETADA EN {total_time:.2f}s")
