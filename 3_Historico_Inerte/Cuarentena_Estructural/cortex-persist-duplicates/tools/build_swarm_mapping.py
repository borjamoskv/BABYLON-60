...
def main():
    ...
    for f in sorted(agents_dir.glob("*.yaml")):
        try:
            ...
            logging.getLogger(__name__).info(f"Error parsing {f.name}: {e}")
    ...
    for a in agents:
        md.append(
            f"| **{a['cat_id']}** | {a['name']} | {a['exergy']} | {a['reality']} | `{a['intent']}` | `{a['provider']}` | `{a['model']}` |"
        )
    ...
    logging.getLogger(__name__).info(
        f"Generated exhaustive mapping for {len(agents)} agents at {out_path.relative_to(workspace)}."
    )
