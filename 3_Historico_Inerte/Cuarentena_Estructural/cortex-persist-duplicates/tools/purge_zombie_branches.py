...
PROTECTED_BRANCHES = {
    ...
    "origin/master",
    "origin/gh-pages",
    "origin/ouroboros-containment",
    "origin/ouroboros-core",
}
...
def main():
    logging.getLogger(__name__).info("🧹 [C5-REAL] Iniciando purga de ramas zombies...")

    logging.getLogger(__name__).info("Sincronizando y podando remotos...")
    ...
    for branch in all_branches:
        ...
        if branch in merged_branches:
            should_delete = True
            logging.getLogger(__name__).info(f"[{branch}] Marcada para borrar (Merged in main)")
        elif is_diff_empty(branch):
            should_delete = True
            logging.getLogger(__name__).info(f"[{branch}] Marcada para borrar (Diff vs main vacío - squashed)")
        elif is_agent_dead_branch(branch):
            should_delete = True
            logging.getLogger(__name__).info(f"[{branch}] Marcada para borrar (Agente inactivo/Zombi)")
    ...
    logging.getLogger(__name__).info(f"Ramas Remotas a purgar ({len(to_delete_remote)}): {to_delete_remote}")
    ...
    for b in to_delete_local:
        ...
        if res.returncode == 0:
            logging.getLogger(__name__).info(f"✅ Local {b} eliminada.")
        else:
            logging.getLogger(__name__).info(f"❌ Error al eliminar local {b}: {res.stderr.strip()}")
    ...
    if to_delete_remote:
        ...
        for i in range(0, len(to_delete_remote), batch_size):
            ...
            if res.returncode == 0:
                logging.getLogger(__name__).info(f"✅ Remotas eliminadas: {batch}")
            else:
                logging.getLogger(__name__).info(f"❌ Error al eliminar remotas {batch}: {res.stderr.strip()}")
