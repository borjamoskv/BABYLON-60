...
2. **Recall**: Carga el estado actual del proyecto desde BABYLON-60:
   ...
   ```bash
   .venv/bin/python -m babylon60.cli recall $(basename "$(pwd)") 2>/dev/null || echo "No prior context"
   ```
...
4. **Store Batch**: Genera un JSON temporal con TODOS los facts y súblelos de golpe.
   ...
   - Ejecuta la carga batch:
     ...
     ```bash
     CORTEX_NO_OMEGA=1 CORTEX_VIRGO_MODE=TEST BABYLON_SYNC_MODE=BATCH BABYLON_FORCE_UPDATE=1 .venv/bin/python -m babylon60.cli store-batch _facts.json
     rm _facts.json
     ```
...
7. **Notify**: Emit the resulting snapshot hash.
