...
class NarrativeGraphEvaluator:
    ...
    def _safe_eval(self, expr: str) -> bool:
        ...
        def _eval(node: ast.AST) -> Any:
            if isinstance(node, ast.Expression):
                ...
                return self._world_state.get(node.id, False)
    ...
    def detect_dead_ends(self) -> list[str]:
        ...
        return [nid for nid, node in self._nodes.items() if not node.choices]
    ...
    def reachable_from(self, start_id: str) -> set[str]:
        ...
        while queue:
            ...
            if current in visited:
                continue
            visited.add(current)
            if (node := self._nodes.get(current)) is None:
                continue
            queue.extend(
                n for choice in node.choices if (n := choice.get("next_node")) and n not in visited
            )
        return visited
...
if __name__ == "__main__":
    ...
    logging.getLogger(__name__).info(f"[ConlangEngine] Generated name: {name}")
    ...
    logging.getLogger(__name__).info(f"[MagicSystem] Validation: {result.value} | Sanderson Coefficient: {coefficient:.2f}")
    ...
    logging.getLogger(__name__).info(f"[NarrativeGraph] Audit: {report}")
