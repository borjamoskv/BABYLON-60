...
def test_funnel_correlation_range() -> None:
    ...
    logging.getLogger(__name__).info(f"  - Correlation: {correlation:.4f}")
    logging.getLogger(__name__).info(f"  - Mean L2 Attention (MAL): {results['l2_mean']:.2f}")
    logging.getLogger(__name__).info(f"  - Mean L1 Revenue ($): {results['l1_mean']:.2f}")
