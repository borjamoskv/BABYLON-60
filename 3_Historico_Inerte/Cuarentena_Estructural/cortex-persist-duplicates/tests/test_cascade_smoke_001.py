...
async def test_cascade_smoke_001():
    ...
    logging.getLogger(__name__).info(f"[META-LEVEL OK] Judgment Diagnosed: {judgment.diagnosis}")

    # Control should mutate the strategy based on the judgment
    mutations = meta.control(judgment)
    assert len(mutations) > 0
    logging.getLogger(__name__).info(f"[MUTATION OK] Strategy Mutated: {mutations}")
