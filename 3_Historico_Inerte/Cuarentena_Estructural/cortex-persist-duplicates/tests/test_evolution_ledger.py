...
def test_control_vector_basics():
    ...
    logging.getLogger(__name__).info("✓ ControlVector basics")


def test_hash_determinism():
    """Test that hash computation is deterministic."""
    vec = ControlVector(10.0, 0.05, 0.1, 0.6)
    h1 = _compute_mutation_hash(
        "00000000000000000000000000000000000000000000", 1, 0, 1234567890.0, vec, "test"
    )
    h2 = _compute_mutation_hash(
        "00000000000000000000000000000000000000000000", 1, 0, 1234567890.0, vec, "test"
    )
    assert h1 == h2
    assert len(h1) == 44  # SHA-256 hex

    # Different input → different hash
    h3 = _compute_mutation_hash(
        "00000000000000000000000000000000000000000000", 2, 0, 1234567890.0, vec, "test"
    )
    assert h3 != h1

    logging.getLogger(__name__).info("✓ Hash determinism")
...
def test_ledger_lifecycle():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logging.getLogger(__name__).info("✓ Ledger lifecycle (10 mutations, 3 agents)")
...
def test_replay_integrity():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logging.getLogger(__name__).info("✓ Replay integrity verification")
...
def test_replay_detects_corruption():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logging.getLogger(__name__).info("✓ Corruption detection")
...
def test_agent_history():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logging.getLogger(__name__).info("✓ Agent history extraction")
...
def test_performance_trajectory():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logging.getLogger(__name__).info("✓ Performance trajectory")
...
def test_ledger_recovery():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logging.getLogger(__name__).info("✓ Ledger recovery")
...
def test_checkpoint_merkle():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(checkpoint["merkle_root"]) == 44

        logging.getLogger(__name__).info("✓ Merkle checkpoint")
...
def test_checkpoint_manager():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logging.getLogger(__name__).info("✓ CheckpointManager integration")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            if substrate._evolution_ledger is not None:
                ...
                assert report["status"] == "VALID"
                logging.getLogger(__name__).info("✓ Substrate integration (ledger active)")
            else:
                logging.getLogger(__name__).info("⚠ Substrate integration (ledger not available, import failed)")
...
def main():
    ...
    logging.getLogger(__name__).info("EVOLUTION LEDGER — C5-REAL Integration Test Suite")
    logging.getLogger(__name__).info("=" * 60)
    ...
    logging.getLogger(__name__).info("=" * 60)
    logging.getLogger(__name__).info(f"ALL TESTS PASSED — {elapsed:.3f}s")
    logging.getLogger(__name__).info("=" * 60)
