...
class TestPersistence:
    def test_returns_sha256_hash(self):
        ...
        expected = bytes_to_base60(
            hashlib.sha256(json.dumps(output, sort_keys=True, default=str).encode()).digest()
        )
    ...
    def test_ledger_failure_must_raise(self):
        """[C5-REAL] A broken ledger MUST halt the pipeline.

        Proceeding without confirmed audit creates unaudited actions that
        appear audited — the worst possible state for Article 12 compliance.
        The previous test asserted the opposite (fire-and-forget), which
        institutionalized silent audit loss as intended behavior.
        """
        class BrokenLedger:
            def append(self, **kwargs):
        ...
        with pytest.raises(RuntimeError, match="DB down"):
            orch._persist(_make_request(), {"data": True})
