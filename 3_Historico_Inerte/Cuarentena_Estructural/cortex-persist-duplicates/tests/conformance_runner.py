...
def run_conformance_tests():
    if not VECTORS_DIR.exists():
        logging.getLogger(__name__).info("No conformance vectors found.")
        return
    ...
    for vector_file in VECTORS_DIR.glob("*.json"):
        ...
        logging.getLogger(__name__).info(f"Executing: {vector_id}")
        ...
        try:
            if vector["category"] == "TV-SER":
                ...
                if len(hashes) == 1:
                    ...
                    if computed_hash == vector.get("expected_hash"):
                        logging.getLogger(__name__).info(f"  [PASS] {vector_id}")
                        success_count += 1
                    else:
                    ...
                    logging.getLogger(__name__).info(f"  [FAIL] {vector_id} - Hashes did not converge: {hashes}")
                    failure_count += 1
            ...
            logging.getLogger(__name__).info(f"  [ERROR] {vector_id} - {e}")
            failure_count += 1
