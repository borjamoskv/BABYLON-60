...
"""Integration tests for babylon60.engine.core.store_mixin.StoreMixin.
...
class TestStore:
    ...
    async def test_store_deduplication_bypassed_with_nonce_or_signature(self, engine):
        """Exact structural hash dedup should be bypassed if nonce or logos_signature is present."""
        content = "Deduplication target fact content to bypass."
        id1 = await engine.store(
            project="test",
            content=content,
            fact_type="knowledge",
        ...
        # 1. With nonce in metadata -> bypassed
        id2 = await engine.store(
            project="test",
            content=content,
            fact_type="knowledge",
            source="agent:test_suite",
            meta={"nonce": "nonce_unique_123"},
        )
        assert id1 != id2

        # 2. With logos_signature in metadata -> bypassed
        from babylon60.utils.canonical import cortex_hash

        nonce = "nonce_unique_789"
        expected_sig = cortex_hash(f"{content}{nonce}test".encode())
        id3 = await engine.store(
            project="test",
            content=content,
            fact_type="knowledge",
            source="agent:test_suite",
            meta={"nonce": nonce, "logos_signature": expected_sig},
        )
        assert id1 != id3
        assert id2 != id3
