...
def test_merkle_tree_root():
    ...
    leaves = [cortex_hash(b"a"), cortex_hash(b"b")]
    tree = MerkleTree(leaves)

    expected_root = cortex_hash((leaves[0] + leaves[1]).encode())
...
def test_merkle_tree_odd_leaves():
    ...
    # L2: [H(a+b), H(c+c)]
    # L3: [H(H(a+b)+H(c+c))]
    h_ab = cortex_hash((leaves3[0] + leaves3[1]).encode())
    h_cc = cortex_hash((leaves3[2] + leaves3[2]).encode())
    expected_root = cortex_hash((h_ab + h_cc).encode())
