    ...
    def test_verify_adapter_success_npz(self, mock_home) -> None:
        ...
        try:
            ...
            np.savez(
                weights_npz,
                **{
                    "layers.0.self_attn.q_proj.lora_a": np.zeros((8, 128), dtype=np.float32),
                    "layers.0.self_attn.q_proj.lora_b": np.zeros((128, 8), dtype=np.float32),
                },
            )
        except ImportError:
...
class TestAutonomousTrainingDaemon:
    ...
    async def test_run_cycle_success(self, mock_consolidate, mock_home) -> None:
        ...
        np.savez(weights_npz, lora_a=np.zeros((16, 8)), lora_b=np.zeros((8, 16)))
...
class TestLLMProviderLoRARouting:
    @pytest.mark.asyncio
    async def test_vllm_routes_to_adapter_when_verified(self, mock_home, monkeypatch) -> None:
        mock_presets = {
            "vllm": {
        ...
        import babylon60.extensions.llm._presets as presets_mod
        monkeypatch.setattr(presets_mod, "_PRESETS_CACHE", mock_presets)
