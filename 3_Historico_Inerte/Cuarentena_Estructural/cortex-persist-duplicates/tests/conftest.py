...
# Relax Ouroboros Entropy Guard latency threshold for tests (avoids CancelledError during I/O)
os.environ["CORTEX_OEG_LATENCY"] = "2.0"
...
@pytest.fixture(autouse=True)
def isolate_key_manager():
    """Ensure KeyManager's class-level fallback keyring is isolated per test to prevent xdist concurrent leaks."""
    from babylon60.crypto.keys import KeyManager

    original_keyring = KeyManager._fallback_keyring.copy()
    KeyManager._fallback_keyring.clear()
    yield
    KeyManager._fallback_keyring.clear()
    KeyManager._fallback_keyring.update(original_keyring)
...
def pytest_collection_modifyitems(config, items):
    """Skip UI tests on non-macOS platforms."""
    import sys
    import pytest

    if sys.platform != "darwin":
        skip_ui = pytest.mark.skip(reason="UI control tests require macOS")
        for item in items:
            if "ui_control" in str(
                item.path
            ):  # fspath is deprecated in newer pytest, path is preferred
