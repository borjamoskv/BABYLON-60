...
def test_queue_retries_on_provider_failure(tmp_path):
    ...
    with store.tx() as conn:
        conn.execute(
            """
            INSERT INTO ledger_events (event_id, ts, tool, actor, action, payload_json, semantic_status, prev_hash, hash)
            VALUES ('e1', '2026-03-18T00:00:00Z', 'mac_maestro', 'agent', 'click', '{}', 'pending', 'GENESIS', 'TESTHASH')
            """
        )
