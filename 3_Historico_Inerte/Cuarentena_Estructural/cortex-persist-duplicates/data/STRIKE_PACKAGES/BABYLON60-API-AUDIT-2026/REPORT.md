...
**Location:** `babylon60/routes/admin.py:22-56`
...
**Location:** `babylon60/routes/admin.py:147-164`
...
async def handoff_generate(
    request: Request,
    engine: Babylon60Engine = Depends(get_engine),
) -> dict:
    # NO AUTHENTICATION
```
...
**Location:** `babylon60/routes/tips.py`
...
**Location:** `babylon60/auth.py:141-176`
...
**Location:** `babylon60/api.py:141-198`
...
**Location:** `babylon60/api.py:201-207`
...
**Location:** `babylon60/search/utils.py:36-44`
...
**Example in `babylon60/routes/admin.py:45-46`:**
```python
...
**Location:** `babylon60/routes/admin.py:86-124`
...
**Location:** `babylon60/api.py:290-301`
...
- `babylon60/temporal.py` properly validates table aliases using `isalnum()`
- `babylon60/graph/backends/sqlite/query.py` uses placeholders correctly

### 3. **Tenant Isolation**
- Strong tenant isolation enforced across all data access
- `babylon60/routes/facts.py` properly checks `fact["project"] != auth.tenant_id`
- Tests in `tests/test_consensus_security.py` verify isolation

### 4. **Input Validation**
- Pydantic models with field validators in `babylon60/models.py`
- `MCPGuard` in `babylon60/mcp/guard.py` provides additional validation
