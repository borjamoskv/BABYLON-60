...
(define-public (swap-x-for-y
	...
	(let (
		...
		(updated-active-bin-id (if (and (or (is-eq updated-y-balance u0) (and initial-bin-balances-empty (> dx u0))) (> bin-id MIN_BIN_ID))
															 (- bin-id 1)
															 bin-id))
...
(define-public (swap-y-for-x
	...
	(let (
		...
		;; Calculate new active bin ID (default to bin-id if at the edge of the bin range)
		(updated-active-bin-id (if (and (or (is-eq updated-x-balance u0) (and initial-bin-balances-empty (> dy u0))) (< bin-id MAX_BIN_ID))
															 (+ bin-id 1)
															 bin-id))
