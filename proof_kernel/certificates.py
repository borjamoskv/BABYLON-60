...
class ClosureCertificate:
    def __init__(
        ...
        residual_microbits: int,
        epsilon_threshold: int = 1000,
    ):
        if residual_microbits >= epsilon_threshold:
            raise ValueError(
                f"Ω171 Violated: Cannot certify completeness. Residual entropy {residual_microbits} >= threshold {epsilon_threshold} microbits."
            )
    ...
    def verify(self) -> bool:
        ...
        if current_hash != self.cert_hash:
            raise ValueError(
                f"Ω171 Violated: Certificate Tampering Detected. Hash mismatch: {current_hash} != {self.cert_hash}"
            )
