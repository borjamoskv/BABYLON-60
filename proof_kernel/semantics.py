...
def verify_kernel_minimality(verifier_ast_nodes: int, generator_ast_nodes: int) -> bool:
    if verifier_ast_nodes >= generator_ast_nodes:
        raise ValueError(
            f"Ω173 Violated: Verifier complexity ({verifier_ast_nodes} AST nodes) exceeds or equals Generator complexity ({generator_ast_nodes} AST nodes). The TCB is too large."
        )
    return True
