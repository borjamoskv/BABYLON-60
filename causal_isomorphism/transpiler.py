...
from causal_isomorphism.regime_validator import RegimeValidator, ValidationReport
...
class TranspilationResult:
    ...
    def full_report(self) -> str:
        lines: list[str] = ['╔══════════════════════════════════════════════════════════════╗', '║     CAUSAL ISOMORPHISM TRANSPILER — BABYLON-60             ║', '║     Trilingual Regime: F# → {Solidity, Rust}              ║', '╚══════════════════════════════════════════════════════════════╝', '', f'Source:    {self.source_file}', f'Hash:      {self.source_hash}', f'Timestamp: {self.timestamp}', f"Status:    {('✅ VALID' if self.is_valid else '❌ REGIME VIOLATIONS DETECTED')}", '', '── IR Module ──', f'  Name:       {self.ir_module.name}', f'  Unions:     {len(self.ir_module.unions)}', f'  Records:    {len(self.ir_module.records)}', f'  Functions:  {len(self.ir_module.functions)}', f'  Submodules: {len(self.ir_module.submodules)}', '', '── Solidity Target ──', self.solidity_report.summary(), '', '── Rust Target ──', self.rust_report.summary()]
...
class CausalIsomorphismTranspiler:
    ...
    def transpile_file(self, source: Path, output_dir: Path | None=None) -> TranspilationResult:
        ...
        result = TranspilationResult(source_file=rel_source, ir_module=ir_module, solidity_output=solidity_output, rust_output=rust_output, solidity_report=sol_report, rust_report=rust_report, source_hash=source_hash)
    ...
    def transpile_source(self, source: str, module_name: str='Anonymous') -> TranspilationResult:
        ...
        self.rust_emitter = RustEmitter()
        rust_output = self.rust_emitter.emit_module(ir_module)
        return TranspilationResult(source_file='<string>', ir_module=ir_module, solidity_output=solidity_output, rust_output=rust_output, solidity_report=sol_report, rust_report=rust_report, source_hash=source_hash)
