...
from causal_isomorphism.ir import IR_BOOL, IR_BYTES, IR_FLOAT, IR_INT, IR_STRING, IR_UNIT, EmitPermission, FunctionClassification, IRDiscriminatedUnion, IRExpr, IRExprKind, IRFunction, IRMatchArm, IRModule, IRParam, IRPattern, IRRecordField, IRRecordType, IRType, IRTypeKind, IRUnionCase, RegimeLayer, ir_custom, ir_map, ir_result
FSHARP_TYPE_MAP: dict[str, IRType] = {'float': IR_FLOAT, 'double': IR_FLOAT, 'int': IR_INT, 'int32': IR_INT, 'int64': IR_INT, 'string': IR_STRING, 'bool': IR_BOOL, 'unit': IR_UNIT, 'byte[]': IR_BYTES}

def resolve_fsharp_type(type_str: str) -> IRType:
    type_str = type_str.strip()
    result_match = re.match('Result<(.+),\\s*(.+)>', type_str)
    if result_match:
        ok_t = resolve_fsharp_type(result_match.group(1))
        err_t = resolve_fsharp_type(result_match.group(2))
        return ir_result(ok_t, err_t)
    map_match = re.match('Map<(.+),\\s*(.+)>', type_str)
    if map_match:
...
class FSharpParser:
    RE_NAMESPACE = re.compile('^namespace\\s+(.+)')
    RE_OPEN = re.compile('^\\s*open\\s+(.+)')
    RE_MODULE = re.compile('^(\\s*)module\\s+(\\w+)\\s*=')
    RE_UNION_START = re.compile('^\\s*type\\s+(\\w+)\\s*=\\s*$')
    RE_UNION_INLINE = re.compile('^\\s*type\\s+(\\w+)\\s*=\\s*\\|')
    RE_UNION_CASE_BARE = re.compile('^\\s*\\|\\s+(\\w+)\\s*$')
    RE_UNION_CASE_PAYLOAD = re.compile('^\\s*\\|\\s+(\\w+)\\s+of\\s+(.+)')
    RE_RECORD_START = re.compile('^\\s*type\\s+(\\w+)\\s*=\\s*\\{')
    RE_RECORD_FIELD = re.compile('^\\s*(\\w+)\\s*:\\s*(.+?)(?:;|\\}|\\s*$)')
    RE_RECORD_END = re.compile('^\\s*\\}')
    RE_LET_FN = re.compile('^\\s*let\\s+(\\w+)\\s*(.*):\\s*(.+?)\\s*=')
    RE_MATCH_START = re.compile('^\\s*match\\s+(.+?)\\s+with')
    RE_MATCH_CASE = re.compile('^\\s*\\|\\s+(\\w+)\\s*(.*?)\\s*->')
    RE_SPRINTF = re.compile('sprintf\\s+"([^"]+)"\\s+(.*)')
    RE_COMMENT = re.compile('^\\s*//')
    RE_DOC_COMMENT = re.compile('^\\s*///\\s*(.*)')
    ...
    def parse_source(self, source: str, module_name: str='Anonymous') -> IRModule:
        ...
        self._root_module = IRModule(name=module_name, source_layer=RegimeLayer.ONTOLOGY)
    ...
    def _parse_line(self, idx: int, line: str) -> int:
        ...
        if rec_match:
            ...
            field_matches = re.findall('(\\w+)\\s*:\\s*([^;}\\s]+(?:<[^>]+>)?)', line)
            for fname, ftype in field_matches:
                if fname != rec_name:
                    self._ctx.current_record.fields.append(IRRecordField(name=fname, ir_type=resolve_fsharp_type(ftype)))
            if '}' in line:
                self._commit_record()
            return idx
        ...
        if self._ctx.current_record is not None:
            if '}' in stripped:
                ...
                if field_match:
                    self._ctx.current_record.fields.append(IRRecordField(name=field_match.group(1), ir_type=resolve_fsharp_type(field_match.group(2))))
                self._commit_record()
                return idx
            field_match = self.RE_RECORD_FIELD.match(stripped)
            if field_match:
        ...
        if fn_match:
            ...
            func = IRFunction(name=fn_name, params=params, return_type=return_type, body=body_expr, is_pure=classification != FunctionClassification.EVENT_EMITTER, classification=classification)
            if self._ctx.current_module is not None:
    ...
    def _parse_union_case(self, case_line: str) -> None:
        ...
        of_match = re.match('(\\w+)\\s+of\\s+(.+)', content)
        if of_match:
    ...
    def _parse_payload_fields(self, payload_str: str) -> list[tuple[str, IRType]]:
        ...
        for part in parts:
            colon_match = re.match('(\\w+)\\s*:\\s*(.+)', part)
            if colon_match:
    ...
    def _parse_params(self, params_str: str) -> list[IRParam]:
        params: list[IRParam] = []
        for match in re.finditer('\\((\\w+)\\s*:\\s*([^)]+)\\)', params_str):
            pname = match.group(1)
    ...
    def _parse_function_body(self, body_lines: list[str]) -> IRExpr:
        ...
        for line in body_lines:
            ...
            if in_match:
                ...
                if case_match:
                    ...
                    current_pattern = IRPattern(case_name=case_name, bindings=bindings, is_wildcard=is_wildcard)
        ...
        if current_pattern is not None:
            ...
            arms.append(IRMatchArm(pattern=current_pattern, body=arm_body))
        if arms:
            return IRExpr(kind=IRExprKind.MATCH, match_expr=IRExpr(kind=IRExprKind.VARIABLE, variable_name=match_var), match_arms=arms, requires_permission=EmitPermission.PHYSICS_COMPUTATION)
    ...
    def _extract_bindings(self, bindings_str: str) -> list[str]:
        ...
        if not bindings_str:
            return bindings
        for part in re.findall('(\\w+)', bindings_str):
    ...
    def _lines_to_expr(self, lines: list[str]) -> IRExpr:
        ...
        if sprintf_match:
            ...
            return IRExpr(kind=IRExprKind.STRING_FORMAT, format_string=fmt, format_args=args, requires_permission=EmitPermission.PURE_QUERY)
        ctor_match = re.match('(\\w+)\\s+(.+)', combined)
        if ctor_match:
            case_name = ctor_match.group(1)
            args_raw = ctor_match.group(2).strip()
            args = [IRExpr(kind=IRExprKind.VARIABLE, variable_name=a.strip().strip('"')) for a in re.split('\\s+', args_raw) if a.strip()]
            return IRExpr(kind=IRExprKind.CONSTRUCTOR, case_name=case_name, constructor_args=args, requires_permission=EmitPermission.PHYSICS_COMPUTATION)
        if combined.startswith('"') and combined.endswith('"'):
            return IRExpr(kind=IRExprKind.LITERAL, literal_value=combined.strip('"'), literal_type=IR_STRING)
        try:
            float(combined)
            return IRExpr(kind=IRExprKind.LITERAL, literal_value=combined, literal_type=IR_FLOAT)
        except ValueError:
            pass
        return IRExpr(kind=IRExprKind.VARIABLE, variable_name=combined)
