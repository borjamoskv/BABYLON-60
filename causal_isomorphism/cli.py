...
Usage:
    python -m causal_isomorphism.cli transpile domain_kernel/IRPAutomata.fs -o generated/
...
def cmd_validate(args: argparse.Namespace) -> int:
    """Validate an F# source against the Trilingual Regime."""
    source = Path(args.source)
    if not source.exists():
        print(f"❌ Source file not found: {source}", file=sys.stderr)
        return 1
...
def cmd_inspect(args: argparse.Namespace) -> int:
    """Inspect the IR generated from an F# source file."""
    source = Path(args.source)
    if not source.exists():
        print(f"❌ Source file not found: {source}", file=sys.stderr)
        return 1
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="causal-isomorphism",
        description="Causal Isomorphism Transpiler — F# → {Solidity, Rust}",
    )
