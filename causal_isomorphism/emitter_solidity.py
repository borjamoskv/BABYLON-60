...
from causal_isomorphism.ir import REGIME_PERMISSIONS, EmitPermission, FunctionClassification, IRDiscriminatedUnion, IRExprKind, IRFunction, IRModule, IRParam, IRRecordType, IRType, IRTypeKind, RegimeLayer
...
class SolidityEmitter:
    ...
    def _emit_events(self, module: IRModule) -> None:
        ...
        for union in module.unions:
            if not union.is_simple_enum:
                self._line(f'event {union.name}Committed({union.name}Tag indexed prevTag, {union.name}Tag indexed newTag, uint256 latentSteps);')
